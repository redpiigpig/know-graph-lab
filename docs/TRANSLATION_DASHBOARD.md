...
2026-09-17 也把 ACCS 那一組撤了——該叢書 2026-09-11 已完工（37,918 列／73 書卷、
稽核八項全綠），`accs-gemini` lane 更早就從 keeper 移除。完工的東西留在面板上只會
每次都掃一遍、佔掉七十列（[[feedback_disable_finished_schedules]] 同一個道理）。

**這支只管全集翻譯這一半。** 收書（z-lib／華藝）、圖書館 OCR／解析、外文電子資料庫
那幾條管線 2026-09-17 起一律看 `scripts/watch_pipelines.py`（桌面「管線對帳」捷徑，
跑一趟約 36 秒）：那支是拿產出對帳、有警訊就回 exit 1，排程叫得動；本面板是讀
checkpoint 看逐卷進度，兩者分工不重疊。
