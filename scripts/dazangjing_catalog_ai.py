...
def prompt_for(record: dict[str, Any]) -> str:
    ...
    return (
        ...
        # 這條定義原本整個缺席，模型於是自行發明「不在聖經正典 → wai」，
        # 把巴西流《聖靈論》、奧古斯丁《論信望愛》這類正統教父著作全判成外藏。
        "canon zheng/wai does NOT mean 'is it in the biblical canon'. It means acceptance by the "
        "Nicene catholic tradition:\n"
        "  zheng (正藏) = works received within the Nicene/catholic mainstream — orthodox patristic "
        "treatises, homilies, liturgy, canon law, hagiography, conciliar documents. Being outside the "
        "biblical canon is IRRELEVANT here; only collectionKey 'jing' concerns biblical canon tiers.\n"
