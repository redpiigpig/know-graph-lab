...
SYSTEM_PROMPT = """繁體中文 OCR 校正。輸入若干段以 ###S{n}### 分隔。回傳 JSON `{"cleaned":[...]}`，陣列長度必須與輸入段數相同，依序對應。

修字級錯誤：
- 服示／歐示／做示／敢示／毆示／敵示／數示／教示 → 啟示
- 默示 → 啟示
- 使國位／使西拉／便古拉／便商拉／便茜拉／便茵拉／便哥拉 → 便西拉
- 以諸／以誼 → 以諾
- 句末孤立 o → 。；句末 o J → 。」；句首孤立 - → 「
- 殘留簡體 → 繁體
- 拋棄 OCR 雜訊殘字
