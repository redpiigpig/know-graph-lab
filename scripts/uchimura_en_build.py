...
3. 語域：**現代白話文**（2026-09-10 使用者定調）。內村的英文帶維多利亞時代文風與強烈的個人告白語氣，譯文要保留那份莊重、熱切與偶爾的自嘲，但**用白話寫**——不要用句末的「也／矣／乎／哉」，不要拿「之」當「的」用。**唯一例外是詩詞韻文**（引用的詩行、讚美詩），那些照韻文體譯。引文中的聖經句子一律用《和合本修訂版》（2010）的經文（prompt 末附查好的經文時逐字照錄），除非作者刻意用了不同譯法。
...
def make_engine(backend: str = "auto"):
    ...
    def translate_para(en: str) -> str:
        ...
        te.PROMPT_TMPL = bible_quote_ref.with_hint(UCHIMURA_EN_PROMPT_TMPL, src)
