...
3. 語域：**現代白話文**（2026-09-10 使用者定調）。內村的英文帶維多利亞時代文風與強烈的個人告白語氣，譯文要保留那份莊重、熱切與偶爾的自嘲，但**用白話寫**——不要用句末的「也／矣／乎／哉」，不要拿「之」當「的」用。**唯一例外是詩詞韻文**（引用的詩行、讚美詩），那些照韻文體譯。引文中的聖經句子照 prompt 末附的經文照錄（內村讀的是欽定本，附的多半是本站照欽定本直譯的淺近文言；沒附時用《和合本修訂版》（2010）），除非作者刻意另譯或改動字句；經文照錄處是文言沒關係，正文仍用白話。
...
def make_engine(backend: str = "auto"):
    ...
    def translate_para(en: str) -> str:
        ...
        if not src:
            return ""
        # 聖經引文用欽定本直譯（kjv_zh，沒譯到的退回和修）：查出原文引的經文、交給模型照錄（bible_quote_ref）
        import bible_quote_ref
        te.PROMPT_TMPL = bible_quote_ref.with_hint(UCHIMURA_EN_PROMPT_TMPL, src, prefer="kjv_zh")
