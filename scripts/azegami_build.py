...
REGISTRY: dict[str, dict] = {
    "muky-shugi": {
        # 🚨 命名空間：內村 d0000000-…、**矢內原 e0000000-…（0001..0005）**、
        # 賀川 f0000000-…。畔上原本也寫 e0000000-…-0001/2/3，跟矢內原整組對撞：
        # 上架時 upsert 回 **HTTP 200（更新既有列）而不是 201（新建）**，
        # 表面「上架完成」，實際是把矢內原三本蓋掉；對方管線再跑一次又蓋回來，
        # 於是畔上三本從 DB 消失而 store 仍標 done。新增作者務必先掃過
        # stores/collectedWorks.ts 既有前綴，並確認 upsert 回的是 201。
        "ebook_id": "e1000000-0000-4000-8000-000000000001",
        "title": "無教會主義",
    ...
    "shodai-no-hitobito": {
        "ebook_id": "e1000000-0000-4000-8000-000000000002",
        "title": "初代的人們",
    ...
    "toyo-bunka": {
        "ebook_id": "e1000000-0000-4000-8000-000000000003",
        "title": "東洋文化的復興與基督教",
...
8. 語域：**現代白話文**（2026-09-10 使用者定調）。原文是戰前文語体，但中譯一律白話——不要用句末的「也／矣／乎／哉／焉／耳」，不要拿「之」當「的」用。**唯一例外是詩詞韻文**，那些照韻文體譯。
