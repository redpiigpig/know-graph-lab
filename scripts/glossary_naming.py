...
ADMIN_REGISTERS: list[dict] = [
    ...
    {"key": "chunqiu", "label_zh": "春秋制", "source": "列國霸業、成文法典、南方巫覡",
     "central": ["相", "令尹", "上卿"],
     "local": ["畿", "大尹", "縣公", "牧"],
     "note": "霸業立法期（漢摩拉比≈齊桓）、商業神權（巴比倫≈楚/吳越）。兩層地方：大尹（大省總管）→縣公（城邑之長）。古巴比倫、新巴比倫。"},
    ...
    {"key": "tang", "label_zh": "唐制", "source": "道州縣、節度使軍鎮、經略安撫使、四方都督",
     "central": ["宰相", "三省", "六部", "國師"],
     "local": ["道", "軍道", "節度使", "經略使", "都護", "都督", "州", "府", "縣", "留守", "方"],
     "note": "軍區/藩鎮、邊疆經營、國教。東羅馬軍區（軍道節度使）、阿拉伯-阿拔斯（道經略使）、薩珊-波斯（四方都督＋祆教國師）。"},
    ...
    {"key": "sinographic", "label_zh": "漢字圈自有", "source": "日/韓/越漢字官制（律令仿唐、武家幕府、六曹）",
     "central": ["太政官", "議政府", "六曹", "內閣", "將軍", "老中"],
     "local": ["國", "道", "府", "郡", "縣", "省", "藩", "守護", "地頭", "大名", "觀察使", "守令", "國司"],
     "note": "漢字文化圈（日/韓/越）政權：官名本即漢字，直用原稱不另譯。日本律令仿唐、武家幕府；朝鮮六曹仿明；越南阮朝仿清。"},
    {"key": "zhou_feudal", "label_zh": "周封建五等爵", "source": "《周禮》公侯伯子男、諸侯共主",
     "central": ["皇帝", "選帝侯", "大相"],
     "local": ["公", "侯", "伯", "子", "男", "大公", "君侯", "邊侯"],
     "note": "封建諸侯共主制；中世紀西歐 duke公/marquis侯/count伯/viscount子/baron男。神聖羅馬帝國＝諸侯共主（皇帝＋選帝侯＋邦國），如周天子御諸侯（二級用君侯/邊侯，較「方伯」合中古味）。"},
]
...
REGISTER_BY_POLITY: dict[str, str | list[str]] = {
    ...
    "古巴比倫": "春秋制", "新巴比倫帝國": "春秋制", "迦太基": "春秋制", "阿茲特克帝國": "春秋制",
    "新亞述帝國": "戰國秦制", "阿契美尼德-波斯帝國": "戰國秦制",
    "塞琉古-希臘帝國": "戰國秦制", "托勒密-埃及": "戰國秦制", "孔雀-印度帝國": "戰國秦制",
    "羅馬共和國": "漢制", "羅馬帝國": ["漢制", "魏晉制"], "笈多-印度帝國": "魏晉制",
    "拜占庭帝國": ["唐制", "宋制"], "阿拔斯-阿拉伯帝國": "唐制", "薩珊-波斯帝國": "唐制",
    "安息-帕提亞帝國": "遼金元制", "塞爾柱-突厥帝國": "遼金元制",
    "蒙古帝國": "遼金元制", "伊兒汗國": "遼金元制", "金帳汗國": "遼金元制", "帖木兒帝國": "遼金元制",
    "匈奴帝國": "遼金元制", "突厥汗國": "遼金元制", "印加帝國": "遼金元制",
    "鄂圖曼-土耳其帝國": "明清制", "蒙兀兒帝國": "明清制", "俄羅斯帝國": "明清制",
    "薩法維-波斯帝國": "明清制", "馬利帝國": "明清制", "暹羅": "明清制",
    "中世紀西歐封建": "周封建五等爵", "神聖羅馬帝國": "周封建五等爵", "查理曼-卡洛林帝國": "周封建五等爵",
    "衣索比亞帝國": "周封建五等爵",
    "日本（律令制）": "漢字圈自有", "日本（幕府）": "漢字圈自有",
    "朝鮮（李朝）": "漢字圈自有", "越南（阮朝）": "漢字圈自有",
}
