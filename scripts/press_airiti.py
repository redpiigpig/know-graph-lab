...
JOURNALS = {
    ...
    # ── 民間宗教
    # 民俗曲藝不在華藝的「人文學＞宗教學」學科分類底下（被歸在民俗類），
...
#    中點還有 • (U+2022) 與 ・ (U+30FB) 兩種：譯名裡的「保羅‧尼特」書目寫 U+2027、
#    華藝寫 U+2022，只差這一個字元就整篇對不上。
_PUNCT = re.compile(
    r"[\s　《》〈〉「」『』（）()\[\]【】：:；;，,、。.？?！!"
    r"‐-―─-┃﹘﹣－\-_~·‧•・．"
    r"／/\\|'\"“”‘’]+")

# 異體字摺疊。**明列字表，不做繁簡轉換**——繁簡轉換會把〈人間佛教〉與〈人间佛教〉
# 以外的東西也一起折掉，而症狀是「下載到的不是我要的那篇」。這裡只收兩組在本批
# 書目上實際踩到的正繁異體：
#   臺／台  —— 「現代臺灣佛教與印順法師」vs 書目寫的「現代台灣佛教…」，共 4 篇對不上
#   衆／眾  —— 「衆生平等」vs 書目寫的「眾生平等」
# 兩組都是同一個字在繁體中的兩種寫法，折在一起不會把不同的篇合成一篇。
# 要再加字之前先想清楚：有沒有可能兩篇文章只差這一個字而確實是不同的文章。
_VARIANTS = str.maketrans({"臺": "台", "衆": "眾"})


def norm_title(t):
    """篇名比對鍵。書目裡的篇名與華藝的篇名常差在標點、全半形與正繁異體，去掉就好；
    🚨 但**不要做繁簡轉換**，那會把不同的篇合成一篇（見 _VARIANTS 的字表原則）。"""
    s = unicodedata.normalize("NFKC", t or "").translate(_VARIANTS)
