...
TRAD_FIXES = [
    ...
    ('慄田', '栗田'),
    # 里 / 裡 — s2tw 把「里」當成「裡面」的裡，但音譯名裡的里一律是 里
    ('埃裡亞德', '埃里亞德'),      # Mircea Eliade（商周繁體譯本用這個譯名）
    ('亞裡士多德', '亞里士多德'),
    ('亞裡斯多德', '亞里斯多德'),
    ('裡爾克', '里爾克'),
    ('哈裡發', '哈里發'),
    ...
    ('託尼', '托尼'),
    ('託克維爾', '托克維爾'),
    # 燻 / 熏 — 燻是燻肉的燻；熏習(vāsanā)、熏染、熏修是佛學／中醫用字。
    # 2026-09-16 稽核：全館 34 本、562 處中鏢，集中在唯識與佛性主題
    # （《佛教的概念與方法》一本就 310 處）。來源不是 opencc（s2tw 的
    # 「熏习→熏習」是對的），是 Gemini OCR 在模型內部自己做簡繁轉換轉錯的，
    # 而這張表當時沒有這一條，於是一路寫進館藏沒有任何閘攔得住。
    # 🚨 只收「後一個字使它必定是術語」的組合，絕不可無差別把 燻 換成 熏。
    ('燻習', '熏習'),
    ('燻染', '熏染'),
    ('燻修', '熏修'),
    ('燻發', '熏發'),
