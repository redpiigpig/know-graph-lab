...
# 🚨 2026-09-09 整批下修。舊值是配合「行高多算兩成」的 fit() 訂的；
#    估算改準之後，0.78 等於「字只要小於 24pt 就拆」，結果是一堆
#    只有兩三行的「（續）」頁——兩張半空的頁比一張 20pt 的頁差。
#    0.62 ≒ 內文 19pt，仍然是投影看得清的大小。
#    不變式：FIT_FLOOR ≤ SPLIT_AT，否則 fit() 的下限夾住之後會溢出。
LADDER = [(0.62, 0.60), (0.58, 0.56), (0.54, 0.52), (0.52, 0.50)]
...
# 🚨 「本書」也要換掉。使用者 2026-09-09：「我在課堂上沒有在用講義」——
#    投影片上自稱「本書」，等於要學生去對一份他們手上沒有的東西。
#    但不能無差別取代：「一本書」「同一本書」是量詞，《日本書紀》是書名。
