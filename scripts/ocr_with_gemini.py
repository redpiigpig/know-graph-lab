...
def process_one_haiku(haiku_client, book: dict, src_path: Path, max_retries: int = 2) -> dict:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 頁數與字數都正常、內容卻是前一頁的複述 —— 見 detect_repeated_pages。
            ok, why = repetition_verdict(non_empty)
            if not ok:
                print(f"  ⚠ {why}")
                return {"status": "fail", "error": why, "transient": False}
...
def process_one(client, book, src_path, model, max_retries=3):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 同一道閘也套在 Gemini 路徑：重吐前一頁不是哪個模型獨有的毛病。
            ok, why = repetition_verdict(non_empty)
            if not ok:
                print(f"  ⚠ {why}")
                return {"status": "fail", "error": why, "transient": False}
