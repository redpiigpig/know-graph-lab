...
def _decode_best(raw: bytes) -> str:
    ...
    for codec in order:
        ...
        if s >= GOOD_ENOUGH:
            break
    return best if best is not None else raw.decode("big5", "replace")
...
def _build_index(tree: str) -> tuple[str, int]:
    """逐檔轉 UTF-8，生一份涵蓋全部正文檔的目次，回傳 (進入點, 篇數)。"""
...
def convert(src_chm: str, dst_epub: str, title: str = "") -> int:
    """轉一本。回傳收進去的正文篇數；失敗丟例外。"""
    work = tempfile.mkdtemp(prefix="chm2epub_")
    try:
        _extract(src_chm, work)
        tree = _pick_tree(work)
        index, n = _build_index(tree)
...
if __name__ == "__main__":
    ...
    n = convert(sys.argv[1], sys.argv[2], sys.argv[3] if len(sys.argv) > 3 else "")
    print(f"✓ {n} 篇　{os.path.getsize(sys.argv[2]) / 1024:.0f} KB")
