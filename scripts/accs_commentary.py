...
FATHER_FIXES: dict[str, str] = {
    ...
    "屈梭多模": "金口若望",
    "屈稜多模": "金口若望",   # OCR 稜/梭 誤認（太14-28 掃描，18 筆）
    "金口約翰": "金口若望",
    "約翰·屈梭多模": "金口若望",
    "約翰屈梭多模": "金口若望",
    ...
    # 波提亞的希拉流 (Hilary of Poitiers)；OCR 被/波 誤認（太14-28，2 筆）
    "被提亞的希拉流": "波提亞的希拉流",
    # 亞波里拿留 (Apollinaris)；OCR 旨/留 誤認（太14-28 斷片，1 筆）
    "亞波里拿旨": "亞波里拿留",
    # 偽託類統一「託名X」（Pseudo-）
    "託區利羅名作品": "託名區利羅",
    "託革利免名作品": "託名革利免",
...
_REF_RE = re.compile(
    r"""
    # 章節分隔符：ACCS 各卷體例不一，冒號與**句點**混用（'1:1-4' 與 '1.1-4' 都有，
    # 全庫 33% 是句點式；馬太 1-13 高達 79%）。只認冒號會讓句點式被當成「單獨一節」，
    # 於是 build_rows_auto 往前沿用上一章 → 整章註釋被塞進錯的章（2026-08-19 查出）。
    (?:(?P<chap>\d+)\s*[:：.．]\s*)?  # 可選章號（'1:' / '1.'）
    (?P<v1>\d+)                     # 起節
