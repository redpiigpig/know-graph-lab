...
## 語感（向元文琪譯本借的，實測自 789 節逐節對齊資料）

商務印書館元文琪譯《阿維斯塔》是華語世界唯一有份量的祆教經典譯本。本站另譯，
但**句法上向它看齊**，好讓讀者並排對照時不覺得是兩個世界的東西。可借的有三項：

9. **短句。** 他每節平均 87 字、3.9 句，**平均每句 22 字**。祆教經文是誦唸用的，
   長句一堆逗號會唸不下去。譯文請斷成短句，寧可多一個句號。
10. **呼格自成一句，不併進下一句。** 英譯的 "O Mazda!"、"O Maker of the material world!"
    一律獨立，後面接句號或驚嘆號再起新句——他 29% 的節裡都這樣處理。
11. **保留條列與重複的分行。** 同一問答換一個對象問十次，就分十行，不要併段。

🚨 **只借句法，不借專名。** 他譯自波斯文選編本，神名用的是**波斯語形式**
（巴赫曼＝沃胡‧馬納、梅赫爾＝密特拉、阿赫裡曼＝安格拉‧曼紐、法裡東＝斯萊塔奧納）。
本站正文譯的是阿維斯陀語原典，**專名一律以下方詞表為準**——
拿波斯語形式去譯阿維斯陀語原文，等於把中古波斯的用語套回一千多年前。

🚨 **也不要學他的方括號。** 他有 63% 的節帶方括號補字，那是他的波斯文底本的
編者補入，不是本站英譯有的東西。方括號只在英譯本身有的時候才保留（見第 5 條）。
