...
最後：腳註。**腳註一定要收**（研究者要靠它核對出處，漏掉等於毀了這一頁）。
  ...
  * 🚨 **註文內部的「1.」「2.」是引文列舉，不是註號。**一條註常含好幾段引文，
    印成「1. ⋯⋯　2. ⋯⋯」。那整串屬於**同一條註**，要寫成一行
    `[^13]: 1. ⋯⋯ 2. ⋯⋯`，**絕不可拆成 `[^1]:` 與 `[^2]:` 兩條**。
    判準：註號是註文區每一條**開頭的小字**（與正文上標同一個數字）；
    列舉號是正文大小、後面跟著「.」或「、」。
  * 🚨 **註號認的是正文裡那個上標**。先看正文有哪些上標數字，註文區就該有
    對應的同樣數字，兩邊必須對得起來；對不上時以**正文上標**為準。
  * 本書註號**每章從 1 重編**，所以同一個數字會在不同章重複出現，這是正常的；
    但**同一章之內**必須是 1、2、3⋯ 依序遞增，不跳號也不重複。
  * 該頁沒有腳註就完全不要寫 `[^…]`。
