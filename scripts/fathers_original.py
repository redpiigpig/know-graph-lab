...
def work_name(chapter_path: str) -> str:
    ...
    # 中文數字的章號後綴（「第六卷 第二十二章」）。2026-09-11 修 chapter_path 被
    # 正文吞掉那批之後才冒出來——修好的路徑帶的是章名原樣的中文數字。
    # 🚨 條件要窄：**前面要有空白**、**結尾是章或節**。少了前一個條件，
    #    prefix 自己（「第一卷」）會被剝成空字串，本來對得上的那幾段反而全斷；
    #    少了後一個，「第三十九至二十六條」這種乾淨路徑會被砍頭。
    name = re.sub(
        r"(?<=\S)\s+第[\d一二三四五六七八九十百]+"
        r"(?:\s*[-–—至]\s*[\d一二三四五六七八九十百]+)?[章節]\s*$", "", name)
