...
🚨 **這裡量的是「兩頁一不一樣」，不是「內容是不是胡謅」**。原書本來就重複的話，
   它一樣會判不合格。已知的誤判類型：**同時收全文與節本的書**。
   《佛光祈願文》（星雲大師全集第 46 卷）就是：官網每篇祈願文同時發全文與「精要版」
   兩頁，而精要版登的是同一段文字 ——

       p036  為父母親友祈願文            537 字
       p040  為父母親友祈願文(精要版)     537 字  ← 與 p036 逐字相同

   重複來自來源網站，書本身沒問題。判定不合格之後**先看一眼再動資料**：
   看 chapter_path 有沒有透露那是原書結構（節本、選錄、附錄重刊）。

   一個便宜的分辨法：**同一個頁號重複（pN≈pN）來源不可能長那樣，一定是管線問題；
   不同頁號（pA≈pB）才需要人看**。2026-09-07 那批 82 本裡 78 本是同頁號，
   另外 4 本人工看過也都確實是幻覺（p8 是 p7 砍掉開頭一句後重吐那種）。
"""
