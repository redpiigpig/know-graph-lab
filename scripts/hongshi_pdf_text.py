...
🚨 **站上的全文不是從這些 PDF 抽的，兩件事不要混。**
   R2 已經有 1,831 份全文（`yinshun-hongshi-fulltext/` 與 `dadaodao-fulltext/`
   兩個前綴），那是從各刊**網頁**抓的。所以「這個 PDF 沒有文字層」
   不代表「站上這一期沒有全文」——實測玄奘佛學研究 v17／v41 是純掃描
   （0 個字型、每頁一張圖、0 個文字 block），但 R2 上那兩期的全文
   有 1.3 萬與 1.8 萬漢字，好好的。
   **要判斷站上缺不缺，去數 R2，不要去看 PDF 的文字層。**

🚨 **不要 OCR。** 跟 J-Stage 那批一樣，這些 PDF 多數有文字層，PyMuPDF
   直接抽得到，整批走 OCR 會白燒配額而且品質更差（見 [[feedback_ocr_strategy]]）。
   全帙逐檔驗過，真正是純掃描的只有三期：

       弘誓雙月刊-103          64 頁・每頁 35 字・56 頁全空
       玄奘佛學研究-v17        36 頁・0 字・無字型
       玄奘佛學研究-v41        36 頁・0 字・無字型

   其中玄奘那兩期站上已有全文（見上），所以**真正的缺口只有弘誓 103**。
   另外「每頁字數低」不等於純掃描——玄奘 v01 每頁只有 474 字，但它有
   4 個字型、6 個文字 block，只是篇幅短又多圖版頁。判準要看字型與
   文字 block，不能只看字數。
