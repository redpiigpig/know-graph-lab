...
META_MARKERS = (
    ...
    "抱歉", "無法進行翻譯", "這段文字似乎",
    # 🚨 "作為一個" 拿掉了（2026-09-17）：它會抓到正常譯文（「以色列作為一個整體」、
    # 「現實作為一個生命有機體」）。ACCS 那次稽核就是被它誤報的，這裡再犯一次。
    # 要抓「作為一個 AI」這種自稱，就寫明確一點：
    "作為一個 AI", "作為一個語言模型", "作為一個人工智慧",
    ...
    # 🚨 第三批（2026-09-17）：這三條是在「疑似臆造」那一堆裡撿到的——它們其實是
    # 元回覆，只是開場白換了說法，沒被判準明確的那一類抓到，就一路留到啟發式那邊。
    "我已準備好翻譯", "閣下提供的", "此處英文原文",
)
META_WINDOW = 40
