...
def rules(c):
    ...
    if topic:
        out.insert(4, ('口頭報告', topic + [
            '分組與報告順序於期中考前公告；每組報告後須繳交書面大綱一份。',
            '報告當週未到者，該項成績以零分計。',
        ]))
    return out
...
def build(key):
    ...
    row(t, ['授課教師', CS.TEACHER, '聯絡方式', '課後當面洽詢，或透過系辦公室轉達'])
    for r in t.rows:
    ...
    para(doc, '＊表中章節為授課參考範圍，非逐章講畢不可；教師得視課堂實際進度合併或取捨章節，'
              '調整一律於課堂公告，考試範圍以公告為準。',
         size=10, indent=0.2, space_after=2)
    para(doc, '＊第 17、18 週為自主學習與文本閱讀週，不到校上課；'
              '請自行研讀指定章節，並於期限前與教師個別討論自評。',
    ...
    # ── 六、本學期國定假日 ──
    heading(doc, '六、本學期國定假日與上課日')
    para(doc, '連假期間照常上課，只有上課日剛好是國定假日當日才停課順延。', indent=0.2, space_after=3)
    t = grid(doc, [4.6, 4.4, 8.0])
    shade(row(t, ['假期', '名稱', '本課程受影響情形'], bold=True).cells)
    for d, name, effect in CS.HOLIDAYS:
        row(t, [d, name, effect])
    para(doc)
