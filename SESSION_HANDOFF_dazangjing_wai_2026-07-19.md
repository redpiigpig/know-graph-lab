...
**背景 task id `w5asbfld3`／run id `wf_5145d9ee-e44`**，於交接時仍在跑（research → consolidate → verify）。

**接手方式（擇一）**：
1. **若該 run 已完成**：讀 `C:\Users\user\AppData\Local\Temp\claude\...\tasks\w5asbfld3.output` 取 `result.proposed`；若 verify 部分撞額度，改從 journal salvage（見下）。
2. **若 run 已失效／要重跑**：**fresh 重跑並重傳完整結構化 args**（切勿 bare-resume，會搞丟 args）。args 內容見本檔第五節。
