...
- ✅ **c12 草稿 v7 重排＋修改草稿 Word 下載（2026-07-04）** — 依使用者新大綱把 `c12-revision-draft.md` 重排為五節（一、前言：方法與材料＋**「聖嚴法鼓系統」邊界定義**；二、共同背景＝佛學教育兩條平行建制史＋大專學佛運動＋共性；三、印順×聖嚴互動與同異（含福嚴住持之邀）；四、跨系人物＋三樁事件（教育機構交集／**CBETA 兩系共構**（惠敏李志夫杜正民＋恆清北美百萬美元＋福嚴 Y 輯）／印順學研討會與祝壽文集）；五、結論），基礎視角＝作者 j1「印順學的興起」接受史雙範式；源檔同步 `scripts/data/c12_期刊改寫_修改草稿.md`。**Word 下載**＝[server/api/works/draft-docx.get.ts](../../../server/api/works/)（讀 `<ref>-revision-draft.md` 即時 md→docx：#/##/### 標題、**粗體**、>引文、-條列、〔註N〕小字；A4/NSimSun/首行縮排），「修改草稿」分頁 header 加「⬇ 下載 Word」鈕（`/api/works/draft-docx?ref=<paper_ref>`），任何有 revision-draft 的 paper 計畫通用。
