...
- ✅ **c12 v8 定稿 Word（真腳註＋標楷體引文，2026-07-04）** — 使用者自改一二節後放根目錄 `…(改寫檔).docx`；[scripts/build_c12_docx.mjs](../../../scripts/build_c12_docx.mjs)（npm docx 9，`FootnoteReferenceRun`）重建 `…(改寫檔·腳註版).docx`：一二三節照錄使用者散文、四節列點改敘事散文、35 條〔註N〕→頁下真腳註（號碼一對一）、印順1965覆信＋聖嚴「學者風貌」兩段抽成**標楷體獨立引文**、內文新細明體12pt首行縮排、末附徵引書目四類（歷史論文格式）。同腳本同步輸出網站版 md（`{{N}}`→行內〔註N〕），DATA 陣列單一資料來源；要改內文→改 DATA/NOTES 再 `node scripts/build_c12_docx.mjs`。根目錄兩個 docx 不進 git。
- ✅ **c12 草稿 v8 腳註版散文定稿＋Word 下載（2026-07-05）** — 使用者要求「不可列點式、要腳註、引用用標楷體、符合歷史論文架構」。① 第四節跨系人物由列點改寫成**敘事散文**（李志夫/藍吉富/惠敏/清德/林建德每人一段、厚觀/昭慧一段）；② 兩段一手引文（印順 1965 覆聖嚴信「八敬法…當重視平等性」、聖嚴〈護教思想〉「只會看病而拙於治病的學者風貌」）改為**標楷體獨立引文段**；③ 35 條〔註N〕→**真‧頁下腳註**。**權威產檔器＝[scripts/build_c12_docx.mjs](../../../scripts/)**（`node scripts/build_c12_docx.mjs`）：DATA 陣列存正文、NOTES 陣列存 35 註，用 docx `FootnoteReferenceRun`+`footnotes` 產真腳註；內文新細明體+Times/引文標楷體/A4/25mm/首行縮排；**同時輸出**根目錄交付檔「…(改寫檔·腳註版).docx」＋回寫網站 md（`public/content/works/c12-revision-draft.md`＋`scripts/data/c12_期刊改寫_修改草稿.md`，行內〔註N〕+文末「## 註釋」）。
  - **Word 下載端點升級**＝[server/api/works/draft-docx.get.ts](../../../server/api/works/)：讀 `<ref>-revision-draft.md`，**解析文末「## 註釋」段 → 真腳註**（行內〔註N〕轉 `FootnoteReferenceRun`、註釋段本身不重複印）、`>` 引文渲染成**標楷體**獨立段、`-` 條列→懸掛縮排書目、摘要前短行置中（作者/系所）。與 build 腳本同版式，任何遵此慣例（行內〔註N〕+文末註釋清單）的 paper 草稿通用。「修改草稿」分頁 header「⬇ 下載 Word」鈕（`/api/works/draft-docx?ref=<paper_ref>`）。已 :3000 實測：35 腳註順序正確、2 標楷體引文、無殘留標記。
...
- ✅ **卷／期／起訖頁：可引用性（2026-09-10）** — 這條管線的用途就是供論文寫作引用，原本 schema 卻**連一個頁碼欄位都沒有**（venue＋year 不足以讓讀者回到原文）。[database/lit-review-add-citation-locus.sql](../../../database/lit-review-add-citation-locus.sql) 已跑：`lit_review_entries` 加 `volume`／`issue`／`pages`（都是 TEXT——有「3-4 合刊」「e12345」這種寫法），`lit_review_sections` 加 `page_number` INT。書目那一行由 `lit_review.parse_citation_locus()` 抽（認得「第 25 號」「3 卷 1 號」「vol. 17, no. 2, pp. 91-122」，漢數字「第五號」正規化成 5），`ingest_lit_review.py --seed` 一併入庫。
...
- ✅ **第三案 pong-pastoral-spirituality〈龐君華會督的衛斯理神學實踐與「新修道主義」願景〉（2026-09-15）** — 台灣宗教學會 2026 年會（10/23-24，輔大濟時樓，主題「靈性運動、療癒與諮詢」）投稿。`public/content/papers/pong-pastoral-spirituality.txt` 只存**摘要層**工作文本，全文論證在 `public/content/works/pong-pastoral-spirituality-revision-draft.md`（v0.4，47KB，含兩個 markdown 表格與大量 `[龐會督典藏](url)` 連結）。
  ...
  - **交付位置**：`G:\我的雲端硬碟\玄奘\博一上\投稿\2026.10.23-24 台灣宗教學年會\`（成品不進 git）。同夾有大會徵稿公告與個別論文提交資料表；**公告未規定撰稿體例與附錄位置**，採「徵引書目→附錄」序。
  - 🚨 **引註走史學體例、不用作者—年份**（使用者 2026-09-15 定）：29 條夾註（龐君華2022a 這種）全轉成頁下註，首見出全稱、再見用簡稱；書目同步改成「作者，題名，出版項，年份」的徵引書目體例，`## 註釋` 段附在 md 最末（產檔器讀它轉 `w:footnote`，且該段本身不印）。
  - **投稿版 vs 工作稿**：`--omit=進一步研究與定稿清單` 把工作用待辦整節排除，站上草稿仍保留該節。書目區（`## 參考文獻`／`參考書目`／`徵引書目` 底下）自動改懸掛縮排。
  - 🚨 **材料性質一律標示**（使用者 2026-09-15 要求）：學位論文與講座逐字稿標（未出版）、口訪標「口述訪談紀錄（未出版）」並自成一類、二手研究文獻另立一類；§（三）研究方法補「材料建置與研究倫理」兩段（經師母楊肇悅同意訪談 → 《無境界者》9 紀念專輯 → 建置數位典藏；作者兼建置者與詮釋者的自覺）。
  - 🚨 **龐君華會督數位典藏（nonchurch2025.com/pong-archive）尚未公開**——俟著作權授權處理完成才上線，目前是登入牆。書目**不可掛該站連結**（投出去是死連結），一律寫「龐君華會督數位典藏（尚未公開），文稿／影音編號 N」。`/articles/`（《無境界者》）是公開的，連結照掛。
...
- 🧹 **拒答／亂碼清空重譯（2026-09-25）**：[scripts/lit_review_retranslate.py](../../../scripts/lit_review_retranslate.py)（`scan`→`clear`→`run`→`status`）。偵測主判準＝`translation_fix.clear_reason`，另補整段「????」、模型自己續寫原文沒有的英文、書目段豁免（`is_bib`）。分母 71,667 段（排除 language=zh 3,401），命中 **3,643 段／349 篇**（回話 2,824、U+FFFD 788、續寫英文 20、????11），抽查 30 筆零誤判；清空＝**刪掉 zh 列**（本表「未翻」的表示法，reader 只顯示原文），原值備份 `output/lit_review_backup_2026-09-25.json`。🚨 U+FFFD 原文欄 0 例＝翻譯時模型 byte-fallback 多吐的一個字元，不是切段——777 段剝掉即可用、直接寫回；原文本身是連結按鈕／PDF 控制字元的 753 段留白不重譯（要救得從原 PDF 重抽）。其餘 ~2,100 段由 fleet_keeper lane `litreview-retrans` 逐段補譯（Gemini→NVIDIA→Haiku，每段寫入前過關卡，進度檔 `output/lit_review_retrans/progress.json`，全部補完才印 `LITREVIEW_RETRANS_COMPLETE` 自動退場）。🚨 已知切段問題（未修，屬 lit_review.py）：〈Taixu〉(4815) 原文被切成半句碎片；entry 440 #37 中英錯位。
---
...
- **2026-09-06 全量結果**：110 筆 → 已核實 63／版本年份不同 46／**查無 0**／查詢失敗 1
  （Comte-Sponville，429，其英譯本 Open Library 查得）。另驗一道：**查得年份「早於」報告首版年的 0 筆**
  ——與「46 筆全是再版」一致。這一驗值得每次都跑，早於首版才是真錯，晚於只是版本。
