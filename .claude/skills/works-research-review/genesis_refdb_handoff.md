...
| B1 空無與創生前態 | 7 | ✅ 39 | ✅ 9 | `..._B1.md` + `_zh` |
| B2 默然與不可言說 | 6 | ✅ 36 | ✅ 8 | `..._B2.md` + `_zh`（臨在世界論已移 V3） |
| B3 神聖、虛無與終極存在 | 7 | ✅ 39 | ✅ 8 | `..._B3.md` + `_zh` |

**🎉 全部 15 卷完成（M/E/O/V/B 五系列英文＋中文皆齊）。累計約 663 筆。**

## 剩餘工作
- **書目層全部完成，無待辦卷**。後續可選：
  1. 全文逐段轉錄（OA only）：`python -X utf8 scripts/ingest_lit_review.py --fetch-fulltext --project genesis-philosophy --resume --engine gemini --pace 1`（見下方「全文逐段轉錄」段；可加 `--book-id <BID>` 單卷）。
  2. 抽查各卷 reader（`/works/genesis-philosophy/book/<BID>`「研究回顧」分頁）顯示是否正常、四領域徽章與章節徽章對齊。
  3. 若使用者要擴充某卷某領域，照「每卷標準流程」單開該領域代理補入即可。
- ⚠️ 卷內跨領域 ref_key 去重已做（B3 與 V2 神聖/虛無、B1 與 O1 海德格/空性 等跨卷重覆＝允許，只去**卷內**重覆）。
