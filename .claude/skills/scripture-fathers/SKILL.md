...
### 🚨 三欄稽核：`scripts/audit_fathers_columns.mjs`（2026-09-11）

補完第三欄不等於呈現正確。稽核工具**直接跑 reader 那支 `buildParallelColumns`**，
量使用者實際看到的列：
...
- 取源走 **R2**（線上 reader 讀的就是它），所以 `G:` 沒掛載照跑。
- 🚨 **不要在外面重寫一套「兩欄段落數相不相等」的判準。**對齊的真正邏輯是
  `alignByAnchors`（章節號／`{{p:NNN}}` 錨點 → 最長遞增鏈 → 區間填充），近似它會把
  十九本全判成壞掉——第一版就是這樣，全紅，而那是判準錯不是資料錯。
- 🚨 **重複原典要設長度下限**（現為 500 字）。兩封信引同一句經文（23–42 字）不是病；
  巴西流《書信集》曾這樣誤報 5 組。

**首輪結果（19 本中 16 本乾淨）**，三本的病因完全一樣——**整卷原典沒有切片，
原封不動掛在該卷每一段上**，讀者翻到該卷任何一頁，原典欄都是整卷：

| 書 | 症狀 |
|---|---|
| 希拉里《論三位一體》十二卷 `709f43f9` | 9 組害到 **210/222 段**（95%）；一塊 48,823 字的拉丁掛在連續 36 段。原典溢出 94.3% |
| 蘇格拉底／索佐門《教會史》 `29782dd6` | 2 組害到 25/80 段；31,340 字希臘文掛在 16 段。溢出 50.6% |
| 奧古斯丁《上帝之城》 `1eb50be9` | 1 組害到 12/90 段，集中在卷十七。溢出 29.4% |

修法：這三本要回頭把原典**按章切片**再重掛，不能整卷塞。修完重跑本稽核收斂到 0。
