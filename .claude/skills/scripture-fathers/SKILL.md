...
🚨 **居普良那一部的卡點是我們自己的分段，不是取源。** 拉丁全本 Corpus Corporum
   十八部都有（作者 idno 878），但站上「居普良 論述集」的**第 83 段是一個 140,898
   字、445 個正文段的巨塊**，裡面裝了《論教會的合一》到《論嫉妒與羨慕》十部各自
   獨立的著作，每一部的編號都從一重來。同一個數字在那一段裡指十個不同的地方——
   `_both_anchors` 因此把 17 個章標題全部丟掉（它們的號碼都已經被別部著作的節號
   佔用），最後只認得出 151 個彼此矛盾的節號。**要先用 multi_h3_splitter 把那一段
   按著作重切**，切完就能直接照 `cc-book` 收。
