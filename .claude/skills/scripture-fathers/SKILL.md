...
### 🚨 三欄稽核與重切（2026-09-11，19 本全綠）

補完第三欄不等於呈現正確。稽核**直接跑 reader 那支 `buildParallelColumns`**，量使用者
實際看到的列：
...
🚨 只讀 R2 會量到過期的東西——修完本機、還沒同步時跑，數字一動也不動，看起來像
「修了沒用」。反之只讀本機會漏掉「線上其實還沒更新」。**兩邊都要驗。**

#### 病灶與修法

首輪 19 本有 13 本的原典欄整卷重複。往下追，**病灶不在補原典那一步**——
`fathers_add_original.py` 本身逐段對齊、實測命中 100%。真正的病灶是 CCEL 老毛病
「章節標題吞內文」的殘留：那些段的 `chapter_path` 不是路徑，而是**「正確章名＋整段
正文」**（最長 785 字）。路徑認不出來 → `spans_for` 跳過 → 保留建書當初掛的整卷。
英文欄同病，而且早於原典欄。
...
| 步 | 工具 | 做什麼 |
|---|---|---|
| 1 | `fathers_repair_chapter_paths.py --apply` | 比對全書標題池截斷、章號後沒分隔符就只留章號、逐段補卷次 |
| 2 | `fathers_add_original.py --work … --apply` | 逐段重建原典欄 |
| 3 | `fathers_clear_dup_sources.py --lang la\|grc --apply` | 清掉步驟 2 對不上、仍整塊重複的 |
| 4 | `fathers_reslice_en.py --apply` | 英文欄按 `{{p:NNN}}`＋段首節號就地切 |

🚨 **先重建再清**。對齊器只覆寫它對得上的段；先清會讓對不上的段永遠空著。

#### 這條線特有的六個「看起來成功的失敗」

1. **不要在外面重寫對齊判準。** 第一版在 Python 端用「兩欄段落數相不相等」近似
   `alignByAnchors`，19 本全紅——判準錯不是資料錯。要問「呈現對不對」就跑呈現的那支。
2. **標題池要取全書的。** 一章橫跨多段而 `## ` 只在第一段；只看自己的話 211 段裡有
   71 段永遠修不到，而那正是整卷重複的大宗。
3. **「索引 第3章」是合法後綴不是黏上的正文。** 只比對前綴會誤砍 11 段——把好路徑改壞。
4. **卷次傳遞要在 `volume` 換時歸零。** 否則傳到隔壁那部著作，命中率照樣好看而配到
   別部的原文（實測希拉里卷一因此掉到 0/48）。
5. **`work_name` 擴充要窄**（前面要有空白、結尾是章／節），否則 prefix 自己
   （「第一卷」）被剝成空字串。
6. **重切要對「起點相同的段」去重。** 同一頁被切成三段、三段都只帶那個 `{{p:NNN}}`，
   不去重就拿到一字不差的切片——重複只是從「整卷」縮成「一小塊」，更難查覺
   （優西比烏殘留 8 組／13 段）。

#### 重切切不精確的那一段，兩條走不通的路（別再試）

* **累計段落數**：中文翻譯時被重新分段，段落數是英文的 1.7 倍（卷六 228 對 125）。
* **只用節號**：只有 39% 的段撈得到起始節號。

沒有錨點的段**留白**，不沿用前一段的切片。留白那幾段的英文會落在前一個有錨點的段裡
（切片一路切到下一個錨點），**一個字都沒丟**，只是集中顯示。

#### 成果與唯一的例外

希拉里《論三位一體》原典溢出 **94.3% → 0.0%**；英文切片中位 6,931 字（原本每段都掛
整塊 81,984 字，十二倍收斂）。**19 本裡 18 本「整塊沒切片 0 組」、原典溢出全 0.0%，
本機與 R2 一致。**

⚠️ **唯一的例外：居普良《論述集》（`0e08c662` 的 #271–273，3 段）故意留著整塊。**
那一部的對齊器本來就對不出錨點（**既有錯配，不是這次改出來的**）：spec 的 `markers`
（`論文三`／`第四篇`／`論文六`…）確實存在，但落在 **#296–349**，而帶「居普良 論述集」
路徑的段是 **#103–137**——兩個範圍對不上，`flat` 裡根本沒有那些標記可找，所以整部不收。
而它的拉丁文**一個錨點都沒有**（無 `{{p:NNN}}`、無節號），`fathers_reslice_en --lang la`
也切不動。

🚨 **清理不該順手把內容弄丟。**第一輪把它清空了，`test/fathers-original-render.spec.ts`
的「居普良 論述集 第195章」立刻轉紅——那條測試是對的。已從 `.bak_paths` 還原。
要真修得先解決「居普良那幾部到底落在哪些段」這個路徑錯配，屬於另一件事。
...
| 奧古斯丁《論罪的功過與赦免》三卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 129/129（100%）|
| 奧古斯丁《論聖靈與字句》 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 57/57（100%）|
| 奧古斯丁《論自然與恩典》 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 64/64（100%）|
| 奧古斯丁《論伯拉糾案的審理》 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 57/57（100%）|
| 奧古斯丁《論基督的恩典與原罪》兩卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 86/87（99%）|
| 奧古斯丁《論婚姻與情慾》兩卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 79/79（100%）|
| 奧古斯丁《論靈魂及其起源》四卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 100/100（100%）|
| 奧古斯丁《駁伯拉糾派的兩封書信》四卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 100/101（99%）|
| 奧古斯丁《論恩典與自由意志》 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 37/37（100%）|
| 奧古斯丁《論責備與恩典》 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 44） | 逐節 | 30/30（100%）|
| 奧古斯丁《論聖徒的預定》兩卷 | NPNF1 Vol 5 | 拉丁‧Corpus Corporum（PL 45） | 逐節 | 78/78（100%）|
| 居普良《論述集》九部 | ANF Vol 5 | 拉丁‧Corpus Corporum（PL 4） | 逐章 | 165/167（99%）|
| 希拉里《論三位一體》十二卷 | NPNF2 Vol 9 | 拉丁‧Corpus Corporum（PL 10） | 逐節 | 373/379（98%）|
| 耶柔米《名人傳》135 章 | NPNF2 Vol 3 | 拉丁‧Corpus Corporum（PL 23） | 逐章 | 81/81（100%）|
...
Corpus Corporum 的 TEI 三種形狀都有，spec 的 `unit` 挑一種，挑錯就解析出 0 章：

| `unit` | 章在哪 | 例 |
|---|---|---|
| （預設）| `<div2>` 的 `CAPUT II.` | 迦仙、耶柔米《名人傳》、根那狄 |
| `"work"` | `<div3>` 的 `I.` `II.`（羅馬數字）| 居普良那一批 |
| `"paragraph"` | `<div3>` 的 `1.` `2.`（Migne 節號）| 奧古斯丁反伯拉糾、希拉里 |

《論行為與施捨》是第四種：章直接掛在 `<div1>`（26 個）。`parse_cc_work` 找不到帶
編號的 `div3` 就退用 `div1`，不退的話那一部解析出 0 章而只回報「命中 0」。

#### 抽查一定要抽「中間那一章」，不能只驗首末

🚨 **根那狄《名人傳》續編就是被中間那一章擋下來的。** 拉丁本在 Corpus Corporum
   （作者 998、文本 7668，100 章），中譯也切得乾淨；第 2 章（JULIUS）與第 98 章
   （POMERIUS）都逐字對得上。但**中間對不上**：中譯第 32–86 章比 Migne 多一號
   （中 49「保利努斯」＝拉 48 PAULINUS，而拉 49 是 EUTROPIUS），第 87 章起又回到
   同號。前後各自成立、中間整段錯開一位，一個偏移量修不了，所以整部不收。
   只驗首末的話這一部會全部通過，然後五十幾章配到隔壁那個人的傳記上。

#### 錨點抽哪一種，要逐部指定

同一冊裡兩種錨點都有是常態。希拉里《論三位一體》第一、五卷寫「## 第二十章」，
第二、九、十二卷寫「11. 」——`cc-book` 預設只讀章標題，那三卷就整卷命中 0，看起來
像取源壞掉。spec 加 `"anchor": "both"` 兩種都讀。

同一個 TEI 檔常被好幾部共用（迦仙《會談錄》三部共用一檔、希拉里十二卷共用一檔），
`fetch_original` 有 `_TEI_CACHE`，`--work all` 一次跑完只抓一次。
...
🚨 **Migne 的奧古斯丁有 CAPUT 與節號兩套編號，Schaff 的英譯是按節號編章的。**
   《論聖靈與字句》拉丁本 36 個 CAPUT、66 個節，而 NPNF 的「第 N 章」跑到 66。
   拿 CAPUT 對的話前 36 章配得上、逐章排得整整齊齊，第 37 章之後整批落空——而
   落空的原因在畫面上看不出來。NPNF1 第五卷因此全部走 `unit: "paragraph"`（讀
   `<div3>` 那一層），不是 `parse_cc_chapters` 的 `<div2>`。
   反例：同一卷的《論人在義中的完全》中譯是「第十八章——(39.)」，章號走 CAPUT、
   括號裡才是節號——兩套都對不上 `<div3>`，所以那一部照實留空。

🚨 **中譯的編號停在原典之前，可能安全也可能不安全。** 尾巴幾章的標題沒被解析出來
   是安全的（照實留空即可）；NPNF 把兩章併成一章則是整卷往前錯位。兩者在數字上
   長得一模一樣。所以預設一律留空，**讀過那一塊最後兩章、確認編號真的對得上**，
   才把卷次列進 spec 的 `short_ok`。這一卷有四部是這樣過關的。

🚨 **中譯在一卷中間重複一次編號時，切塊器會把一卷切成兩塊。**〈論伯拉糾案的審理〉
   的「第二十一章」寫了兩次，切出來兩塊的最大編號都小於原典節數，兩塊就都被判成
   錯開而整部留空。`blocks` 允許相鄰兩塊指到同一卷（`[1, 1]`），合起來判。
...
納徒派）、NPNF1 Vol 8（詩篇講解 149 章）。（ANF Vol 5 的居普良已用 `markers` 收了，
見下。）

🚨 **安波羅修那一冊（NPNF2 Vol 10）不只是「未分篇」，它的正文本身缺頭又亂序。**
   2026-09-02 量的：那一個前綴共 3,210 段、2,490 個節號錨點，看起來是全冊裡錨點
   最多的一冊，值得做。實際上——
   · **第一段就從《論聖職人員的職責》卷一第 31 章、第 160 節開始**。卷一第 1–30 章
     （第 1–159 節）整段不在冊子裡；全冊第一次出現「第一章」是在第 9 段（那已經是
     《論奧祕》），第一次出現「1.」是在第 6 段。
   · 節號序列往回跳 **161 次**，而這一冊該有的重編只有十九次左右（八部著作、十九卷）。
   · 第 2 段的最後兩段都以「200.」開頭而內容不同。
   要收得先把那一冊的正文補齊並重新排序，那是翻譯／分段那一層的事。**在那之前
   不要看它的錨點數字**——2,490 這個數字會讓人以為它是最好做的一冊。

🚨 **居普良那一部已收（2026-09-02，165/167），但不是靠重新分段。** 站上「居普良
   論述集」的第 83 段是一個 **140,898 字、445 個正文段的巨塊**，裡面裝了九部各自
   從第一章重編的著作。重切那一段會動到 chunk_index，而 `data/dazangjing/ancient.ts`
   有 155 條 `/fathers/{id}?page={n}` 直達連結是照 chunk_index 寫的——動了就會安靜
   地指到別的地方。所以改成**不動分段、只在對齊時把那一段按著作切開**（`markers`）。

   `markers` 是逐部著作**自己的標題**（「論文第一篇」「論文三」「論文 V.」…），
   逐一對應 `urls` 的同一順位。**章號重編的位置不能當切點**——九部的編號互相穿插，
   而且中譯本身就缺了兩部的正文。

   🚨 **錨點一定要逐部抽，不能整塊一起抽。** `both_anchors` 的去重規則（「這個號碼
      已經被別處佔用了」）在巨塊裡會把 17 個章標題全部丟掉——它們的號碼都被隔壁那
      部著作的節號佔走了，而腳本只回報命中低。

   🚨 **最後一部要收在自己那一段之內。** 不收的話它會一路吃到整部的最後：實測吃進
      了下一段的編者附註（「我們的英文版本遵循希伯來文編號…」），那幾列拿到了
      《論嫉妒與羨慕》第一、五章的拉丁文，讀起來像正文。

   🚨 **中譯少了兩部的正文而註腳還在。** 論文二《論貞女的服飾》整部只剩註腳（段
      52–92），論文十《論嫉妒與羨慕》只剩第 14–18 章（前十三章的註腳在段 412–434）。
      讀者看到的是「一堆註腳指著不存在的正文」。這是翻譯那一層的缺口，還沒補。
