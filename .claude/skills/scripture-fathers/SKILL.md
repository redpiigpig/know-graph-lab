...
| 使徒教父＋猶斯定＋偽依納爵 21 部 | ANF Vol 1 | 希臘‧First1KGreek TEI | 逐章 | 496/497（100%）|
...
⚠️ **七封真書的中譯把短、長兩種抄本排在一起，而我們只有短本。** 「依納爵致特拉勒
   人書」那幾段裡，同一章先是短本的譯文、緊接著是長本的譯文（章標題也因此走第二
   遍：6…12、1…6）。目前兩遍都配到短本的希臘文——內容相近但不是同一個抄本。要
   修的話得把 `tlg1443.tlg002` 的對應那封當第二卷餵進去，靠 `assign_books()` 分。
   命中率看不出這件事（現在是 100%）。
