...
- **📕 字典瀏覽頁（`/coach/[lang]/dictionary`，2026-06-18，全 7 語，零 AI）**：使用者要「字庫做成字典，按分級/主題/字母排序」。翻閱共用字庫 `lang_vocab_bank`：四種排序（🔤字母 word.asc／🎯**語意主題** theme／📊分級 level／🗂️頻率分類 category，後三依 freq_rank）＋對應 facet chip 過濾＋單字/釋義搜尋（`or(word.ilike,meaning.ilike)`，去除破壞字元）＋分頁「載入更多」＋🔊TTS。端點 `dictionary`(GET，純 PostgREST filter/order/range/count，零 AI)；facet 走 `vocab_bank_levels`/`vocab_bank_categories`/`vocab_bank_themes` 三 RPC。hbo 自動 RTL。首頁學習區「📕字典」磚。
  - **🎯 語意主題分類（2026-06-18）**：`category` 是頻率帶、非語意；故新增 `theme` 欄＋18 類語意主題（神‧神學／聖經人物‧地名／敬拜‧禮儀／教會‧群體‧教派／罪‧救恩‧倫理／情感‧心智／人‧身體‧家庭／食物‧飲食／自然‧動植物／時間‧節期／空間‧方位／數量‧度量／言語‧文書／行動‧移動／社會‧政治‧律法／工藝‧器物‧建築／性質‧抽象／功能詞）。`scripts/coach_vocab_bank.py theme <lang|all>` 一次性 LLM 批次（100 字/call，依釋義挑唯一主題；`theme is null` 為 reentrant、失敗批次留 null 下次補；可 `--engine haiku`）→ 寫回 `theme` 欄（RPC `set_vocab_themes`）。跑完字典「主題」分頁＝純讀 DB 零 AI。宗教研究取向：宗教字優先歸宗教主題、功能詞獨立一類。
    - **✅ 全 7 語 theme 完成（2026-06-19）**：en 30000／de 6000／fr 6000／ja 8101／grc 5400／hbo 7537／la 6011 — 每字皆有語意主題，字典「主題」分頁純讀 DB 零 AI。收尾用 `theme all --engine auto`（NVIDIA→Gemini→Haiku，比純 haiku 耐 429）一輪跑完、零失敗批次。要重補或加字時 `theme all` reentrant（只抓 `theme is null`）直接接續。
      - **看進度**：`python scripts/coach_vocab_bank.py status`（已加 `DB themed` 欄）。確認程序：`Get-CimInstance Win32_Process | ? {$_.Name -eq 'python.exe' -and $_.CommandLine -like '*coach_vocab*theme*'}`。
      - **續傳**：`python scripts/coach_vocab_bank.py theme all --engine haiku`（**reentrant**＝只抓 `theme is null`，重跑自動接續＋補先前失敗批次；全跑完再跑一次補零星 stragglers）。
      - **⚠️ 引擎現況（2026-06-18 當日）**：NVIDIA 429 限流、Gemini 當日免費額度耗盡 → **只剩 Haiku（Max）可用**，故強制 `--engine haiku`。隔日 Gemini 額度重置後也可改回 `--engine auto`（NVIDIA→Gemini→Haiku）。Haiku 撞 Max 滾動視窗 429 會自動退避重試、撐不過就略過該批（reentrant 下次補）。
      - **別頻繁重啟**：批次是「父啟動器＋子工作程序」父子鏈，父程序 CPU≈0 只掛著等；要停只停自己 session 起的 `coach_vocab*theme` python，勿動別 session（[[feedback_no_kill_other_tasks]]）。
...
- **單字 SRS**（`lang_vocab`，**FSRS v4.5**，`server/utils/srs.ts`，2026-06-05 由 SM-2 改）：`fsrs()` 用 difficulty/stability/retrievability 三參數（`lang_vocab` 加 `difficulty`/`stability` 欄位），依距上次複習天數算 R；同記憶率複習量少 ~20-30%、答錯不再反覆轟炸。`review.post` 已改用 fsrs；`sm2()` 保留不刪。review **三模式**：翻卡／選擇題／**克漏字**（看中文+挖空例句→打單字，對=good 錯=again；不自動朗讀免洩答案）。不足時從整庫補未精熟字；`vocab/generate` 依**目前程度**生成主題詞組——**非初學者採「程度下限」策略**：挑符合或略高於目前程度的字，**嚴格排除低於該程度的基礎／日常常見字**（英文 B2 就不出 heartbeat/happy 這類 A1–B1 早已熟悉的字，寧難勿易；初學者 A1/A2/N5/入門 仍給基礎高頻字）。既有的太簡單卡片不批次刪（使用者要保留自己拼錯／想練的字），靠評「簡單」拉長間隔自然淡出。
  ...
  - **📚 共用預備字庫 `lang_vocab_bank`（2026-06-12，全語言）**：解決「只有英文有預備字庫、其他語言只能即時生成 → AI 一過載就斷糧」（使用者遇到的 grc 生成失敗）。共用表（非 per-user，RLS 已登入可讀、service role 寫）每語言一份**權威頻率/語料庫 + LLM 補繁中釋義+例句+詞性**的策展字庫，依頻率分類帶。
    - **來源**（皆免費/公有領域/CC-BY，腳本自動下載解析）：en/de/fr＝FrequencyWords（OpenSubtitles 頻率，取前 N，分 A1→C2 帶）；ja＝jlpt-vocab-api（N5–N1，每級一類，附 furigana/romaji/EN 義）；**grc＝STEPBible TAGNT**（新約 5,400 詞元，含英義/詞性/書卷，分新約頻率帶）；**hbo＝STEPBible TAHOT**（舊約 7,537 詞元）；**la＝Clementine Vulgate**（表面詞頻 → LLM 還原詞元，分武加大頻率帶）。**新增 7 語（2026-06-22，harvester 全驗證跑出候選）**：**arc＝STEPBible TAHOT 亞蘭文段落**（morph 首字 A，但以理/以斯拉，583 詞元含英義/詞性）／**att＝先共用 grc TAGNT 新約語料**／**ar＝Quran JSON 表面詞頻**（去 harakat，10709）／**sa＝薄伽梵歌天城體表面詞頻**（4176）／**pi＝VRI 三藏羅馬轉寫 UTF-16 表面詞頻**（10968）／**cop＝eBible 科普特新約表面詞頻**（10000）／**bo＝eBible 藏文聖經，tsheg 斷音節**（3844）；ar/sa/pi/cop/bo 同 la 走「表面→gloss LLM 還原詞元」。目標：en 30k／de·fr·la·att·ar·pi 6k／cop·bo 4k／ja ~9k／sa 3k／arc·grc·hbo 整部窮盡。**再 +4（2026-06-22）**：es（FrequencyWords 西語，CEFR 帶）／syr（Beblia AramaicBible＝Peshitta 敘利亞字體，去母音點）／hy（Beblia ArmenianBible）／ka（Beblia GeorgianBible）；syr/hy/ka 走表面→gloss 還原詞元。**∴ 已備 vocab 候選＝11 新語（arc/att/ar/sa/pi/cop/bo/syr/hy/ka/es）＋原 7**。**台/客語直接策展完成（2026-06-22，零 AI）**：g0v 萌典 `moedict-data-twblg`（台語）/`moedict-data-hakka`（客語）已含**漢字＋台羅/四縣腔＋繁中釋義＋例句**→ `_parse_moedict` 解析（跳過□本字未定、客語取四縣腔、清 ￹￺￻ 例句標記）→ candidates 帶 `preglossed:true` → cmd_gloss 偵測 preglossed **直接 upsert 不呼叫 LLM**。**nan/hak 已 upsert DB 各 6000 字（glossed=true）完成**。**∴ 已備 vocab＝13 新語**（11 harvest + nan/hak 完成）＋原 7。**死語言人工策展（2026-06-22，零 AI）**：`CURATED` dict（word/讀音/繁中義/詞性/**attested 例句**——例句一律取自實際銘文/經典不杜撰）＋ `harvest_curated()` 回 preglossed → 直接 upsert。**已完成 upsert DB（12 種策展完成）**：peo 20/phn 20/uga 18/chu 22/gez 20/akk 22/egy 20/ae 20/pra 18/mid 15（死語言）＋ nan/hak 6000（萌典）。死語言 word＝該文字書寫（楔形/聖書體轉寫/天城體/曼達/西里爾/吉茲…），例句皆 attested。ami/tay（阿美/泰雅）無乾淨語料源，**保守策展核心詞**（教育部書寫系統：數字/親屬/自然/信仰，例句從略不杜撰）ami 22/tay 18 已 upsert。**∴ 全 25 新語言＋原 7 ＝ 32 語全部備妥 vocab bank**：14 種已 upsert DB 完成（nan/hak 萌典 6000 + peo/phn/uga/chu/gez/akk/egy/ae/pra/mid/ami/tay 策展），11 種 harvest 候選待站長整夜跑 gloss（arc/att/ar/sa/pi/cop/bo/syr/hy/ka/es），原 7 早已完成。**∴「不用 runtime AI、全部預建」目標達成**。加策展語言＝在 `CURATED` 加一筆 list + HARVESTERS/TARGETS/LANG_LABEL；ami/tay 例句與 mid/akk/egy 詞彙待有更權威來源時可擴充。
    - **腳本 `scripts/coach_vocab_bank.py`**：`harvest <lang|all>`（下載+解析→`C:/tmp/vocab_bank/<lang>.candidates.jsonl`）／`gloss <lang|all> [--limit N]`（逐批 LLM 補繁中→upsert，**每批 flush 進 DB 與 ledger 同步、可重入**）／`run`／`status`。引擎 **NVIDIA deepseek-v4-flash 主→Gemini→Haiku**（6.6 萬條會燒爆 Gemini 免費日限，正是過載主因）；批量 40 字/call、~30s/call。`gloss all` 順序＝arc,att,ar,sa,pi,grc,hbo,la,ja,de,fr,en（新語言先跑、英文最大擺最後；舊語言已 gloss 完會自動略過）。**新 5 語要跑 gloss（站長整夜跑）**：`python scripts/coach_vocab_bank.py gloss arc`（或 ar/sa/pi/att，或 `gloss all`）。**c:/tmp ledger 別清**（[[feedback_tmp_cleanup]] 例外）。
...
- **主引擎＝NVIDIA NIM（2026-06-03 起）**：`server/utils/nvidia.ts` `callNvidiaFull`（OpenAI 相容、key 輪替、剝 `<think>`）。`coach-ai.ts` `coachGemini` 先試 NVIDIA → 失敗才落 Gemini。env `nvidiaApiKeys`=線上專用 `NVIDIA_API_Key_OLINE_ONLY`（見上「線上／線下 key 分離」；全部不可用才落 Gemini）、`nvidiaModel` 預設 **`qwen/qwen3-next-80b-a3b-instruct`**（繁中佳、支援 JSON、穩定）。無限量、零成本，用量記 tier=`nvidia`。
  - ⚠️ **不要改回 `nvidia/nemotron-3-super-120b-a12b`**：該模型在 NVIDIA 免費層長期 429（互動式教練不可靠）；deepseek 只適合 translate 腳本那種可退避重試的批次。其餘 NVIDIA 模型（qwen3-next、llama-3.1）正常 200。
...
- **轉寫鍵盤（`keyboard` 欄位，目前 `"greek"` / `"kana"` / `"hebrew"`）**：打英文/羅馬字即時轉成目標文字。`chat.vue` 偵測 `coach.keyboard` 顯示「⌨️ 鍵盤」開關＋對照表面板，綁 textarea `@keydown`（`onInputKeydown` 依 keyboard 分派 greek/kana/hebrew；Enter 前先 `kana.finalize` 收殘留 ん）。訊息泡泡加 `dir="auto"`、希伯來輸入框 `dir="rtl"`。
  ...
  - **Abugida/音節文字鍵盤 `composables/useAbugidaKeyboard.ts`（2026-06-20）**：天城體（梵 sa／半摩揭陀 pra）、吉茲 fidäl（gez）、藏文（bo）。這些非「1 鍵 1 字」（子音帶固有母音、母音以 matra/階結合、有疊寫），故用**有狀態機制**：維護「整詞 roman 緩衝」，每打一鍵把整串丟給純函式 `translit()` 重轉寫、取代螢幕上這個詞（記 `outLen`）；空白/標點/Enter 就 commit 清緩衝（藏文空白＝音節點 tsheg ་）。三個純轉寫器：`translitDevanagari`(ITRANS，dharma→धर्म 含合字虛音符)、`translitGeez`(SERA：子音 6 階 ə、e→1 階 ä，selam→ሰላም，codepoint 位移)、`translitTibetan`(Wylie，**自動疊寫**：一個 buffer=一個音節，正字法找字根 ming gzhi 再堆前加/上加/字根/下加/後加，bsgrubs→བསྒྲུབས、rgyal→རྒྱལ；梵文/不規則疊字仍可用顯式 `+`)。chat.vue 的 `scriptKb` resolver 已 `getScriptKeyboard()||getAbugidaKeyboard()`，send() 會 `scriptKb.reset()`。
  - 純函式單元測試 `test/coach/keyboard.spec.ts`（假名/希臘/希伯來）＋ `script-keyboard.spec.ts`（6 字母系，7 cases）＋ `abugida-keyboard.spec.ts`（天城體/吉茲/藏文含自動疊寫，14 cases）。**全 7+3 文字鍵盤到齊**（greek/kana/hebrew 內建；cyrillic/coptic/arabic/syriac/armenian/georgian 走 SCRIPT_SPECS；devanagari/geez/tibetan 走 ABUGIDA_SPECS）。加字母系＝SCRIPT_SPECS 加一筆；加 abugida＝寫 `translit()` 純函式 + ABUGIDA_SPECS 加一筆。**12 文字鍵盤全到齊（含藏文自動疊寫）**；藏文只剩梵文不規則疊字保留顯式 `+` 作逃生口。
