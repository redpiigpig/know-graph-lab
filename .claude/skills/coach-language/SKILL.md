...
- **💬 情境實用句（英文，2026-06-05）**：`server/data/enSentences.ts` 6 情境（機場/學校辦事/學術場合/稅務局/醫院/日常辦事）×12 = 72 策展實用句（情境繁中／範例英文／中譯／用法提示）。頁 `/coach/en/sentences`：看情境→**先試答**（口說 STT 或寫作）→**看建議解答**（TTS＋中譯＋提示）→🎙️複述比對（粗略相似度）／✨換類似句／下一句；練到底自動用 AI（`sentences/more`，NVIDIA 主）補更多 → 寫作口說無限練。端點 `sentences`(index GET 句庫 / more POST AI 延伸)。要加情境/句子改 `enSentences.ts`。首頁入口（限 en），與文法課/情境角色交叉連結。
