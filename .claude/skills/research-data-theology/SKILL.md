...
### 🚨 2026-09-12 抽樣複核：精確率從 35% 到 100%

每區隨機抽 20 筆（母體不足 20 的全抽）逐筆判讀，數字寫進 `PRECISION` 並印在頁面上：

| 100% | 95–90% | 80–70% | 65% | 35% |
|---|---|---|---|---|
| 神學史、性別、解放、世俗 | 系統神學 95、方法論 90 | 自由 80、全球 75、實踐 70 | 聖經神學 | **各地的神學、敘事神學** |

**這一項的成果不是「清單變乾淨」，而是「讀者知道這份清單有多不乾淨」。**
所以假命中一律**不刪**——刪掉會一併犧牲召回率，而且從結果上完全看不出來漏了什麼。
作法是把低精確率的詞列進 `WEAK`，只有「命中的詞全是弱詞」的篇目才標 `weak: true`，
頁面上另列一區並顯示它命中了哪個詞。

弱詞與它們為什麼弱（全部實測，不是猜的）：

* 🚨 **`聖經神學`** 命中長老教會「大專**聖經神學研究班**」的活動報導——20 筆抽樣裡
  有 7 筆是這個。這是最典型的一種假命中：詞完全正確，指的東西完全不同。
* 🚨 **`原住民`**、`族群`（各地的神學）——大量命中原住民社會議題的評論與報導；
  `族群` 還會命中「熟齡族群」「族群融合」。本區 137 筆裡有 105 筆只靠弱詞命中。
* 🚨 **`敘事`**（敘事神學）——聖經敘事批判、敘事治療、文學敘事研究、身分敘事全部命中，
  與敘事神學是四回事。55 筆裡 44 筆。
* `禮儀`（幾乎全是「中國禮儀之爭」，那是宣教史）、`崇拜`（「祖靈崇拜」「偶像崇拜」）、
  `宗教教育`（政教關係與法制）——實踐神學。
* `全球化`（「全球化與台灣」「全球化傳染病」）、`神論`（景教文獻《一神論》）、
  `現代主義`（會命中「後現代主義」）。

⚠️ 另有一個**不是靠標弱詞能解決**的問題：性別神學那一區精確率 100%，但相當比例是
教會刊物的短文與社論而不是學術論文。「切題」與「學術性」是兩個軸，現在只量了前者。
...
**2026-09-12 首次全跑完畢：822 種刊、452,459 篇，對帳零警告、零失敗刊。**
其中 739 種不在 DOAJ 那 313 種裡。

### 🚨🚨 `language` 欄不可信，這是本支最危險的一個坑

算出來的數字整齊、不報錯、看起來完全正常。實測：

| 刊 | 實際語言 | Crossref 標的 |
|---|---|---|
| 《Praktische Theologie》 | 德 | 3,489 篇中 **3,145 篇標成 en** |
| 《Zeitschrift für Pädagogik und Theologie》 | 德 | 606 篇**全部標成 en** |
| 《Archiv für katholisches Kirchenrecht》 | 德 | 8,828 篇**全部未標** |
| 《Zeitschrift für Theologie und Kirche》 | 德 | 500 篇中 495 篇正確標 de |

language 由出版社自行登記，De Gruyter 一系大量把德文論文登記為 en。全庫加總得出
「德 2,667／未標 154,499」——而真正的德語篇數至少是表上 de 的五倍以上。
**要判語言請從刊物層下手**（刊名、出版社、ISSN 所屬國）。索引裡另存
`nonenglish_journals`（按刊名非 ASCII 判的下界，81 種／35,939 篇），並附
`languages_warning` 字串，頁面必須連警告一起顯示。

⚠️ 另一件過濾不掉的事：**書評**。老牌期刊的書評出版社多半就登記成
`journal-article`，所以 Church History 那 17,314 篇裡有相當比例其實是書評
（題名長得像「某書．某人譯註．某地：某社，1989. vi + 345 pp.」）。
不要寫個 "pp." 正則去砍——會連正文論文一起砍。要處理就在呈現層標示。
