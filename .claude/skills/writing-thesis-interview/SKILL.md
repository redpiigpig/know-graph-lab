...
| 範本 | 適用 | 特色 |
|---|---|---|
| [`01.16 釋印悅法師口述訪談紀錄.txt`](../../../public/content/interviews/01.16%20釋印悅法師口述訪談紀錄.txt) | 法師 (面訪) | 前言兩節 + 四個主題節 + 附件訪談大綱 |
| [`04.10 邱敏捷教授口述訪談紀錄.txt`](../../../public/content/interviews/04.10%20邱敏捷教授口述訪談紀錄.txt) | 學者 (面訪) | 前言三節 (含「概念簡史」)、Q&A 翔實 |
| [`05.11 釋見岸法師口述訪談紀錄.txt`](../../../public/content/interviews/05.11%20釋見岸法師口述訪談紀錄.txt) | 法師 (電話訪談) | 前言三節 (含「相關文獻摘選」) |
...
（一）訪問動機
[一段：為什麼訪問這個人 — 他與研究主題的具體關係、能提供什麼別人提供不了的視角。
**不要**重複「筆者碩士論文研究的主題是『印順導師人間佛教思想的傳承與實踐...』」這個論文題目鋪陳——論文自己的前言已經講過一次了，附錄訪談稿每篇再貼一遍是冗餘。]

（二）受訪者簡介
[一段：生年、籍貫、學歷、職涯、與本研究主題的關係。對法師：依止師父、剃度年、得戒、現任職務。對學者：學經歷、代表著作。
如果這場訪談有獨特背景（透過誰引介、現場特殊事件、受訪者帶來什麼資料、訪談地點的意義），就在簡介末尾或訪問動機中順帶一句帶出；**不要**獨立成「（三）訪問情形」章節貼公式（簽名同意錄音、訪後合照這種）。]

(可選 三、相關文獻摘選 — 若受訪者有重要著作可直接引用)
...
附件、[受訪者]訪談大綱（可選 — 把訪綱 docx 的訪綱題目附在最後；不重貼主旨段＋受訪者簡介段）

筆者與[受訪者]的訪談合照（合照行直接放，不要冠「附件二、」）

【拍攝時間：YYYY年M月D日，地點：[地點]】
```
...
每份訪談紀錄完成後，逐項對照（2026-05 全套訪談稿審稿結果歸納）：

### 結構（必須）

- [ ] **第 1 行**：`[訪談標題]：[受訪者]訪談記` — 開頭無 BOM、無 docx 自動編號「`1.  `」前綴
- [ ] **第 2-4 行**：`受訪者：` / `訪問時間：` / `訪問地點：` — 三行緊鄰，**彼此之間不空行**（舊檔常有 docx 殘留空行，新檔不要）
- [ ] **第 5 行起**空一行進入 `一、前言`
- [ ] **前言兩節必備**：`（一）訪問動機`、`（二）受訪者簡介` —（三）相關文獻摘選 可選
- [ ] **訪問動機不貼論文題目鋪陳**：直接寫「為什麼訪問此人」，不要每篇都重貼「筆者碩士論文研究的主題是『印順導師人間佛教思想的傳承與實踐...』」開頭段（論文前言講過一次就夠，附錄訪談稿不需重複 32 次）
- [ ] **不獨立「訪問情形」節**：不貼「簽名同意錄音、訪後合照、給訪綱參考」這套公式；訪談獨特背景（引介人、現場事件、受訪者帶來的資料）融進訪問動機或簡介末尾一句話帶出
- [ ] **主節編號**：`二、`、`三、`、`四、`、`五、`…（漢字 + 頓號，不是阿拉伯數字）
- [ ] **附件**（可選）放最後：`附件、[受訪者]訪談大綱` —— 用「附件」不要編號；訪後合照行直接放、不冠「附件二、」前綴
- [ ] **附件內不重貼主旨／受訪者簡介**：那些是主體前言的副本，附件只要訪綱題目（編號 1.2.3.）即可

### Q&A 密度（必須）

- [ ] **每個主節 2-5 個獨立 `筆者：` 問句** — 不是一節壓成一個 mega-question，也不是把 Gemini 的碎片化「嗯／是」逐字保留
- [ ] **每個回答 1-3 段** — 段落間**不重貼說話者標籤**
- [ ] **「嗯」「是」「對」等確認直接刪除**，不獨立成 turn
- [ ] **總字數參考**：60-90 分鐘訪談整理後約 **25K-50K 字**；輸出 < 15K 字而音檔 > 60 分鐘，多半過度精簡需補回細節／典故／人名／年份
...
- [ ] **受訪者簡稱**：
  ...
  - 牧師 → `[姓]牧師：`、主教 → `[姓]主教：`
  - 居士／老師／女士／先生 → 同稱謂
- [ ] **「受訪者：」這一行（第 2 行）用完整稱呼**（釋 XX 法師／XX 教授），與 Q&A 簡稱不同

### 內容品質（必須）

- [ ] **簡介寫過的 bio（出生年、籍貫、學歷）在 Q&A 不重問**，直接從實質研究內容切入
- [ ] **佛教領域人名、寺院、術語對照 Step 4 修正表校驗**（弘誓、印順、昭慧、性廣、嵐園、栴檀精舍等容易聽錯）
- [ ] **訪綱對應「資料夾日期 vs 真實訪問日」對不上的（何宗勳、艾琳達、張莉筠等），以 docx／訪綱內訪問日為準**

### 常見套式陷阱（2026-05 全套審稿歸納）

寫稿時若出現以下模式，停下重想：

| 陷阱 | 為什麼差 | 改進方向 |
|---|---|---|
| **標題**：「人間菩薩道」「印順思想的傳承與實踐」「弘誓的 XX」「XX 的修學歷程」 | 多篇撞詞，看不出該人獨特角度 | 用該人最具差異性的職務、學派、事件做題眼。例：何日生「慈濟學的建構與印順思想的對話」、長叡「華雨精舍住持與北美弘法」、宏印「《妙雲集》的青年弘法者」、圓貌「弘誓第二任院長的學院經營」 |
| **訪問動機開頭**「XX 法師長年跟隨昭慧法師與性廣法師修行...」 | 5+ 篇法師訪談都這樣開頭，看不出此人為何被訪 | 直接點出該人在研究脈絡中的關鍵位置：碩士論文題目、特殊職務、獨特身分（例：「現任弘誓僧團住持＋財務主任」、「第二任院長」、「曾完成《現代僧團規制建構》碩論」） |
| **主節小標題**「XX 法師的修學歷程」「教授的學思歷程」「兩位法師的領導風格」 | 14+ 篇都這樣命名，撞詞 | 用該節實際內容做差異化：「從花藝教師到弘誓出家」「從太空科學到中觀研究的轉折」「菩薩型與禪師型的對比」「住持與指導法師的雙頭分工」 |
| **結尾**「筆者：感謝法師接受訪問。XX法師：好。」 | 客套無實質，浪費附錄空間 | 直接刪掉這組 Q&A，讓前一個實質 Q&A 作為自然收束。若有續訪（如釋昭慧上→下），用一行筆者註解交代「另約後續，即〈XX下〉」 |
| **碎句口語**：「然後／所以／那」連發、「就是說／對啊／嗯」散見、同一論述重複 2-3 次 | 顯露未整理 | 把「然後／所以」減半、合併重複論述、刪「嗯/是/對」獨立 turn |
| **檔案內含他人訪談**（複合檔） | 早期把多人 docx 合併存一檔，網站只顯示首篇，其餘是死碼 | 每人一檔；新整理時不要 append 別人的訪談到同一檔內 |

### 檔案／路徑（必須）

- [ ] 路徑：`public/content/interviews/MM.DD [受訪者]口述訪談紀錄.txt`
- [ ] UTF-8 編碼、LF 行尾
- [ ] `stores/thesisInterviews.ts` 的 `published` 已加入；`driveMissing` 已移除；依日期排序；`filename` **不**含 `.txt`、**不**事先 URL-encode
...
Drive 上每場訪談一個資料夾：
```
G:\我的雲端硬碟\資料\知識圖工作室\研究資料\大愛道革命\口述訪談\YYYY.MM.DD [受訪者]訪談\
...
5. **寫前言兩節**（必須）：
   - **（一）訪問動機**：一段，直接寫「為什麼訪問此人」 — 他與昭慧／性廣的關係、能提供什麼別人提供不了的視角、在研究脈絡中的位置。**不要**用「筆者碩士論文研究的主題是『印順導師人間佛教思想的傳承與實踐...』」這個論文題目鋪陳起頭——論文前言已經講過一次了。
   ...
   - 訪談獨特背景（誰引介、現場特殊事件、受訪者帶來什麼資料、訪談地點的意義）— 融進訪問動機或簡介末尾一句帶出；**不要**獨立成「（三）訪問情形」節貼公式（「簽名同意錄音、訪後合照、給訪綱參考」這種）。
6. **訪談標題**：7-12 字濃縮這場訪談的主軸，後面接「：[受訪者]訪談記」。**避免**「人間菩薩道」「印順思想的傳承」「修學歷程」「兩位法師」這類套式（多篇撞詞），用該人最獨特的職務／作品／事件做題眼：
   - 「弘誓僧團的住持與財務：釋心皓法師訪談記」（強調職務）
   - 「論印順學派的傳承：邱敏捷教授訪談記」（強調學派研究）
   - 「在愛與公義中的信仰對話：盧俊義牧師訪談記」（強調跨宗教對話）
   - 「《妙雲集》的青年弘法者：釋宏印法師訪談記」（強調歷史角色）
   - 「弘誓第二任院長的學院經營：釋圓貌法師訪談記」（強調職務）
   - 「華雨精舍住持與北美弘法：釋長叡法師訪談記」（強調職務＋地理）
...
## 批次清理腳本（2026-05 全套審稿用）

清理舊檔（38 篇）時寫了 3 個一次性腳本放在 `c:\tmp\`，未來若要做類似批量處理可以參考：

| 腳本 | 用途 |
|---|---|
| `c:\tmp\strip_interview_format.py` | 刪論文題目鋪陳段（以「筆者...印順導師人間佛教思想的傳承與實踐」開頭的段落）＋ 刪「（X）訪問情形」公式段（簽名同意錄音、訪後合照那種）。處理 35 篇用 |
| `c:\tmp\strip_docx_prefix.py` | 清掉檔頭 BOM 與「1.  」/「25. 」docx 自動編號殘留 |
| `c:\tmp\truncate_duplicates.py` | 截斷複合檔（一個檔案內嵌多人訪談）；用 hardcoded `CUT_POINTS` 指定每檔切點 |
| `c:\tmp\strip_outline_duplicates.py` | 清附件內重複的「主旨」「受訪者簡介」段，保留訪綱題目 |
| `c:\tmp\rename_appendix.py` | 「附件一」→「附件」；「附件二、合照」前綴拿掉 |

整體 4 個 commit 累積刪 1,424 行（513e0b3 + 7b7f3d1 + f7eaf50 + 46d931f）。
