...
- Seed：[scripts/seed_glossary_offices.py](../../../scripts/seed_glossary_offices.py)（hand-curated **204 筆**、`--dry` 預覽＋自檢、`--replace` 全量重灌、`on_conflict=name_english,polity`）
...
**狀態（2026-07-01）**：✅ 表已建、Vue tab 已接、seed **204 筆**上線 —— **每個帝國撐開成完整權力金字塔**（中央≥部長級→〔超一級〕→一級→二級〔→三級〕），每政權皆備 中央＋一級＋二級。巴比倫定楚職**縣公**＋兩層省制（大尹→縣公）。⏳ 各帝國中段【提】細項使用者可於 UI 逐項校訂；文體翻譯（台語文讀／粵語）屬「漢譯世界史計畫」更大願景，本表只做官職／行政區定名。
