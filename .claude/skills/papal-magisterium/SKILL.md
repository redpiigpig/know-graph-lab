...
### 跟 [[scripture-canon-portal]] / [[fathers]] 分工
...
- [[scripture-canon-portal]] — `/creeds` pipeline 元件（`paragraphParser.ts` / `alignDocs()` / `textLoader.ts`）直接複用；Denzinger 中譯 ebook 含部分中譯 DH 對位
- [[fathers-translation]] — 4-7c 教宗 = 教父，雙邊都收（見 §0 分工）
