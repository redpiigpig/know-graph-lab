...
### 8.2 研究自動化補全流程（目錄驅動；交接快照 2026-06-19）

> 目標：每代 ≥1000 卷、每藏**正藏** ≥100（經藏除外）。靠「按正典子單位展開合集」＋持續研究補全，不灌水。cadence＝**一次一目標穩扎打**（user 定）。

**📊 進度快照（2026-06-19 深夜）**：前 320／古 854／中 270／近 207／現 **279**＝**約 1,930 卷**。
- 最新：**modern/lun/zheng（現代論藏正藏）64→114 達標**（兩批：batch1 因 529 提前收 34 卷，fresh 補跑 batch2 +50。新增「東正教神學部」洛斯基/弗洛羅夫斯基/布爾加科夫/施梅曼…14 卷；自由存在補過程神學+新詮釋學；處境解放補亞非拉；天主教補呂巴克/孔加爾/史希爾貝克斯/龔漢斯）。
- 前一輪：medieval/lun/zheng（中世紀論藏正藏）24→101 達標（新增「敘利亞與東方教會神學部」巴爾‧赫布雷烏斯 2 卷）。先前本輪研究入庫約 354 卷：首批 71（中律外聖訓 kitab／古論外斐洛＋交鋒外教／近代論清教／前經 ANE／現代譯校原典集成／現代類書神學辭書）＋前/論 101（前蘇格拉底 35＋柏拉圖亞里斯多德新柏拉圖 66）＋前/詩藝 48（荷馬/品達/悲劇）＋前/律 24（ANE 法典）＋前/史 38（希羅多德/修昔底德/李維/兩河編年）＋前/儀 72（密特拉禮儀/PGM/祕儀）。**前藏十藏已大致補滿**。

**🔑 方法＝目錄驅動枚舉（2026-06-19 起，user 指示）**：不憑記憶舉例，研究 agent **系統走查機構書目目錄**——美國國會圖書館 catalog.loc.gov（LCC：BR 教會史／BS 聖經／BT 教義／BV 實踐神學‧禮儀‧講道‧宣教／BX 各宗派）、梵蒂岡圖書館 opac.vatlib.it＋digi.vatlib.it、WorldCat、HathiTrust、Internet Archive，配指定全集卷目（Migne PG/PL、Schaff ANF/NPNF、Corpus Christianorum、CCEL…）逐目枚舉（每源 15–30＋）。已寫進 workflow prompt。

- **規格**＝`data/dazangjing/collection-goals.ts`（`COLLECTION_GOALS` 十藏 scope＋sources、`VOLUME_TARGETS`、`COMPILATION_RULES`〔含合集拆分〕、`CLASSIFICATION_NOTES`、`RESEARCH_QUEUE`）。
- **workflow**＝`.claude/workflows/dazangjing-research.js`（Skill `dazangjing-research`）。args＝`{era,eraName,boundary,collection,collectionName,canon('zheng'|'wai'),canonLabel,goal:{zhengScope|waiScope,sources},existingTitles}`。階段：**Research**(目錄枚舉，多源並行)→**Consolidate**(合併同書異譯名近似重複)→**Verify**(對抗式查核 confidence≥0.6)。回傳 `result.proposed[]`（含 parent/extent/source_citation）。
- **跑一輪 SOP**：① 選 era×藏(優先補薄的代/正藏<100)；② `python scripts/dazangjing_titles.py <era>` 取現有書名，挑該藏相關者當 existingTitles；③ `Workflow({name:'dazangjing-research', args})`（**fresh，args 必傳**）；④ 完成後讀 `…/tasks/<id>.output` 的 `result.proposed`；⑤ **inline python 合併**：對「該代全部 title_zh」去重→依 `division` 欄正規化分部(agent 的 division 常不一致/帶全路徑，要 canon 映射)→組 TS division 物件→插在目標 collection 對應 canon 的 `divisions: [` 之後(brace 定位)；⑥ `npm run test:run -- test/dazangjing.spec.ts` 必綠→commit+push。
...
8. **529 Overloaded ≠ 額度**：是伺服器暫時過載（subagent 秒死、0 tokens）。退避 ~90s 後 fresh 重跑即可，別誤判為撞額度而長等。

**待續**：~~medieval/lun/zheng~~✅（101）；~~modern/lun/zheng~~✅（114）；續推各基督教代×藏朝 100/正藏（現最薄＝**近 207／中 270／現 279**——但這是全藏總數，各代仍有個別正藏<100 的藏待補）。下一目標建議：近代(early-modern) 論藏正藏，或各代史傳/宣道/禮儀等正藏<100 者。提案永不自動 merge，一律人工過目後入庫。
