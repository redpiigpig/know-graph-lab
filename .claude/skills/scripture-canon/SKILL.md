...
**📊 進度快照（2026-06-22）**：前 320／古 **907**／中 270／近 **380**／現 **285**＝**約 2,152 卷**。
- **論藏正藏各代已達標**：古 171／中 101／近 98／現 118。✅
- **史傳藏進度**：**近代史傳藏正 71→101 ✅**；**古代史傳藏正 47→100 達標 ✅**（兩輪：r1 +26 西方/東方核心；r2 +27 多元）。各代史傳：古 **100**／中 **20（研究已備、未入庫）**／現 13。
  - 🚧 **中代史傳藏正 20→100 研究完成、待 verify+入庫（2026-06-23 交接）**：跑 `dazangjing-research`（era=medieval, shizhuan, zheng, 14 個多元角度）→ **313 候選 → 287 fresh（對 270 中代既有去重）→ consolidate 147 → verify 只過 25**（verify 階段撞 session 額度 resets 5pm Asia/Taipei，~122 verify agent 全 fail）。**workflow 只研究不入庫**，故中代史傳仍 20。
    - 📦 **salvage 已存**：`C:/tmp/medieval_shizhuan_salvage.json`（`verified_pass_25` 已過查核／`merged_147` 全去重候選池／`pending_unverified` 122 待查核）＋腳本 `C:/tmp/salvage_med_shizhuan.py`、`C:/tmp/fix_proposed.py`。**下一 session：對 122 pending 跑 `dazangjing-verify-only`（embed candidates, model 留空走 Opus）→ 連同 25 已過者重度 curation（title_orig 全 corpus 去重＋division 映射）→ insert（trailing-comma-safe）→ test→push。**
...
**待續**：論藏各代✅。史傳藏：~~近代 22→71~~✅ ~~近代 71→101~~✅ ~~古代 47→100~~✅（2026-06-22）。**🚧 中代史傳 20→100：研究＋consolidate 完成（147 候選），verify 撞額度只過 25，salvage 在 `C:/tmp/medieval_shizhuan_salvage.json`——下一 session 接續＝對 122 pending 跑 `dazangjing-verify-only`→curation→insert→test→push（詳見上方 🚧 條）。** 之後：**現代史傳 13→100**（多元：亞非拉美/原住民歸信/女宣教士女神學家/獨立教會/五旬節）。再輪補其他薄藏正藏<100（律/譯校/書函/禮儀/詩藝/類書/宣道）。提案永不自動 merge，一律人工過目後入庫。
