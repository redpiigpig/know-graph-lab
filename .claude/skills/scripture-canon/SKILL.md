...
**📊 進度快照（2026-06-22）**：前 320／古 **880**／中 270／近 **380**／現 **285**＝**約 2,125 卷**。
- **論藏正藏各代已達標**：古 171／中 101／近 98／現 118。✅
- **史傳藏進度**：**近代史傳藏正 71→101 達標 ✅**；**古代史傳藏正 47→73**（進行中，目標 100）。各代史傳：古 **73**／中 20／現 13。
  - 近代史傳五部：宗教改革史 19／殉道 18／宣教 18／域外民族誌 11／教會通史 35。
  - 古代史傳七部（+26 後）：通史 +9（含晚期教會史家 Evagrius/Gelasius/Theodore Lector、三部教會史、亞美尼亞/敘利亞 Eghishe/Lazar/Sebeos/John of Ephesus/Zachariah）／編年 +7（Orosius/Prosper/Hydatius/Chronicon Paschale/Malalas/Edessa/Sulpicius）／聖徒傳 +9（Lausiac/Hist.Monachorum/Theodoret Religiosa/John Moschus 靈苑/Cyril Scythopolis/Melania/Symeon Elder+Younger/東方聖人傳）／宗統 +1（Liber Pontificalis Duchesne）。
  - 💡 教訓：① sweep 回的「magnum opus」常已在既有庫內（近代 Strype/Knox/Heylyn/Fuller；古代 Eusebius/Socrates/Sozomen/Theodoret/Rufinus/Bede/沙漠父老/安東尼傳/居普良行傳/Perpetua/Polycarp/里昂殉道——**title_orig 前綴去重＋人工查既有 zh 異譯是關鍵**）。② **古代既有庫已吸收絕大多數名著，乾淨新書池淺**——一輪 sweep 僅得 ~26 淨新（多為東方傳統與冷門編年/聖傳），補到 100 需多輪不同角度。③ Eastern（敘利亞/亞美尼亞/科普特，含 Miaphysite）比照既有 敘利亞教會史/亞美尼亞史 判**正**。④ verify 會亂建 division（church-history/universal-history/church-biography 非合法部名，須正規化回 7 部）＋偶誤判正統為外。
  - ⚠️ **insert 踩雷**：`ancient.ts` 既有各 division 末筆**有** trailing comma，`early-modern.ts` **沒有**——append 前綴逗號前須判 `if last==','` 否則生 `},,` sparse-array 洞（4 個洞令 spec 4 fail）。insert_generic.py 已修。
  - 🔧 workflow：`dazangjing-verify-only`（args.candidates→純 verify ≥0.6）＋ salvage 腳本（c:/tmp：salvage_anc.py/anc_dedup_view.py/anc_build.py/insert_generic.py）。**529 Overloaded** 會整段 wipe classify/verify（非額度）→從 journal salvage classified（古代這輪救回 120 inScope）。
...
**待續**：論藏各代✅。史傳藏：~~近代 22→71~~✅ ~~近代 71→101~~✅ ~~古代 47→73~~（2026-06-22）。**下一步：第二輪古代 sweep 補 73→100**（角度換：個別聖徒傳 Vita/Passio、科普特-喬治亞-衣索匹亞聖傳、個別主教/教座傳、拜占庭編年 Theophanes/Nikephoros 邊界、高盧西班牙國族教會史 Gildas/Isidore），再補中 20／現 13。之後輪補其他薄藏正藏<100（律/譯校/書函/禮儀/詩藝/類書/宣道）。提案永不自動 merge，一律人工過目後入庫。
