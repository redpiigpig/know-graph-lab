...
### 8.2 研究自動化流程（測試＋人工過目閘；2026-06-16）

> user 選定「測試＋人工過目閘 ＋ 建規格＋排程定期跑」：排程定期**上網研究產『待審提案』**，merge 由人過目放行（**不自動入庫**，因策展準確性）。

- **收錄目標規格**＝`data/dazangjing/collection-goals.ts`（`COLLECTION_GOALS` 十藏×正/外 scope＋來源、`RESEARCH_QUEUE` 優先佇列、`COMPILATION_RULES`、`CLASSIFICATION_NOTES`、`ERA_BOUNDARIES`、`ERA_ASSIGNMENT_PRINCIPLE`、`DERIVED_RELIGIONS_BY_ERA`）。凡例頁 `/dazangjing/about` 由此渲染。
- **研究 workflow**＝`.claude/workflows/dazangjing-research.js`（Skill：`dazangjing-research`）。args＝`{era,eraName,boundary,collection,collectionName,canon,canonLabel,goal:{zhengScope|waiScope,sources},existingTitles}`。流程：多來源並行列候選(8–15/源)→對既有去重→逐筆對抗式查核真偽＋metadata(confidence≥0.6 才留)→回傳 `proposed[]`（含 source_citation）。
- **去重工具**＝`scripts/dazangjing_titles.py <era|all>`（dump 既有 title_zh）。
- **跑一輪 SOP**：① 取 `RESEARCH_QUEUE` 頂端項；② `python scripts/dazangjing_titles.py <era>` 取 existingTitles；③ `Workflow({name:'dazangjing-research', args})`；④ 把回傳 `proposed[]` 寫成待審檔 `scripts/data/dazangjing_proposals/<era>_<col>_<canon>.ts`（**勿直接寫進 data/**）；⑤ 摘要給 user 過目；⑥ 核可者才補進 `data/dazangjing/<era>.ts` 對應藏的部、跑 `vitest run test/dazangjing.spec.ts`、commit。
- **排程**：cron routine 定期觸發上述 SOP 產提案並通知（cadence 見 routine 設定）。提案檔僅供審閱，永不自動 merge 到 master。
