...
**📊 進度快照（2026-06-22）**：前 320／古 854／中 270／近 **380**／現 **285**＝**約 2,099 卷**。
- **論藏正藏各代已達標**：古 171／中 101／近 98／現 118。✅
- **史傳藏進度**：**近代史傳藏正 71→101 達標 ✅**（2026-06-22 sweep 283 真紀錄→110 inScope→雙重去重抓出 11 本已存檔→collapse 同作多版/剔二手全集論藏類書現外→32 提案→verify-only 32/32 過→人工剔 2（Burnet《當代史》偏世俗政治判外、莫斯科公會議改律藏）＝30 入庫）。各代史傳仍薄：**古 47／中 20／現 13＝下一目標**。
  - 近代史傳五部現況：宗教改革史 19／殉道與聖徒 18／宣教史 18／域外民族誌 11／教會通史 35。
  - 💡 教訓：sweep 回的「magnum opus」常已在既有 71 內（Strype Cranmer/Annals/Eccl.Memorials、Knox、Heylyn Ecclesia Restaurata、Fuller 兩史皆已存）——**title_orig 前綴去重是抓這類重複的關鍵**（scripts 暫存於 c:/tmp：salvage.py/dedup_check.py/insert_em.py）。verify 會亂建 division（church-biography 等）＋偶把正統學術誤判外（Wharton 駁 Burnet＝正非外），須人工覆判。
  - 🔧 新增 `dazangjing-verify-only` workflow（args.candidates→純 verify ≥0.6），撞額度後 salvage classified→單獨補 verify 用。
...
**待續**：論藏各代✅（古171/中101/近98/現118）。史傳藏：~~近代史傳 22→71~~ ✅ ~~近代史傳 71→101~~ ✅（2026-06-22）。**下一目標：古代史傳藏正 47→100**（教父教會史 Eusebius/Socrates/Sozomen/Theodoret/Rufinus、殉道錄、修道院史傳、東方教會史傳），再補中 20／現 13。之後輪補其他薄藏正藏<100（律/譯校/書函/禮儀/詩藝/類書/宣道，見 §8.2 進度快照前的各代計數）。提案永不自動 merge，一律人工過目後入庫。
