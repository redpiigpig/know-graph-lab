...
description: 五個基督教經典/傳統對照工具的入口（/scripture 聖經多版本+教父註釋+各教會次經第二正典 / /creeds 21 次大公會議+各教會尼西亞信經+新教信條全譜 / /canon-law 教會法規 / /fathers 教父著作搜索 / /apocrypha 典外文獻搜索）。Status: **/scripture 32 版本平行對照 + /creeds 21 次大公會議 + /apocrypha 黃錫木 10 卷 Vision OCR 全部上線**。2026-05-31 /apocrypha 完成：123/132 卷 (93.2%) 有內容 / 2.26M 字繁中 / 2,058 footnote / 教父-style reader（10 sections/頁 + 三層 sidebar + 註釋集中頁底 + 章節 label）。
---
...
| `/apocrypha` | 典外文獻搜索 | DB | ✅ **黃錫木 10 卷 / 123 docs / 2.26M 字 / 2,058 footnote** |
...
**Phase E：黃錫木《基督教典外文獻》OCR** ✅ 2026-05-31 完成 — Vision OCR + classifier + reader 全套上線。詳見下方 `/apocrypha` 章節。文字已乾淨可作 OT 偽典／NT 偽典中文來源。
...
✅ **黃錫木主編《基督教典外文獻》10 卷**（基督教文藝出版社 2008-2013）
...
apocrypha_versions (
  code VARCHAR(30) PK,              -- 'cct_zh' (黃錫木主編) / 'charles_apot' /
...
1. **英文骨架為權威**：公版英譯（OT 偽典 Charles APOT 1913 / NT 偽典 M.R. James 1924）的 **章:節 = 唯一骨架**；黃錫木中文「貼上」骨架 1:1 → 節數跟著英文（不過度切節）、中英同 `(ch,v)` 對齊。中文某節無對應就留白或併入鄰節。
...
- 中文：黃錫木全文分 window 餵 **NVIDIA qwen3-next**（引擎 NVIDIA-first：徹夜 jung/mueller 把 Gemini free pool 佔滿且 request 會 hang；deepseek-v4-flash reasoning 模型大 JSON 會 timeout）；prompt 給「英文各章開頭內容」當語意 anchor → 中文對到正確 `(ch,v)`；輸出經 `merge_verse_windows` clamp 到骨架。
...
**主編＝黃錫木（2026-06-11 使用者確認）**：《基督教典外文獻》全套主編是**黃錫木**（非先前誤記的黃錫木）。`apocrypha_versions.cct_zh` 已更新 `name_zh='基督教典外文獻 (黃錫木主編)'`、`name_en='… (ed. Huang Ximu)'`。
