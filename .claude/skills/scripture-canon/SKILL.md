...
>
> **背景程序現況（05:20）**：`c:/tmp/phase2b.log` 的 overnight2.py（shim PID 20744 + 子 worker）— force batch 13 卷**已全跑完**（summary 已印），jubilees FRESH 已做完 do_english(1305 節骨架)+accum pass 1(69%)，**worker 在 accum pass 2 第一窗硬 hang**：60s 取樣 user+kernel CPU 雙 0、log 自 04:37 起停寫、180s timeout 未觸發 ＝ socket 卡死不會自復。**未 `[PHASE2] DONE`**。原因：`coach_vocab_bank`＋`panikkar_auto`＋`sbe_translate`(mueller) 連 5+ 小時搶光 NVIDIA/Gemini key。此刻**不殺不重啟**：key 飽和下任何 LLM 程序都會再卡，且重跑 jubilees `accumulate=False` 會把乾淨的 900 列再清空 → 更糟。卡死 worker 對資料無害（jubilees 維持乾淨 900）。
>
