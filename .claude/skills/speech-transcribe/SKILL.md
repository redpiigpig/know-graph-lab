...
description: 受邀演講／學系專題講座／公開講座的完整 pipeline — 海報資訊 OCR → ffmpeg 切音 → Gemini Audio 平行轉錄（含 PPT context）→ Claude 對話內清稿 → PPT 上 R2 → 寫入 public/content/speech/ 並更新 stores/speech.ts。Use when 使用者有新一場演講的「海報＋錄音＋PPT」要上架到 /speech。
---
...
把一場演講（學系專題講座、研討會主題演講、公開講座等）從**海報＋錄音＋PPT** 一路整理成 `/speech/[id]` 上可閱讀的逐字稿頁面，並把 PPT 放到 R2 供下載。
...
錄音（m4a/mp3）           ─┘
       ...
       │
       └─→ public/content/speech/[date-slug].md  +  stores/speech.ts 加新 entry
```
...
| 海報 | 使用者貼進對話的截圖 |
| 暫存 | `_tmp_audio/speech/[YYYY-MM-DD]/` — partN.m4a、partN_raw.txt、ppt_context.txt |
| R2 PPT | `talks-ppt/[YYYY-MM-DD]-[slug].pptx`（bucket = `knowgraphlab`）|
| 上架逐字稿 | `public/content/speech/[YYYY-MM-DD]-[slug].md` |
| Store | [`stores/speech.ts`](../../../stores/speech.ts) |
    ...
    for shape in slide.shapes:
        if shape.has_text_frame:
            for para in shape.text_frame.paragraphs:
                ...
                if t: lines.append(t)
    lines.append('')
Path('_tmp_audio/speech/YYYY-MM-DD/ppt_context.txt').write_text('\n'.join(lines), encoding='utf-8')
"
```
...
### 輸出格式（`public/content/speech/[id].md`）
...
### 7b. [`server/api/speech/ppt-download/[id].get.ts`](../../../server/api/speech/ppt-download/[id].get.ts) — 加 mapping
