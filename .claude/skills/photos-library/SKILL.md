...
> Drive 父目錄：`G:/我的雲端硬碟/資料/知識圖工作室/照片/`
...
|---|---|---:|---|---|
| chenwei  | ✅ 完成 (2026-05-19)＋ 159 stragglers ✅ 已跑完 (2026-05-22) | 13,022 | ✅ 25,001 thumbs | 2026-05-17 第一輪；2026-05-19 補匯入 594 JPG + 36 MP4（Sony HX99）+ 修 440 iPhone 截圖；2026-05-21 刪 626 個 `.AAE` iPhone sidecar；2026-05-21 audit 發現 3 個事件夾共 159 檔未 rename（見「待辦 #5」，✅ 2026-05-22 完成；另 2026-06-25 本機 C: 大缺口補檔後 index 已達 24,803 media，見下方專節）|
...
2. **搬資料夾**：在 Drive 網頁對 `02. 歷年活動照片-69-119` 右鍵 → 移動 → 我的雲端硬碟/資料/知識圖工作室/照片/弘誓相片/
3. **跑 rename**：
   ```powershell
...
### 🎬 雲端影片播放＝Drive 預覽嵌入（2026-06-26 ✅ 已實作＋iPad 實測可播，無 SA）
**問題**：手機/平板連雲端 `PHOTO_BACKEND=r2`；[server/api/photos/file.get.ts](../../../server/api/photos/file.get.ts) r2 模式**影片一律 404**（R2 只存縮圖、無影片原檔）。影片只在 G:，只有 local 模式 `<video>` 播得出來。次要：多數片是 **HEVC**（Android Chrome 不支援）、串流端無 HTTP Range（iOS 需要）。

**解法＝把每支影片的 Drive 檔案 ID 寫進 index，雲端影片改嵌 Drive 播放器**（靠 viewer 自己的 Google 登入驗證、Drive 自動轉碼解 HEVC、R2 不存影片）。

**關鍵：檔案 ID 從本機 Google Drive for Desktop 的 DB 直接讀，不需 Service Account / Drive API / 共用資料夾。**（一開始想用 SA，但發現本機 DriveFS DB 有現成對照，全砍掉。使用者誤把 SA JSON 貼進 .env → 已抽到 gitignored 檔又移除，最終不用。）
- DB：`%LOCALAPPDATA%\Google\DriveFS\<accountId>\metadata_sqlite_db`。**要複製 main+`-wal` 後 `PRAGMA wal_checkpoint(TRUNCATE)`** 才看得到真表（直接開只有 activity_items，資料全在 WAL）。關鍵表 `items(stable_id,id=Drive檔案ID,mime_type,is_folder)` + `item_properties(key='local-title'→檔名)` + `stable_parents(建資料夾樹)`。
- [scripts/harvest_drive_ids.py](../../../scripts/harvest_drive_ids.py)：從 DB 抽三相簿每檔 → `scripts/drive_ids.json`（gitignored）`{album:{relpath:driveId}}`。72,055 檔、路徑對 G: 驗證 100%。
- [scripts/build_photo_index.py](../../../scripts/build_photo_index.py)：載入 drive_ids.json，**只給影片**加 `driveId` 欄位（省 index 大小）。
- [server/utils/photos.ts](../../../server/utils/photos.ts) `indexFileToPhotoFile` 把 driveId 傳給前端。
- 前端 [chenwei month page](../../../pages/photos/chenwei/[year]/[month]/index.vue) + [PhotoFolderBrowser.vue](../../../components/PhotoFolderBrowser.vue)：`isCloud` 且影片有 driveId → grid 用 `drive.google.com/thumbnail?id=&sz=w480`（@error fallback 🎬）、lightbox 嵌 `file/d/{id}/preview` iframe + **「用 Drive 開啟」按鈕**（`/view` 新分頁，iPad Safari 擋第三方 cookie 時的保底）。
- **維護**：新影片 → 重跑 harvest_drive_ids.py + build_photo_index.py + `sync_photos_to_r2 --index-only`。DriveFS DB 格式未來 App 改版可能要修 harvest。
- ⚠️ 本機 `npm run build` 會在 nitro `client.manifest.mjs` 掛（RollupError，與 `#app-manifest` 同源的既有環境 bug，**非本功能造成**，client+server 都編譯成功）。
- ✅ **2026-06-26 使用者 iPad 實測：影片可播**（commit `71dc38c2`）。若嵌入播放器被 Safari 第三方 cookie 擋，按「用 Drive 開啟」保底一定能播——屬預期行為。
...
6. **err=36 真相＝18 個「誤命名影片」**（2023.02 IMG_6786/6811/6812…＋2021-05-08(81) 等）：magic bytes `ftyp…qt  `＝**QuickTime MOV 影片存成 .JPG**（非壞圖），故 sharp 報 `unsupported image format`。**檔案在 Drive 沒問題、只是副檔名錯 → 無縮圖**，非致命。修法＝改副檔名回 `.MOV`。
   - ⚠️ **別用本機 magic-byte 掃描**：讀 12 bytes 也要 cold fault-in，2023.02 一夾就 1,779 檔、2 分鐘跑不完、易 wedge（同 classify 症）。[scripts/fix_misnamed_media.py](../../../scripts/fix_misnamed_media.py) 這支本機掃描版**過慢，已棄用於實跑**。
   - ✅ **正解＝在影片 Drive 嵌入 Phase 1 用 Drive API `mimeType` 偵測**：`harvest_drive_ids.py` 抓 fileId 時，順手把 mimeType=`video/*` 卻叫 .JPG/.jpeg 的檔用 Drive API 改名成 .MOV（走 API、非冷掛載，秒殺無 wedge）。#2 併入 Phase 1 一起做。
...
1. 把照片丟到 `G:/我的雲端硬碟/資料/知識圖工作室/照片/辰瑋相片/{YEAR}相片/`（直接放年份根目錄）
