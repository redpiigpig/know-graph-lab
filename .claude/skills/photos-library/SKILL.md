...
## 訓練相簿 D: 備援補檔 + 轉碼（2026-06-25，**未全部完成**）
...
### ⏳ 收尾連鎖（2026-06-25 07:30 已啟動 detached，**結果未確認**）
`scripts/_training_final_pass.ps1`（PID 當時 34232）依序跑、abort-on-error、log 在 `C:\tmp\final_pass.log`：
1. `rename_photos.py auto training`（更名+重建 index）2. `convert_heic_to_jpg.py`（181 HEIC→JPG）3. `build_photo_index.py` 4. `transcode_videos.py`（轉新事件影片）5. `build_photo_index.py` 6. `sync_photos_to_r2.mjs training` + `--index-only`
- **下次先看** `C:\tmp\final_pass.log` 結尾：`=== FINAL_PASS DONE ===`＝全過；`FAILED at <step>`＝停在該步。各步皆 idempotent，**直接重跑整支 ps1 安全**。
- 已給 `transcode_videos.py` 加永久排除政策 `TRAIN_EXCLUDE_TOP = {2017, fitcasting, 一般相片, 新增資料夾}`（這些夾不轉碼）。

### 🔜 下次插回 D: 槽要做的（ledger 都可續跑）
1. **補 fitcasting 剩 31 支**：`python scripts/fitcasting_transcode_fill.py`（讀 `C:\tmp\fitcasting_ledger.json` 續跑，會自動跳已完成）→ 完再跑一次收尾 ps1
2. **fitcasting 去重**：那 18 支 Drive 原有的（在 `fitcasting/new/`、`fitcasting/增加資料夾/` 子夾）跟 D: 613 支時長都對不上、沒被當重複跳過，最壞 18 支重複，核對去重
3. **新增腳本**：[scripts/fitcasting_transcode_fill.py](../../../scripts/fitcasting_transcode_fill.py)（D:→暫存→G: 轉碼補檔，不先上傳 90GB 原檔）、[scripts/_training_final_pass.ps1](../../../scripts/_training_final_pass.ps1)（收尾連鎖）
...
## 辰瑋相簿 本機 C: 大缺口補檔（2026-06-25，**未執行，等訓練收尾跑完**）

> 來源：`C:\Users\張辰瑋的資料\辰瑋相片\`（2014–2024相片）＝辰瑋本機完整集；比對發現 **G: 辰瑋相簿大約缺一半**。
...
- **HEIC**：C 共 629（G 只轉過 196），需另比對；**CR2 RAW 21 檔 G 完全沒有**
- C: 結構：**2021–2024 已在 `{Y}.MM` 月夾**（同 G:，直接落對應月夾免 classify）；2014–2020 事件子夾(心靈之旅等)+部分散檔(2017散897/2018散2499/2019散2485)

### 使用者決策（2026-06-25）
- **全部補上**（走 classify+rename）；**CR2 補、HEIC 補（轉 JPG）**

### 執行方式（**訓練收尾連鎖跑完才能動**，否則 rename/index 互撞 + 共用碟 I/O 爆）
跑 [scripts/_chenwei_fill_pass.ps1](../../../scripts/_chenwei_fill_pass.ps1)（detached）：
1. [scripts/chenwei_fill_from_local.py](../../../scripts/chenwei_fill_from_local.py)：缺的 img/vid 依**相同相對路徑** copy 到 G（2021-24 直落月夾、2014-20 保留事件夾結構）；CR2 全 copy；HEIC 先轉暫存 jpg(q92)→ size 比對 G，已在跳過、否則 jpg 直落 G 相對路徑（避免跟 196 舊轉檔重複）。ledger `C:\tmp\chenwei_fill_ledger.json` 可續
2. `classify_photos.py` plan+execute（散檔分月，月夾/事件夾不動；move log 可 rollback）
3. `rename_photos.py auto chenwei`（更名+重建 index）
4. `build_photo_index.py` + `sync_photos_to_r2.mjs chenwei` + `--index-only`
- log：`C:\tmp\chenwei_pass.log`，結尾 `=== CHENWEI_PASS DONE ===`；各步 idempotent，可重跑整支 ps1
- ⚠️ Drive 雲端配額要夠 ~45GB；HEIC 去重靠「同 script 同 quality 轉同源 HEIC → byte 相同 size」
