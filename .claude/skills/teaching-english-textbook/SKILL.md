...
### 🚨 最貴的那一條：驗收要驗「成品」，而且不可以拿局部講全稱

2026-09-17 使用者問「所以你有寫過測試每一頁都檢查過了是嗎」。當時我已經回報過
「十三道閘全綠、50/50 通過」，聽起來像驗收完了。實際上：

- 那 45 項測試測的是**閘門函式本身**，餵的是手寫假資料，不碰課本內容。
- `--check` 跑過全部 50 課，但只驗那十三條**機械規則**；規則沒涵蓋的一律說 ✓。
- **一頁 PDF 都沒看過**。版面改對了沒有，我是用「頁數少了 47 頁」推論的。

去把 PDF 渲染成圖來讀，一次就看到 `Please, I am fine.`、`we have OK`。接著
每看一個新區塊就再找到一批東西：

| 我當時說 | 實際 |
|---|---|
| 十三道閘全綠 | 課文有不是英文的句子 |
| 「課文成了有情節的短文」 | 那是中途抽樣；最後落地的更差，我沒重看 |
| 「練習題經得起檢查」 | 只看了選擇題頁；重組與造句有 `We are hello.`、`I am thank.` |

規矩：
1. **驗收一定要把 PDF 渲染成圖來看**（`fitz` 開檔 `get_pixmap(dpi=135)`，B5 約
   1000×1400px，在截圖上限內）。頁數、字數、通過率都不能代替看。
2. **一課有六個區塊**（學習目標／課文／單字／文法／情境對話／練習／解答），
   看了其中一個不等於看過這一課。至少各抽一頁。
3. 報告只講**你真的看過的範圍**：「抽看了上下冊各 N 頁，含解答頁」，不要寫
   「練習題沒問題」。



### 全書掃描：先驗你的掃描器，它比課本更容易出錯

2026-09-18 使用者說「都要一次做好」，於是把 408 頁全部掃過一遍。四類警示，
**沒有一個是真的**：

| 掃描器說 | 實際 |
|---|---|
| 缺字方框 50 處 | `□` 是**學習目標的核取方塊**，本來就該印 |
| 英文句混全形標點 107 處 | 抓到 `s。`、`r，`——那是中文句尾剛好接在英文字後面 |
| 超出版心 37 處 | 頁寬 516pt、右界 476pt，離頁緣還有 40pt，正常行末 |
| 課文／文法裡的全形標點 97 處 | 全是中文說明句嵌英文詞（「…用哪個動詞。」），用全形是對的 |

要是把那 107 處照單全收去改，會把正確的中文說明改壞。

判準要跟意圖對齊：想抓「英文句用了全形標點」，就要**逐句切開、確認整句沒有中文**
再判，不能用「英文字母後面接全形標點」這種代理指標。改對之後全書是 0 處。

真正掃到的只有一件：造句翻譯的第 8 題被擠到單獨一頁（408 頁裡 4 頁），
最後兩題加 `keep_with_next` 就好。
