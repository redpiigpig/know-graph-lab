...
episcopal_succession 六項 SQL 稽核（懸空 predecessor/consecrator FK、年代倒置、同席同號重複、see↔sees 表對齊、前任較晚）已全跑過並修復：sees 表 8 列中間點半形「·」→全形「‧」正規化（succession 3,141 列全用「‧」，字元不一致曾讓凱撒利亞×2/長安景教 25 位主教連不到教座）；圖爾庫/哈瓦那兩筆任期年顛倒；漢堡鏈修復（現任赫塞原編號 3=「1994 重建後第 3 任」撞進中世紀鏈，移鏈尾 17 號，翁尼前任改回林伯特）。殘留 13 筆「前任較晚」為刻意策展（帕拉馬斯路過停留/西里爾在此出生等名人註記卡、亞美尼亞宗主教多任期），非 bug 勿修。

## 🌍 全球教座大幅擴充（2026-07-12 起，目標 +300-500 座）

使用者定調現有 397 座偏少（全球有記載主教譜系數千）；確認 telescoping UI（預設收合＋「+N 被立」選單逐層 reveal＋focus occlusion 空間重用）可承載，關鍵在**每座 parent wiring 正確**即收在對的設立者選單下。

**可重用管線**：研究 → 結構化 JSON → `scripts/apply_episcopal_batch.py`。
- JSON schema 見該腳本檔頭；`parent_see_zh` 用可讀座名（歧義用 `座名|church`），apply 時解析成 parent_see_id、插 see＋主教鏈（串 predecessor_id）、**冪等**（see_zh+church 已存在跳過）、parent 解不到就跳過不建孤兒。
- 批次 JSON 存 `database/episcopal-batches/waveN_*.json` 當記錄。
- ⚠️ **主教名單寧缺勿虛構**：see 詮釋（名/傳統/年/parent/現任首牧 see 層欄位）可靠優先；主教鏈只放高把握錨定（現任＋奠基＋著名），不全就 notes 標「主教鏈部分/待補」。研究 agent 跑大量 WebFetch 會 watchdog stall，改由主 loop 依既有知識產 JSON 較穩。
- 驗收：apply 後查「懸空 parent FK=0」＋抽查新座 parent＋API graph 含新座名。

### 已完成 3 waves（2026-07-12，397 → **446 座**，+49 distinct）
- **Wave1 東正教自主/都主教座 7**（`database/episcopal-batches/wave1_orthodox_autocephalous.json`）：布加勒斯特/貝爾格勒/索非亞/地拉那/華沙(正教)/普雷紹夫/帕特雷（接君堡或雅典，split=自主年）。
- **Wave1 信義宗下屬教區 20**（`wave1_lutheran_dioceses.json`）：瑞典 8（隆德/斯卡拉/林雪平/斯特蘭奈斯/韋斯特羅斯/韋克舍/哥德堡/維斯比/斯德哥爾摩）＋芬蘭 3＋挪威 3（尼達洛斯等）＋丹麥 2＋非洲 3（美卡尼耶穌/坦尚尼亞/馬拉加西）（接各國已有信義宗首牧座，split=宗改年）。
- **Wave2 羅馬公教大主教座 17**（`wave2_rc_metropolitans.json`）：薩爾茨堡/佛羅倫斯/巴勒莫/梅赫倫-布魯塞爾/埃斯泰爾戈姆/聖多明各(美洲首席)/聖薩爾瓦多巴伊亞/瓜達拉哈拉/金夏沙/達卡/阿比讓/雅溫得/羅安達/宿霧/可倫坡/長崎/曼谷等（接羅馬，post-600 拉丁禮直屬）。
- **Wave3 東方正教 eparchy 5**（`wave3_oriental_eparchies.json`）：衣索比亞正教(亞的斯亞貝巴)/厄立特里亞正教(阿斯馬拉)接科普特亞歷山卓；亞美尼亞伊朗(新朱利法/德黑蘭)接埃奇米亞津；馬蘭卡拉正教(科塔亞姆)接馬拉巴爾。
- 全部懸空 parent FK=0、進 API graph、主教鏈守寧缺勿虛構。

### 🔑 關鍵發現：真實缺口遠小於「+300-500」表面數字
冪等去重擋掉**大量「以為缺、其實已有」**的座（光 Wave2 RC 40 候選就 22 個已存在：利馬/波哥大/首爾/東京/聖地亞哥/河內…）；加抽查 RC 大主教 26/33、聖公宗 10/11 已在庫——**DB 遠比表面完整**。真實 distinct 缺口就是這 3 waves 補的這類（自主教會首都/北歐信義宗下屬/真漏的 RC 大主教/東方 eparchy），**再幾十個即補齊高價值部分**，並非憑空多 300。堆到 +300 只能加「各國二三線 suffragan 長尾」（每國幾十小教區）——UI 撐得住但價值低、**手寫會逼近虛構違反紀律**。

### 待續（下次接手，二選一或並行）
1. **再一波高價值真缺口（推薦，~+30-50）**：聖公宗非洲省級大主教座（奈及利亞/烏干達/肯亞/坦尚尼亞）、更多 RC 歐洲 gap、東正教希臘/俄羅斯都主教座。做法同上（手寫 JSON→apply）。
2. **長尾 suffragan 用權威源腳本灌**：寫爬蟲從 catholic-hierarchy.org / gcatholic.org 抓現任主教清單（有據不虛構），轉本批次 JSON schema 再 apply。開新 session 跑較穩。
