...
> - **面板**：2026-09-17 起 `translation_dashboard.py` **不再顯示 ACCS**——本叢書
>   2026-09-11 完工（稽核八項全綠），完工項留在面板上只是每次重掃。要看數字改查 DB
>   或跑 `scripts/accs_audit_quality.py`；真的有新卷取得授權再把那一組加回去。
...
| 利未記 | 12–27 | OCR 整段吐空 | ⏳ 已補到 19/27，剩 PDF 311–332（排隊中）|
| 詩篇 | 28–50 | 同上（詩1-50 卷 PDF 325–588 頁全空）| ✅ **150/150 章、3,694 列**（原 2,682）|
| 雅歌 | 3–8 | 🚨 **掃描檔只掃到一半** | ❌ 要補掃**書頁 431–514**（84 頁）|

回 PDF 核過那些空頁確實有正文：詩1-50 第 400 頁印的是「詩 34.1-22」、出利民申
第 330 頁是「利 27.26-34」。

🚨 **雅歌那一本是「檔在、但只掃了一半」，不是 OCR 漏抓。**《箴言、傳道書、雅歌》
（校園 2010，ISBN 978-986-198-152-9）目錄：箴言 1／傳道書 259／**雅歌 395**／
附錄 515。掃描檔共 472 頁，最後一頁是**書頁 430**、頁眉「歌 2.1-7」，正文在
「這朵花為所有人而生長 ◆ 希坡律陀：……因為」之後**斷在句子中間**。
偏移量 **書頁 ＝ PDF 頁 − 42**（書頁 395 ＝ PDF 437，與 config 的 sng 起點相符）。
所以 config 裡 `sng: 437-472` 的終點只是「檔案到此為止」，不是雅歌的結尾。
**要補掃的是書頁 431–514，共 84 頁**（雅歌 2:8–8:14）；附錄 515 之後不影響註釋。
補掃進來之後：把新頁接到同一個 PDF、重算 `sng` 的 ranges、刪掉 checkpoint 裡
雅歌那些行與 `.done`，再 `--resume` 重跑。
...
### 🚨 冪等修正一定要**等所有 OCR 收工才跑**

`ingest_accs_genesis.py` 每次 upsert 都是拿 raw 重建整批列，**會把三支正規化腳本
改好的教父名／書名蓋回 raw 的寫法**。2026-09-03 實測：跑完 `--apply` 改了 1,288 列，
複查馬上又有 590 列不一致——因為 fleet 的 `accs_ocr_run.py` 正在同一卷上重跑。

所以順序是死的：**先把該卷所有頁 OCR 完 → 再跑五支修正**。判斷還有沒有在跑：

```bash
wmic process where "name='python.exe'" get CommandLine | grep -i "ingest_accs_genesis\|accs_ocr_run"
```

🚨 **移走 `.done` 之後 fleet 會自己接手。** 詩篇這次就是這樣補回來的——我把
`.done` 移走、config 的頁界改成 39-588，`KGL_Fleet_Keeper` 那一條 lane 在兩個半
小時內把死區補完（2,682 → 3,694 列）。**所以別急著自己排一份重跑，先看 fleet 有
沒有已經在做**，兩邊同時跑只是互搶 Gemini。
（反過來說，config 裡沒有的卷 fleet 不會碰——創世記、出利民申、書士得撒那三卷
不在 `accs_volume_config.json` 裡，只能手動跑。）
...
索引全算進去。目錄寫得清楚：**註釋 詩篇1-50篇／書頁 1、附錄／551**，偏移
**書頁 ＝ PDF − 38**，所以正文只有 **PDF 39-588**。已改。

先前只對「多書卷合刊」做過附錄切點（`accs_find_boundaries.py` 會問目錄要附錄起始
頁），單書卷的卷因為 `single_book: true` 直接給了 `1-<page_count>`，這個坑就留著。
**19b（詩51-150，1-760）與其餘單書卷同樣沒切，要一併查。**

這一次沒出事只是運氣：詩1-50 那兩頁索引吐出 154 條，但全是主題索引
（`ref` 空 → `build_rows` 直接跳過）。羅馬書那次中招的是**引用經文索引**，行長得
就像經文引用（`19:18`），那種就會直接進表。實測 psa 2,682 列裡沒有索引行。

### 詩篇 28–50 的死區與重跑範圍

帳本逐頁攤開來看：PDF 1–324 正常（每 60 頁有 22–59 頁吐得出條目），**325 之後
一路到 660 一條都沒有**，661–708 那兩頁是主題索引。所以死區是 **PDF 325–588**
（書頁 287–550，詩篇約 27–50 篇），264 頁。checkpoint 裡那 191 行空頁紀錄已刪、
`.done` 已移走（備份在 `c:/tmp/accs_reocr_backup/`），`--resume` 就會重跑那一段。
...
### 書名錯字候選 235 組：已逐組判完，核可 79 條／改寫 132 列

`accs_normalize_works.py --propose` 產生候選，**判的依據不是字形而是「作者與書卷
分布」**——跟認教父同一個道理。判完寫進 `_zh_canonical`（79 條），套用 132 列，
冪等 0 列。相異寫法 2,742 → **2,570 種**。

核可的都落在四類，共同點是「換掉那個字不會改變它是哪一部書」：

| 類 | 例 |
|---|---|
| 反駁的 OCR | 反**駮**／**駴**／**駸**馬吉安 → 反**駁**馬吉安（12 組） |
| 註釋的異寫 | 詩篇**詮**釋／**注**釋、馬太福音**詳**／**詮**／**詁**／**評**釋 |
| 書卷名被 OCR 壞 | 創世**紀**→創世**記**、利**末**→利**未**、以**帖帖**→以**斯帖** |
| 專名字形 | 論**那**亞→論**挪**亞、論**引**隱→論**該**隱、致莫尼**學**斯→**摩**斯 |

🚨 **被我擋下來、絕不可併的**（下一手若看到它們「長得很像」，記得這是判過的）：

| 看起來像 | 實際上 |
|---|---|
| 馬可福音註釋(7) vs 馬太福音註釋(554) | **兩卷福音書**；可敬者比德真的寫過馬可註釋 |
| 論聖靈(124) / 論聖禮(9) / 論聖職(64) / 論聖子 / 論聖經 | **五部不同的書**，只差一個字 |
| 演講集(192) vs 短講集(29) | Orations vs 短講，不同 |
| 雜篇(107) vs 殘篇(16) | Miscellanies vs Fragments，不同 |
| 約翰一書(40) / 二書(4) / 三書(4) 註釋 | **三卷不同的書信** |
| 詩篇註釋 / 詩篇詳解(198) / 詩篇短講集(145) | **三部不同的書**，別因為都叫詩篇就併 |
| 羅馬書淺釋(12) vs 羅馬書註釋 | 淺／註 字形不近，是另一部書 |
| 七經問題 vs 八經問題 | 奧古斯丁兩者都寫過 |
| FC vs SC | 兩套不同的叢書縮寫 |

🚨 **另一個發現：有 1 個字的 work_title**（`思` 25 列、`和` 22 列、`詩` 7 列）。
候選表裡約 40 組是它們跟 `路`／`弗`／`耶`／`創` 這些聖經卷名縮寫配對，全是雜訊。
**這些單字 work_title 本身就是壞資料**，該另案處理，不是改名能解決的。
...
- 🚨 **不可「有《》就抽出來當書名」**。
  ...
  可還原與 9 列會配錯的分野。
- 分級後**只動逐列查證過安全的**，已修 **116 列**，複查剩餘正好 94 列：
...
**E 是這一輪最值得記的發現**：`body_zh` 整格只有一個《書名》，而 `work_title`
裝著整段正文——兩欄根本是對調的。網站上那 40 則顯示成「出處＝最多 475 字經文、
正文＝一個書名」。對調不丟任何資料（兩個值都保留、只是換位），是最安全的一種修法；
備份要把 `body_zh` 原值一起記下來，否則回復不了。

🚨 **我一度把 D 那批寫成「批次邊界切開的引文續行」，那是錯的，已推翻。**
拿「開頭像續行 ＋ 前一列 body 未收尾 ＋ 同章」三個條件去比對，**命中 0 列**。
真正的病根是欄位錯位，E 是它可解的那一半。別再照「續行」的方向去修這批，
也別把它跟 `accs_merge_split_quotes.py` 混為一談。
...
- 單字 work_title（思/和/詩 共 54 列）待另案處理。
- 94 列正文污染待人工判讀（A2 15／B 9／D 70）。
