---
name: collected-works-multilang
...
| [[fathers-translation]] | 教父原典（公有領域 Schaff/CCEL）中譯精修 | 1 source + 中 | ANF/NPNF/ACCS |
| **collected-works-multilang（本 skill）** | **多卷全集，3+ 語逐段對照** | **2+ sources + 中** | **榮格全集、佛洛伊德全集、某神學家德英中對照** |
...
- 公有領域教父原典（CCEL 乾淨 EPUB，只有英＋中）→ [[fathers-translation]]
...
### 本專案的處理姿態（跟 fathers-translation「參考現成中譯本校準」同源）
...
可沿用 [[fathers-translation]] 的 `parent_volume` 三層樹（作者 ⊃ 著作 ⊃ 章節）概念，但全集多半是**單一作者多著作** → parent_volume 改用「著作集分組」（榮格：《轉化的象徵》《心理類型》《原型與集體無意識》…）。
...
2. **長度比＋順序對齊**（無段碼時）：在同一章內，按段落順序 + 字數比例 greedy 對齊（類似 papal-magisterium 的 `alignDocs()`）。
...
- [ ] **基建：reader N 欄 + `sources` schema**（先做，跟語料無關；見「Reader 多欄擴充設計」）
  - [ ] `ChunkData` 加 `sources?` / `source_order?` + API passthrough
  - [ ] reader toggle 動態化 + parallel grid N 欄
  - [ ] translate 腳本多語 JSONL 輸出變體
...
- [[fathers-translation]] — 公有領域教父原典；「參考現成中譯本校準」姿態本 skill 沿用
- [[ebook-pipeline]] — parse / OCR / standardize / 套書 split 上游
- [[translation-glossary]] — 神學家/神學名詞詞庫（神學家全集才用；其餘全集各自 glossary）
- [papal-magisterium](../papal-magisterium/SKILL.md) — 既有「拉/英/中三欄逐段對照」(alignDocs) 的 content-file 版實作，可參考對齊邏輯（但本 skill 走 ebook reader，不走 content files）
