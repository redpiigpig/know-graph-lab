...
## 🆕 監督接手（2026-06-12 09:27，新 session 從這裡看）

> **一句話**：**全集 14 部翻譯全數完工 = 99%（28708/28852 段）。所有翻譯 pass 已結束、無背景翻譯進程在跑。** 剩 ~144 段（各書 1-3%，origin-growth 97% 最多）是補不上的眉批/OCR 雜訊，照設計跳過、reader 顯英文 fallback——**這是終態，非未完成**。新 session 的工作 = **下方「✅ 收尾」**（主要是 hub 各卷填 ebookId 變可點），不是再監控翻譯。
...
### ✅ 收尾（contributions-1 一到 ~99% 就做）
14 部全 ~99% 後：
1. **hub 各卷可點**：改 `stores/collectedWorks.ts` 馬克斯‧穆勒 author 的 works[]：status→`done`、填 `ebookId`（對照下表）。目前只有 isr 有 ebookId，其餘 13 卷待填。
   | slug | ebookId |
   |---|---|
   | origin-growth-religion | `44444445-4444-4444-8444-444444444444` |
   | natural-religion | `44444441-4444-4444-8444-444444444444` |
   | physical-religion | `44444442-4444-4444-8444-444444444444` |
   | anthropological-religion | `44444443-4444-4444-8444-444444444444` |
   | psychological-religion | `44444444-4444-4444-8444-444444444444` |
   | comparative-mythology | `4444444b-4444-4444-8444-444444444444` |
   | contributions-mythology-1 | `4444444c-4444-4444-8444-444444444444` |
   | contributions-mythology-2 | `4444444d-4444-4444-8444-444444444444` |
   | science-language-1 | `44444446-4444-4444-8444-444444444444` |
   | science-language-2 | `44444447-4444-4444-8444-444444444444` |
   | india-teach | `44444449-4444-4444-8444-444444444444` |
   | six-systems | `4444444a-4444-4444-8444-444444444444` |
   | chips-1 | `44444448-4444-4444-8444-444444444444` |
2. **殘段**（origin-growth 等 ~1-3% 補不上眉批）：接受為英文 fallback；或讓排程修正版 `--run-queue`（lock 失效後自動跑、fail-skip 會把死段標 exhausted）收尾。
3. **可停 auto-committer**（全 commit 後）；更新本檔進度為「翻譯完工」。
4. **重啟卡住的翻譯**（若某本沒人跑）：`python scripts/mueller_haiku_pass.py --loop --engine haiku --only <slug[,slug]>`（resumable）。NVIDIA 引擎龜速（4h 才 +5%），**收尾一律用 `--engine haiku`**。
