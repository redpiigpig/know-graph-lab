...
- **生效方式**：15164 載舊 code 且非本 session 啟（[[feedback_no_kill_other_tasks]]）→ **不停它**。改跑 `scripts/mueller_haiku_pass.py`：FIXED code + **Claude Haiku**（separate engine，無 NVIDIA/coach 競爭，`--work` 路徑免 lock），**跳過 origin-growth**（留給 15164，避免同檔併寫），順序翻其餘 12 本。
- **收尾待辦**：(1) 待 15164 自然退出後，下次排程 `--run-queue` 即吃新 code，可停用 Haiku pass；(2) origin-growth 的 ~74 死段日後若要補，需換 OCR 源或人工。
