...
> **一句話**：**全集 14 部「機翻初稿」全數完工 = 99%（28708/28852 段）；hub 各卷已 status=done＋ebookId、`/collected-works` 馬克斯‧穆勒頁全可點。翻譯 pass 全結束、auto-committer 已停。** 剩 ~144 段（各書 1-3%，origin-growth 97% 最多）是補不上的眉批/OCR 雜訊（英文 fallback，終態）。**新 session 的工作 = 下方「🔧 精修」**（機翻 → 出版品質校對），**不是再翻譯、也不是再做 hub 收尾（已完成）**。
