...
- [x] 抽英德 djvu txt、評估 OCR；Lecture I smoke + 三欄截圖驗證；《宗教學導論》全書四講＋兩附論（執行中）
- [x] 自動化 queue（mueller_auto.py）+ 排程（13 部雙語連續轉錄）
- [ ] 六派換可用 archive 源（mbp 無 txt）
- [ ] queue 完成後：hub 各書 status 轉「已轉錄」、有德文版者升三欄、mueller_glossary term sweep
