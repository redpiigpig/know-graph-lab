...
// 傘狀學科的次領域（era 層）可固定順序（不依生年）：ERA_ORDER['基督宗教研究']=['新約研究','舊約研究','次經與典外文獻','教會史']
```
...
- **基督宗教研究**（傘狀，2026-07-23）：`era` 當**次領域**用（非年代）＝`新約研究`／`舊約研究`／`次經與典外文獻`（2026-09-26 加）／`教會史`，順序由 `ERA_ORDER` 固定；作者**不設 region**（單純三小節，無地域層）。
...
| **基督宗教研究** | 54 | 傘狀四小節（era 當次領域）：**次經與典外文獻**（R.H.查爾斯→施內梅爾赫→維爾梅斯→斯通→查爾斯沃思→萊頓→加西亞馬丁內斯→柯林斯→克勞克→梅耶→金恩＋帕格爾斯自教會史移入；2026-09-26，對照獵表 bs-ap-* 十二筆）。肖像 54 位中 41 位已填（私人站不限 PD：Commons／維基 infobox／任教大學官網），Dodd、Jeremias、Käsemann、Eichrodt、von Rad、Noth、Bright、Childs、Latourette、Bainton、Frend、H. Chadwick、Schneemelcher 十三位維基無單人照留 emoji；文獻面見 /research-data/top-papers/biblical-studies（[[research-data-top-papers]]）／**新約研究**（戴斯曼→多德→耶利米亞斯→凱澤曼→梅茨格→亨格爾→R.布朗→桑德斯→鄧恩→海斯→賴特→波卡姆＋布特曼‧菲奧倫查）／**舊約研究**（威爾豪森→貢克爾→考夫曼→艾希羅特→奧爾布賴特→馮拉德→諾特→德沃→布賴特→克羅斯→柴爾茲→特里布爾→布魯格曼）／**教會史**（莫斯海姆→沙夫→哈納克→拉圖雷特→班頓→弗蘭德→歐文/亨利查德威克→帕利坎→A.卡梅倫→彼得布朗→威爾肯→岡薩雷斯→帕格爾斯→麥卡洛克） | REFERENCE／自譯（近代學術幾全受版權） |
...
### 4. 館藏「已有原文書」×「已有中譯本」通用對齊工具（2026-09-26）

上面 1–3 談的是**我們自己翻譯時**要對的原文／既有譯本；這裡是另一種常見情形：
**兩本書本來就都已經在館裡**（各自獨立入庫、各自的切分粒度），只是還沒有互相對照。
例：詹姆斯《The Varieties of Religious Experience》↔《宗教經驗種種》、伯格
《The Sacred Canopy》↔《神聖的帷幕》。這批清單在
`output/untranslated_inventory/pairs_v2.tsv`（分類「原文＋中譯（待對照）」，共 33 對）。

**沿用泰勒《原始文化》／弗雷澤《金枝》那次「把英文原著併進既有中譯」的做法**
（`scripts/merge_original_column.py`，2026-09-11），但那支是每本手調 regex／人工
補行號的一次性腳本。`scripts/align_reference.py`（純函式核心 + `scripts/tests/
test_align_reference.py` 35 例）把同一套策略通用化成兩層：

1. **章節錨點**：`build_blocks()` 把兩邊的 chunk 序列各自「帶著走」分組——
   偵得到章節鍵（`Chapter III`／`第三章`／`Lectures IV and V`／`1. Title`
   純數字標題…）就開新 block，偵不到就併入上一個 block（處理 OCR 每頁重複
   印一次書眉、也處理中譯本 `chapter_path` 欄位整本是空的情況：這時退而
   掃內文開頭一小截找書眉）。兩邊 block 用章節編號互相配對，**一個原文
   block 的編號可以是好幾個**（英文常把兩三講合成一個 chunk，中文卻是
   分開的——`extract_anchor_keys` 兩邊對稱處理：英文 "Lectures IV and V"、
   中文更誇張直接把「十六十七」黏在一起變成「第十六十七講」沒有分隔符，
   兩者都會拆回 `[16, 17]`）。
2. **章內段落層**：按累計字數比例分配（沿用 `merge_original_column.distribute`
   / `split_paras`），不重寫中文、chunk 數不變。

**中譯本獨有內容／譯者註／原文註腳／原文書目／刪節段落（2026-09-26 使用者定調）**：
- 中譯本獨有的內容（譯者序、譯後記、出版說明…）：照樣保留成 chunk，
  `source_text` 留空，加 `zh_only: true`，章名後補「（中譯本獨有）」。
- 譯者註（`譯者按`／`译者注`／`译注`／`譯按`…各種寫法）：留在中文 `content`
  裡不跟原文配對，統一標成 `〔譯注〕`（`normalize_translator_notes()`，只認
  行首／句首起手式，不誤殺「作者提到譯者」這類正文敘述）。**這是本工具唯一
  會動 `content` 的地方**，且只是標記正規化，不改變句意一字。
- 原作者的註腳／尾註：中譯本內文若找得到 `[^N]`／`〔N〕`／`(N)` 這類參照
  標記，逐條按編號精確配對（`align_notes()`）；中譯本完全沒收錄整個
  註釋／尾註區塊時，原文整段列進 dry-run 報告的「刪節」清單（`omitted_in_
  translation`），**不會硬塞進某個湊巧存在的 chunk**——沒有對應 chunk 就
  沒地方放，报告裡看得到刪了多少字，不會憑空消失也不會亂配。
- 參考書目：走一般章節/扉頁配對（`classify_zone` 認得 bibliography 關鍵字），
  兩邊都有就對齊，中譯本沒收就整段進刪節清單。
- **索引可以略過**：`classify_zone` 認得 `index`／`索引`，也認單一大寫字母
  的節標題（書後索引常按字母分節）；索引缺漏**不計入刪節統計**。

**用法**：

```bash
python scripts/align_reference.py --orig-id <原文ebook_id> --zh-id <中譯ebook_id> --dry-run
# 看章節/段落對上率、抽樣（--samples N，各截 80 字）、刪節與 zh_only 清單都合理再：
...
**2026-09-26 兩本試做結果**：詹姆斯章節錨點 18/20（90%，OCR 太髒的 2 講
找不到）、段落配到原文 87.6%；伯格章節錨點 7/7（100%）、段落配到原文
64.8%（英文原始段落數少於中譯本既有的細切 chunk 數時，多出來的 chunk
結構上就是配不到——比例分配是把「不可切割的一整段原文」分給某一個
既有中譯段落，原文段落用完了，後面的中譯段落只能留空，這不是 bug，
是誠實的覆蓋率數字）。兩本都已寫入＋推 R2。

**推廣到其餘 32 對之前的風險（按遇到機率排序）**：
1. **中譯本 `chapter_path` 欄位是空的**（詹姆斯、伯格皆如此）——章節錨點
   完全要靠掃內文開頭書眉，OCR 越髒漏得越多；先看 `--dry-run` 的章節對上率，
   低於 7 成先人工看幾個「缺」的樣本再決定要不要補救。
2. **只切成 1–4 段的中譯本要先重轉**（`pairs_v2.tsv` 裡「太短」那 10 筆已被
   分類排除，不在 33 對範圍內；但範圍內的也可能有切得太粗的，會讓章節內
   段落覆蓋率被少數幾個超大 chunk 拉低到看起來很差，其實只是切得粗）。
3. **原文若也是掃描/OCR 書**（不像詹姆斯/伯格的原文是乾淨 EPUB／PD txt），
   `chapter_path` field 一樣可能是空的，兩邊都要靠內文掃描，錯誤會疊加。
4. **合章／跳號的中西文標題寫法千變萬化**——目前處理了「Lectures IV and V」
   「第十六十七講」兩種，遇到新花樣（例如中文用頓號「第四、五講」）要加
   對應規則並補測試，不要憑感覺放寬正則。
5. **原文若也不是一本書一個 chunk 而是逐段的**（本次兩本原文剛好都是「一
   章一個既有 chunk」的乾淨結構），章節分組邏輯一樣適用，但段落分配的
   權重來源會變（要用原文自己的段落數而非整章一坨字），目前程式沒有
   特別區分，會自動退化成「一整塊裡再切段」，數字上沒問題，但值得留意。
...
**④岩波《內村鑑三全集》20 卷 ✅ 完工（2026-09-18）**：1932–33 岩波掃描本、
18,074 頁 / 0.88 GB，本機 MinerU 全數轉錄並入庫 —— **17,498 段／1,240 萬字**，
`parsed_at` 20/20、`parse_error` 全清。`scripts/uchimura_zenshu_ocr.py`（排程已 DISABLE）。

由此重建《聖書之研究》357 號書目（`seisho_kenkyu_index.py` → `seisho_kenkyu_build.py`）：
**有篇目的號 345→350、缺號 12→7、篇目 1302→1353**。

🚨 **這條線踩到兩個會靜默毀掉成品的坑，做其他日文全集前務必先看：**

1. **簡繁轉換不可以套在日文上。** 入庫路徑的 `_trad()` 把第 11 卷整本改掉 0.78% 的字：
   `云ふ→雲ふ` 604 處、`余輩→餘輩` 61 處、`岩波書店→巖波書店`、`淀→澱`（毀掉「淀橋」）。
   **字數、段數、頁碼、簡體殘留率全部正常**，結構性稽核一律看不出來。
   已在 `ocr_with_gemini.py` 用假名偵測擋掉，而且 `book_is_japanese()` **整本算一次**
   ——逐段判會漏掉日文書本來就沒假名的扉頁／英文目次／年表（20 卷有 6 卷中招）。
   驗一卷乾不乾淨：`云ふ` 應遠多於 `雲ふ`、`余輩` 應遠多於 `餘輩`、`巖波` 應為 0。

2. **「比較準的 OCR」不等於「抽得比較多」。** 拿 MinerU 的文字重建書目，結果
   **比 archive.org 的粗 OCR 還差**（相異號 339 vs 349）——MinerU 版面感知會把年譜
   表列切成多行，而解析是逐行比對，一斷就不匹配。`--source both` 逐號取較完整者
   才超過基準（352）。別想當然把新引擎當舊來源的升級版直接換掉，**要量**。
...
判準六條（`unusable_reason(text, source)`，測試
`scripts/tests/test_translation_output_gate.py` 20 綠）。
**同一天被使用者抓到四次**，每一次都是「上一版沒想到的形狀」：

| # | 代碼 | 抓什麼 | 為什麼前面幾條擋不住 |
|---|---|---|---|
| 1 | `truncated-reasoning` | 未閉合的 `<think>` | — |
| 2 | `reasoning-leak` | 英文推理招牌句 | 舊守門員只認**中文**提示詞 |
| 3 | `untranslated` | 英文散文殘留（虛詞密度 ≥4.0） | — |
| 4 | `partial-untranslated` | 中文段落裡夾著 ≥120 字母的英文句 | 整段比例過得了關 |
| 5 | `self-talk` | 中文裡的英文自我商議（≥2 個商議語） | 大半是漢字、英文碎片都很短 |
| 6 | `degenerate-repetition` | zlib 壓縮率 < 0.15 | 滑動視窗對不上週期就漏掉 |
| 7 | `untranslated-japanese` | 假名佔比 ≥0.25 **且開頭 12 字內有假名** | 1–6 全在量拉丁字母 |

🚨 **每一條的門檻都是掃過全庫 62,243 段、逐一看樣本才定的，不准憑感覺加判準。**
量下來被剔掉的候選：爾／汝直接計數（1,066 段，幾乎全是音譯人名泰戈爾、
克爾凱郭爾、貝爾）、吾（203 段，多半是人名宮部金吾）、是以（命中「不**是以**
宣教士的身份」）、者也（命中「講**者也**應當」）、之際／以至於（現代書面語）、
未嘗不設限（命中「未**嘗試**馴化」）。

🚨 **四條各有各的「長得很像但不能動」：**
- 第 3 條的**虛詞密度不能省**——穆勒與潘尼卡滿是梵文轉寫（atman、Naighantuka），
  拉丁字母多於漢字是正常的；只看比例會從 181 段真陽性誤報成 4,138 段。
- 第 6 條門檻壓到 0.15，是為了放過**原書就重複**的東西（奧義書問答、法句經偈頌、
  目次點漏，實測 0.19–0.33）。
- 第 7 條**分界是位置不是比例**——和歌並列體例的假名佔比 0.26 跟真傷 0.28 貼在
  一起，但並列一律**中譯在前**，沒譯的一開頭就是日文。
- 第 5 條取 1 個商議語會掃到正常英文引文，取 2 個才乾淨（全庫命中 ≥2 的只有 4 段）。
