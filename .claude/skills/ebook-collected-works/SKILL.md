...
> **潘尼卡 2026-07-01 由「神學」改歸「宗教學」（user 拍板）** —— 其宗教學／比較宗教定位重於純系統神學。神學學科暫無作家。
...
**⏳《七篇致亡靈的佈道》德文列待補（2026-07-02 起）**：/gnostic 已有英＋繁中 182 段（doc `seven-sermons-to-the-dead`）。計畫用 REFERENCE 復用：只加德文 1916 原典為第三版（`gnostic_versions` code=de_1916, category=source, is_default_orig=true → reader `pickForCategory('source')` 自動顯示中/英/原文三欄，只影響此 doc）+ 對齊入 `gnostic_sections`（前付 caption 段 0–35 無德文留空，本文段 36–181 對齊）。**尚未完成**：德文全文抓取（fetch agent stalled）；重啟時先取 Septem Sermones 德文全文（冒頭「Die Toten kamen zurück von Jerusalem…」）再逐段對齊。榮格 hub 書目已用 `externalUrl` 連 /gnostic、不重複建 ebook。
