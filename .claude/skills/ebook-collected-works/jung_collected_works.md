...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 132 rows（標題＋E1–E131，全章 ~63%）**——…V128 胡布爾軍團詩、E129 Marduk春神對抗Tiâmat備戰、V130 七風武器詩、E131 主武器=風與網逼近挑單打。**下一批 V132 + E133 + V134 + E135**＝`jung_ch10_en.txt` 的 `[V132]`（決戰詩：Tiâmat 與 Marduk 相遇張網捕之放Imḫullu入其口使唇不能閉狂風灌其腹剖體斷心制其命踏屍，blockquote）＋ E133（殺母後謀劃創世）＋ `[V134]`（創世詩：分剖母體如剖扁魚為二）＋ E135（Marduk由母體創宇宙=殺母龍在「風受孕」+負面伴隨之意象下進行）。皆 de=德文史詩 en=英譯 zh親譯。逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
