...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 80 rows（標題＋E1–E79）**——…E78 力比多藉洗禮昇華、E79 基督教=古代性崇拜的負片/普里阿普斯在聖提孔復活/聖法洛斯。**下一批 E80…**＝`jung_ch10_en.txt` 的 `[E80]`（對德文 `[D163]` 尾「Wer es noch nicht gelernt hat…」起）：有人尚未承認性是與飢餓對等的功能，故視某些禁忌制度被揭露充滿性象徵為羞辱／精神分析把層層複雜化的象徵回溯還原／要克服自身的德性以免落入卑劣（大德性內裡補償著對卑劣的傾向）／隨後 E81 此時大批人開始丟棄基督教值得弄清它當初為何被接受、E82 勿誤以為我要把力比多還原回原始階段而忘卻… 逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
