...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 171 rows（標題＋E1–E170，全章 ~82%）**——…E168 龍=對亂倫抵抗/提什特里亞、E169（超長）蛇纏樹/Heddernheim密特拉四圖=征服獸性牛=犧牲亂倫願望勝死亡之母→十架之神完成贖亞當罪、E170 圖：密特拉殺牛祭。**下一批 E171**＝`jung_ch10_en.txt` 的 `[E171]`（對德文 `[D446]`）：如前所述把祭牲懸於樹乃普遍流傳的儀式習俗(日耳曼例尤多)，儀式含以矛刺穿祭牲→引奧丁(Edda Havamal)詩「我知我曾懸於風搖之樹九夜/被矛所傷獻於奧丁/我自己獻於我自己」(blockquote de=德文 en=英譯 zh親譯)。E171 含古諾斯詩需清 OCR。逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
