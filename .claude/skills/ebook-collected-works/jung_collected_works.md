...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 129 rows（標題＋E1–E128，全章 ~62%）**——…E126 荷魯斯堤豐、E127 與夜蛇之戰=制伏背叛兒子的母親/巴比倫創世史詩Ea制伏Apsû/Tiâmat圖謀復仇、V128 母親胡布爾的怪物軍團詩。**下一批 E129**＝`jung_ch10_en.txt` 的 `[E129]`（對德文 `[D338?]`）：諸神終以春神=勝利太陽 Marduk 對抗提阿瑪特那可怕軍團，Marduk 備戰，其親手所造之主武器(隨後 V130 詩描述其武器/網/風)；接著 Marduk 與 Tiâmat 決戰擒殺剖分母體=創世。E129 含巴比倫名需清 OCR。逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
