...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 91 rows（標題＋E1–E90）**——…E89 奇異針葉樹、E90 樹的象徵史/被懸諸神/基督十架=生命樹兼死亡之木/普魯塔克奧西里斯神話（母胎中亂倫/Pamyles/Typhon入箱/伊西斯造蛇傷日神=亂倫禁忌之傷/可怕母親夢魘）。**下一批 E91 + E92 + E93**＝`jung_ch10_en.txt` 的 `[E91]`（對德文 `[D198]`）：夜海航行完成後奧西里斯之櫃漂至 Byblos 上岸，臥於環棺生長的 Erica（石南）枝中成參天大樹，國王取此樹作宮殿之柱；奧西里斯離去期（冬至）有持續數千年的哀悼亡神之俗，其「尋獲」εὕρεσις 是喜慶之節；引伊西斯哀尋之一段 ＋ `[V92]`（詩「她如燕繞著那困鎖死眠之神的柱子哀鳴」，blockquote）＋ E93（同母題見 Kyffhäuser 傳說）。逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
