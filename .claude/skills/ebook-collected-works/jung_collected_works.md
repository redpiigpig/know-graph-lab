...
- **🚧 第二部第四章「英雄的無意識起源」(The Unconscious Origin of the Hero)**：待建（`ch09.json`）。德文 ZWEITER TEIL IV「Die unbewußte Entstehung des Heros」第 8948 行起 / 英文 HTML `CHAPTER IV` 第 8169 行起（至 9496，Ch V 在 9497）。**⏭️ 續傳指標：尚未開抽**——下一步先寫 `c:/tmp/_jung_extract_ch9.py`（仿 ch8，de 8948→Ch V 德文起點、en HTML 8168:9496），再逐批建 ch09。
- **⏭️ 後續章節**：V 母親與重生的象徵（英文 HTML 9497+）、VI 為脫離母親而戰（11983+）、VII 雙重母親（13148+）、VIII（16444+）。逐章建 `chNN.json`（英文 PART II 區段以 `_jung_extract_chN.py` 重抽 en 含詩節版；德文照英文段落拼接跳腳註）。
