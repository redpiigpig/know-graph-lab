...
- **🚧 第二部第五章「母親與重生的象徵」(Symbols of the Mother and of Rebirth)**：建設中（`ch10.json`，**全書最大章**）。抽取已跑（`c:/tmp/_jung_extract_ch10.py`，輸出 `jung_ch10_de.txt` 538 段 / `jung_ch10_en.txt` 208 entries 含 12 詩節）。德文 ZWEITER TEIL V 第 10731 行起 / 英文 HTML `CHAPTER V` 第 9497 行起（至 11982，Ch VI 在 11983）。**⏭️ 續傳指標：已建 124 rows（標題＋E1–E123，全章 ~60%）**——…E121 nightmare/mare詞源、E122 mar=死→mors/μόρος命運/諾恩=命運/凱爾特fatae→matres、E123 凱撒拉丁文諸母籤卜決開戰。**下一批 E124**＝`jung_ch10_en.txt` 的 `[E124]`（對德文 `[D316]?`）：斯拉夫語 mara=女巫；波蘭 mora=魔/夢魘；瑞士德語 mōr/mōre=母豬(亦罵詞)；波希米亞 mura=夢魘兼「夜蛾/天蛾 Sphinx」——此奇異關聯由分析揭明：有可動殼(維納斯貝)或翅的動物常作女性生殖器象徵；Sphingina 黃昏之蛾如夢魘於暗中來；最後雅典聖橄欖樹稱 μορία(出自μόρος)，Halirrhotios 欲砍此樹卻以斧自戕。E124 含希臘需清 OCR。逐批 append→`python scripts/jung_build_all.py`→commit `[loop]`。
