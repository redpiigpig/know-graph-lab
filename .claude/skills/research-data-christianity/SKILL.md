...
| 台灣福音派 | `evangelical` | 基督教論壇報 `/ct` | 篇目 23,713（**實際可用 2019–2026**），內文 17,588 已上架 |
...
| 工作 | 狀態 | 接續指令 |
|---|---|---|
| 基督教論壇報內文 | 17,588／23,713（74%） | `python -X utf8 scripts/ct_forum.py --process` 反覆下到不再新增，再 `--publish` |
| 臺灣博碩士論文書目 | **1／14 組**（28 筆），其餘卡在站方驗證碼 | 等 IP 冷卻後 `python -X utf8 scripts/thesis_ndltd.py --search`（會跳過已完成的組）；一次跑少數幾組、分時段 |
| 語料層併入論壇報 | **未做** | 論壇報收完後 `python -X utf8 scripts/corpus_terms.py --build` |
