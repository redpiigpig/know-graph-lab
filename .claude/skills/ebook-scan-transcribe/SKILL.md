...
> **2-up 有兩種，工具不同**（2026-09-19 補）：
> - **上下疊**（橫躺的紙，要先轉正）→ `scan_prep.py`，就是下面這一節講的。
> - **左右並排**（書攤平橫拍、正立，左頁右頁並排）→ `scripts/split_two_up_pdf.py`，
>   只要從書溝切開，不必轉。Geertz《Local Knowledge》即此型。
>
> 🚨 **怎麼發現它是 2-up：看每頁字數與頁碼覆蓋率，不是看它有沒有報錯。**
> 那本第一版 OCR 跑完 exit 0、126 頁、空白率 1.6%，一切正常——但每頁 4,913 字
> （單頁學術書正常約 2,500），而印刷頁碼只撿回 16/126，其餘被「跟前後對不上」的閘清掉。
> 原因是一張紙上有**兩個頁碼**，位移一致性永遠過不了。**那個閘是在示警，不是誤殺。**
> 拆頁後：252 頁、頁碼 220/252、位移 +7 且 220 票全數一致、每頁 2,458 字。
>
> 🚨 中線不要直接取寬度一半——攤平拍照時裝訂線未必置中，取中央 ±12% 內最暗的一欄。
> 存檔一律 `garbage=4, clean=True`（理由見下），實測 2.6 MB 拆完仍是 2.6 MB。
