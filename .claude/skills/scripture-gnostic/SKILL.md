...
>       - **⏸️ 2026-06-15 全類別精修＝暫停中，進度 2620/15563（約 17%）＝交給新 session 續跑**（user：除典外文獻外全用此範本修）。對摩尼教以外所有類別逐段重譯成白話和合本體，**唯獨跳過 23 篇 apocrypha_slug 連結篇**（task #2 等黃根春回填，不可動）。新增 `--exclude-apocrypha`（`d.apocrypha_slug IS NULL`）/ `--exclude-category` / **`--resume`（ledger `c:/tmp/gnostic_refine.done` 記已完成 `slug#oi`，死掉/quota 牆後續跑不從頭）**＋自癒迴圈 [scripts/gnostic_refine_loop.sh](../../../scripts/gnostic_refine_loop.sh)。
>          - **▶️ 新 session 續跑指令**：`bash scripts/gnostic_refine_loop.sh`（會自動 `--resume` 從 ledger 2620 接著跑剩 ~12,943 段）。或單輪：`python -X utf8 scripts/fix_gnostic_quality.py --retranslate --exclude-apocrypha --exclude-category manichaean --resume --engine gemini`。
...
>          - **🚨 ledger 別刪**：`c:/tmp/gnostic_refine.done`（2620 行）是續跑命脈，**勿清**（同 [[feedback_tmp_cleanup]] c:/tmp ledger 保留規則）。`gnostic_refine.lock` 若殘留先刪再啟。
