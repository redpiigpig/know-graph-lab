...
>       - **⏸️ 2026-06-15 全類別精修＝暫停中，進度 3033/15563（約 19.5%）＝交給新 session 續跑**（user：除典外文獻外全用此範本修）。對摩尼教以外所有類別逐段重譯成白話和合本體，**唯獨跳過 23 篇 apocrypha_slug 連結篇**（task #2 等黃根春回填，不可動）。新增 `--exclude-apocrypha`（`d.apocrypha_slug IS NULL`）/ `--exclude-category` / **`--resume`（ledger `c:/tmp/gnostic_refine.done` 記已完成 `slug#oi`，死掉/quota 牆後續跑不從頭）**＋自癒迴圈 [scripts/gnostic_refine_loop.sh](../../../scripts/gnostic_refine_loop.sh)。
>          - **▶️ 新 session 續跑指令**：`bash scripts/gnostic_refine_loop.sh`（會自動 `--resume` 從 ledger 3033 接著跑剩 ~12,530 段）。或單輪：`python -X utf8 scripts/fix_gnostic_quality.py --retranslate --exclude-apocrypha --exclude-category manichaean --resume --engine gemini`。
>          - **🚨 為何暫停**：2026-06-15 下午**三池全飽和**（Gemini 4 key + Haiku Max 全 429，NVIDIA 也僅 ~1 段/分鐘）——當晚其他並行任務把配額搶光，硬跑只互相搶爆。**換引擎治標無效（瓶頸是配額爭用）**。等其他任務結束/配額重置（隔天）再續，throughput 才正常。
>          - **🚨 ledger 別刪**：`c:/tmp/gnostic_refine.done`（3033 行）是續跑命脈，**勿清**（同 [[feedback_tmp_cleanup]] c:/tmp ledger 保留規則）。`gnostic_refine.lock` 若殘留先刪再啟。
