...
>       - **⏳ 2026-06-15 摩尼教精修跑中** — `fix_gnostic_quality.py --category manichaean --retranslate --engine haiku`（新增 `--category` + `--retranslate`：精修＝全類重譯非只修 flagged）。摩尼教 1404 段（原本 0 flagged＝沒壞，純為 register 升級）逐段重譯成白話和合本體。Gemini 三池當日全 429，走 haiku/Max。⚠️ **harness 雙啟動陷阱**：piped/backgrounded 指令的 python 是 `_whisper_venv`→`uv` 的**父子 re-exec**（看似 2 個 process 其實 1 個邏輯 worker，別誤殺）；要停用 TaskStop + 確認 commandline。
>    - 其餘類別 register 仍可同法精修（`--category <key> --retranslate`）。新增變體加進 `fix_gnostic_names.py` 的 `NAME_FIXES`。重譯前可先 `python scripts/export_glossary_from_db.py` 同步詞庫。
