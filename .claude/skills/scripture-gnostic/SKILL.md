...
>       - **✅ 2026-06-15 品質閘內建進 pipeline（根因修復，第三輪）** — user 問「為何之前品質堪憂」的正解：**有測試但測錯東西**。`assert_aligned` 只驗段數相等（en==zh），幻覺成假 essay 段數照樣相等照樣放行——缺的是 **source↔output 不變量**。把偵測器升級成 [scripts/gnostic_library.py](../../../scripts/gnostic_library.py) 的純函式 `classify_translation()`（+8 例 test，共 33 綠），ingest_gnostic.py 新增 `translate_one()` 把它當 **inline 逐段閘**（翻一段就驗一次→失敗重試 2 次→短結構標記退回 verbatim），fix_gnostic_quality.py 改 import 共用同一定義（不漂移）。**未來任何翻譯都自帶體檢。**
>       - **✅ 2026-06-15 register 定案：白話文 + 《和合本》語氣**（user 拍板）。原 prompt「莊重文體」太含糊→Gemini 漂成文言（「又聞昔有二蛇／心同一念」）。`GNOSTIC_PROMPT_TMPL` 改明令：**一律白話、參照和合本、絕不文言**（禁「曰/其/之/乃」古文虛字；用「他便/看哪/於是/就」）。
>       - **✅ 2026-06-15 摩尼教精修完成（白話和合本範本）** — `fix_gnostic_quality.py --category manichaean --retranslate --engine haiku`（新增 `--category` + `--retranslate`：精修＝全類重譯非只修 flagged）。摩尼教 **1404 段全量重譯成白話和合本體**（原本 0 flagged＝沒壞，純 register 升級；1399 clean + 5 verbatim + 0 err）。殘留 12 段文言漂移以閘重譯，2 段頑固 best-effort 再 re-roll，**最終 0 flagged**。Gemini 三池當日全 429 → haiku/Max。成果例：「他便用計謀，取一隻羊羔設為陷阱，好藉此捕獲獅子…其後他醫治了被獅子所傷的那隻羊羔」。
>       - **✅ 2026-06-20 全類別精修完成（白話和合本，全 corpus 收尾）**（user：除典外文獻外全用此範本修）。摩尼教以外所有類別逐段重譯成白話和合本體完成，**23 篇 apocrypha_slug 連結篇照規定跳過**（task #2 等黃根春回填，未動）。自癒迴圈一輪跑完剩餘 3777 段（clean 3655 / verbatim 108 / err 14），再 4 輪 flagged-only re-roll（73→30→21→18→13→10）清掉文言漂移。**最終 task-scope（excl apocrypha）`--dry` = 7 段 flagged，全是正當 verbatim**（書目／`(Text: R. 354)`／`pp. 51-54 of codex...`／編者註／圖說），**0 wenyan、0 meta_leak**。
>          - **🔧 收尾發現的兩個 gate 根因 bug（已修＋加 test，40 例綠）**：①`classify_translation` 的 `哉[！。]` 是**誤判**——和合本本身就用「禍哉／哀哉／聖哉／深哉」感嘆（全 corpus 4 個 哉 命中全是和合本詞），故從 `WENYAN_RE` **移除 哉**（[scripts/gnostic_library.py](../../../scripts/gnostic_library.py)）。②`is_trivial_source` 的 `page` 裸字**誤吃「Pagels」**（Elaine Pagels，諾斯底名學者，本 corpus 常見開頭）把真正文段當頁碼跳過→改 `page\b`（[scripts/ingest_gnostic.py](../../../scripts/ingest_gnostic.py)）；受害的 `genesis-and-gnosis#2`、`books-of-jeu#11` 已用 `translate_one` 補譯。
>          - **📌 摩尼教（範本）補洞**：full-corpus 掃出摩尼教殘留 **16 段 wenyan**（汝/吾/矣，psalms＋奧古斯丁駁摩尼論；應是 `汝/吾` pronoun marker 在 2026-06-15 摩尼教跑完後才加進 gate 才漏掉），`--category manichaean` flagged-only 一輪全清（16 clean），摩尼教重回 0 flagged。
>          - **🚨 ledger 保留**：`c:/tmp/gnostic_refine.done`（15727 行）**勿清**（同 [[feedback_tmp_cleanup]] c:/tmp ledger 保留規則）。日後若要再全量精修，先刪 ledger 才會從頭。`gnostic_refine.lock` 若殘留先刪再啟。
>          - **單輪指令備查**：`bash scripts/gnostic_refine_loop.sh`（預設 haiku/Max，牆了自癒睡 30min）；flagged-only re-roll：`python -X utf8 scripts/fix_gnostic_quality.py --exclude-apocrypha --exclude-category manichaean --engine haiku`；驗收：同上加 `--dry`。瓶頸一向是配額爭用非引擎選擇。
>    - ⚠️ **harness 雙啟動陷阱**：piped/backgrounded 指令的 python 是 `_whisper_venv`→`uv` 的**父子 re-exec**（看似 2 個 process 其實 1 個邏輯 worker，別誤殺）；停用 TaskStop + PowerShell 殺 fix_gnostic_quality cmdline。新增變體加進 `fix_gnostic_names.py` 的 `NAME_FIXES`。重譯前可先 `python scripts/export_glossary_from_db.py` 同步詞庫。
