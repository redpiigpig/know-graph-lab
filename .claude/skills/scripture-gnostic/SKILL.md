...
>       - **⏳ 2026-06-15 全類別精修跑中（user：除典外文獻外全用此範本修）** — 對摩尼教以外所有類別逐段重譯成白話和合本體，**唯獨跳過 23 篇 apocrypha_slug 連結篇**（task #2 等黃根春回填，不可動）。新增 `--exclude-apocrypha`（`d.apocrypha_slug IS NULL`）/ `--exclude-category` / **`--resume`（ledger `c:/tmp/gnostic_refine.done` 記已完成 slug#oi，死掉/quota 牆後 re-run 不從頭跑）**。指令：`fix_gnostic_quality.py --retranslate --exclude-apocrypha --exclude-category manichaean --resume --engine haiku`（~18k 段，過夜跑）。
>    - ⚠️ **harness 雙啟動陷阱**：piped/backgrounded 指令的 python 是 `_whisper_venv`→`uv` 的**父子 re-exec**（看似 2 個 process 其實 1 個邏輯 worker，別誤殺）；停用 TaskStop。新增變體加進 `fix_gnostic_names.py` 的 `NAME_FIXES`。重譯前可先 `python scripts/export_glossary_from_db.py` 同步詞庫。
