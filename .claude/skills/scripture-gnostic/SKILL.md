...
>       - **⏸️ 2026-06-15 全類別精修＝暫停中，進度 11955/15563（約 77%）＝交給新 session 續跑**（user：除典外文獻外全用此範本修）。對摩尼教以外所有類別逐段重譯成白話和合本體，**唯獨跳過 23 篇 apocrypha_slug 連結篇**（task #2 等黃根春回填，不可動）。新增 `--exclude-apocrypha`（`d.apocrypha_slug IS NULL`）/ `--exclude-category` / **`--resume`（ledger `c:/tmp/gnostic_refine.done` 記已完成 `slug#oi`，死掉/quota 牆後續跑不從頭）**＋自癒迴圈 [scripts/gnostic_refine_loop.sh](../../../scripts/gnostic_refine_loop.sh)。
>          - **▶️ 新 session 續跑指令**：`bash scripts/gnostic_refine_loop.sh`（會自動 `--resume` 從 ledger 11955 接著跑剩 ~3,608 段）。或單輪：`python -X utf8 scripts/fix_gnostic_quality.py --retranslate --exclude-apocrypha --exclude-category manichaean --resume --engine gemini`。
>          - **🚨 進度史 / 為何暫停**：2026-06-15 下午三池全飽和（被並行任務搶光）只 ~1 段/分；傍晚 Haiku Max 配額恢復用 `--engine haiku` 衝到 **~24-36 段/分**，3033→**11955（77%）**，再次撞 **Haiku rolling-window 配額牆**（429 backoff）後 user 換 session 收手。**新 session 開跑前先試哪個池有量**：直接 `bash scripts/gnostic_refine_loop.sh`（預設 haiku，牆了自癒睡 30min）；若想快可把 loop 的 `--engine` 改 gemini（免費池恢復時）。瓶頸一向是配額爭用非引擎選擇。
>          - **🚨 ledger 別刪**：`c:/tmp/gnostic_refine.done`（11955 行）是續跑命脈，**勿清**（同 [[feedback_tmp_cleanup]] c:/tmp ledger 保留規則）。`gnostic_refine.lock` 若殘留先刪再啟。
