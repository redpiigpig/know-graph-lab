...
### 範文／經文頁：`course_slides_quotes*.py`（2026-09-16 起）

使用者 2026-09-16 的回饋是**「現在都只有很多簡介而沒有範例」**——稽核當時三十七份
簡報的原文頁是 **0 張**。所以加了一個 `quote` 版型（`course_slides_pptx.s_quote`）：
標楷體大字、左側金色band、右下出處行、可並列第二個本子（`alt`）。

```
scripts/course_slides_quotes.py       # 掛法與 q()／scripture() 兩個工廠；基督宗教概論（ch）
scripts/course_slides_quotes_wr.py    # 世界宗教文化導論（wr）
scripts/course_slides_quotes_sl.py    # 宗教系國文（sl）
```

三本合起來現有 **ch 54／wr 33／sl 44 條**，渲染後（長段自動分頁）約 180 張原文頁。

**掛法＝錨點。** `QUOTES[(課程, 章)]` 是一串 `(錨點, 投影片)`，錨點是**現有某張投影片
的標題、逐字相符**，原文頁插在它後面；錨點寫 `None` 則擺在該章最後。

🚨 **指名，不做模糊比對，而且對不上要出聲。** `course_slides_weekly.inject_quotes`
錨點對不上直接丟 `KeyError`——跟配圖對照表踩過的是同一個坑，但配圖是靜靜不配，
這裡故意改成炸掉。**改投影片標題就要同步改錨點**（潤飾器 `DATA_FILES` 也要含這三檔）。

🚨 **經文一律 `course_quote_bible.bible()` 從版本庫取，不手抄、不靠背誦。**
和合本修訂版的詩體經文在語料裡是殘的（詩篇 88.5%、箴言 91%），`bible()` 逐節比對
和合本長度，疑似截斷就**自動退回和合本並在出處行標明版本**——看到某一頁寫「（和合本）」
而不是「（和合本修訂版）」，那是閘在工作，不是錯，別去改它。

🚨 **原文頁整張跳過改寫器。** `_walk` 看到 `quote` 直接回傳：引文裡的「本書」「第十四章」
是**原文的用字**，照 `BOOK_SELF`／`CH_REF` 改下去就是竄改材料，而且改完看起來完全正常。
代價是**原文頁的標題也不會被清章號**，所以自己寫的時候就不准寫章號。

🚨 **原文頁不計入張數上限**（使用者 2026-09-16 定）。`build_course` 數的是
`s[0] != 'quote'`。讀原文的節奏與講述不同：一張原文頁停留很久，但不佔講述的份量；
照舊全算的話，加了原文就會把講述內容擠掉，正好與「要有範文」的用意相反。

#### 分頁與版面的兩條算式

- `split_quotes` 在 `raw < QUOTE_SPLIT_AT`（0.68）時對半拆，標題加「（續）」，最多八層。
- **有 `alt` 的頁不拆**——並列的兩段必須留在同一頁才對得起來。所以帶 `alt` 的頁只能縮字，
  而 `QUOTE_FLOOR` 夾在 0.60，裝不下就會溢出版面。
  🚨 **補救法是把要講的點寫進標題、不另掛 `sub`**：副標會吃掉一段標題區高度，
  拿掉它 raw 從 0.57 回到 0.63。ch1 的唱和詩頁與 ch4 的心經對照頁都是這樣救回來的，
  **別再把 sub 加回去**。

驗算（改完一定要跑，純函式、不必出 pptx）：

```python
import course_slides_pptx as R
from course_slides_quotes import QUOTES        # 🚨 先 import 這支，不然循環 import
for (c, ch), items in QUOTES.items():
    for _anchor, qt in items:
        for p in R.split_quotes([qt]):
            kw = p[3]
            raw = R.quote_fit(p[1], p[2], kw.get('alt'), kw.get('sub'), kw.get('source'))[0]
            assert raw >= R.QUOTE_FLOOR, (c, ch, p[1], raw)
```
...
| 課程 | 放什麼 | 怎麼放 |
|---|---|---|
| 基督宗教概論 | 福音書、信經、教父、宣言 | 和合本修訂版為主，關鍵處並列思高本 |
| 世界宗教文化導論 | 各教經典**整段**中譯＋建立這門學科的人自己的話 | 每章 2–3 段，**不要只放一兩句** |
| 宗教系國文 | 講義 `chNN.html` 的 `<blockquote>` | 🚨 **只放原文，不附語譯、不附注釋**，全靠課堂講 |

🚨 國文那本的材料**一律取自講義的 blockquote**（原典選讀與作品檔案），不另外找文本、
不憑記憶抄；節錄處用「……」標明，出處行寫清楚是全文還是節錄。用得到的章＝PPA066
七次上課涵蓋的 1,2,3,4,5,8,9,10,11,12,13。

越南《金雲翹傳》開篇（國語字拉丁轉寫）已收在第 13 章——2026-09-17 讓 `s_quote`
支援逐行換字型之後才放得進去，作法見下面〈拉丁轉寫的行要換字型〉。


### 拉丁轉寫的行要換字型：`is_latin_line()` ＋ `LATIN = 'Noto Serif'`

原文頁寫死標楷體，而**標楷體沒有越南文的附加符號**（ơ ư ạ ề），也沒有 Ā 這類
轉寫用字。整行照 KAI 排會被字型回退切得七零八落——單字卡那邊踩過同一個坑
（`Āgama` 印成 `Ā gama`，見 [[feedback_ui_no_text_overflow]]）。

`s_quote` 現在**逐行**判斷：`is_latin_line(ln)` 為真就改用 `LATIN`（Noto Serif，
這台有裝，單字卡本來就在用），主文與 `alt` 兩邊都吃。

🚨 **只有整行純拉丁才換。** 中西混排的行不換——換了會讓中文變成回退字型，
   那是把一個字型問題換成另一個，而且更難看出來。
   判準是「沒有任何 CJK 字元，且拉丁字母至少三個」，所以
   「Samhain 是今日萬聖節的來源」不會被換掉。

🚨 **估算也要跟著改**：拉丁字母約半個中文字寬，`text_height()` 對拉丁行把
   每行字數加倍。不折半的話，整行轉寫會被高估成兩倍行數，字白白縮小一半。

目前用到的只有國文第 13 章的《金雲翹傳》開篇（國語字轉寫）。臺語白話字、
梵巴轉寫日後要上投影片時走同一條路，不必再改渲染器。

### 🚨 高度算式只有一份：`text_height()`

`fit()` 與 `course_slides_fitcheck.py` 都呼叫 `course_slides_pptx.text_height()`。
**誰都不准自己再抄一份。**

2026-09-17 我寫 fitcheck 時，圖方便把高度算式抄了第二份進去當「獨立的第二意見」，
結果加了「拉丁行折半」之後只改了 `fit()` 那一份，fitcheck 立刻把一頁明明裝得下的
越南文轉寫報成「超出 122pt」。**誤報比漏報更費事**——我差點回頭去改那頁的內容。

這與 skill 裡其他幾條同源：`img_layout()` 是文字欄寬的唯一來源、
`body_h()` 是拆頁判斷的唯一來源、`BULLET_SZ` 是字級的唯一來源。
版面上凡是「估的」與「畫的」要一致的地方，一律共用同一支函式，不要複製貼上。
...
> 基督宗教概論第二週不去介紹耶穌生平故事，卻去介紹歷史耶穌的學術探究，意義不大。
> 要先多介紹第一序的基督教在信什麼，第二序的後來的反省可以相對少一點；
> 世界宗教文化導論再來介紹學術演變。
> 這些問題也要寫回講義中。假設讀者雖有大學程度但對基督宗教不熟悉。

**分工**：基督宗教概論＝第一序（這個宗教自己在信什麼、做什麼）；
世界宗教文化導論＝第二序（學術演變、定義譜系、方法論）。國文則是「先工具後文本」。

2026-09-17 照這條改掉的三處（**改回去之前先問**）：

- **CH 第 2 週〈拿撒勒人耶穌〉**：原本 19 張內容頁全是史料批判、對觀福音問題、Q 假說、
  研究三波，沒有一張在講他做了什麼。改成**十五個場景的生平敘事**（領報→誕生→受洗→
  試探→呼召→神的國→比喻→醫病→變貌→榮入→晚餐→被捕→釘死→空墳墓→升天），
  每一幕配一幅名畫與一段福音書；第二序壓成最後三張（史料是什麼／對觀與三波／
  史學能說到哪裡）。
- **CH 第 6 週〈聖經（下）〉**：原本 20 張全在講「學者怎麼讀」。前面補四張第一序——
  文體不是一種、經課表與主日讀經的流程、每日讀經／查經班／時辰祈禱／背經。
- **講義同步**：`christianity-intro/ch02`（整章搬序＋新增〈一、生平的骨架〉）、
  `ch05`（新增〈一、先看基督徒實際上怎麼用這本書〉）、
  `sinographic-literature/ch01`（體例那三節與原典選讀原本卡在內容中間，移到最後）。

🚨 **搬講義的節就會打亂註號**——`lecture_footnote_check.py` 會抓到
（ch02 當時報 `[12..18, 1..11]`），跑 `lecture_footnote_renumber.py <檔>` 收尾。
兩支都要跑，而且 `PYTHONIOENCODING=utf-8`，否則 check 印到中文檔名就 traceback。

⚠ `course_schedule.py` 裡 BBE150 第 2 週的單元標題仍是「拿撒勒人耶穌：史料、處境與教導」。
**沒有改**——那份表同時餵發出去的修課須知與送學校的教學大綱，改了就與紙本分岔
（同 PPA066 9/26 那筆已知的不一致）。要改得連紙本一起決定。


### 耶穌生平十二幅：`course_slide_images_ch.EXACT_CH`

🚨 **名畫一定走 EXACT 指名，不走搜尋評分。**「Nativity」「Baptism」這種詞在 Commons
有成千上萬張，評分挑出來的多半是十九世紀的三流複製畫或現代教堂彩窗。

已收（全部 Public domain，2026-09-17 逐張下載驗證）：領報（Fra Angelico）、誕生／榮入／
被捕／升天（Giotto 斯克羅威尼四幅）、受洗（Piero della Francesca）、試探（Duccio）、
呼召（Caravaggio）、寶訓（Bloch）、醫病（Rembrandt 百古爾登版畫）、變貌（Raphael）、
晚餐（Leonardo）。釘十架與空墳墓沿用既有的 `crucifixion-icon`／`empty-tomb-icon`。

🚨 **新 key 要同時寫進 `IMAGES_CH` 與 `EXACT_CH`**：抓圖的迴圈跑的是 `IMAGES`，
只寫 `EXACT` 那一份的話該張永遠不會被抓，而且不報錯。`IMAGES` 那邊的查詢詞只是佔位。

🚨 升天那一張查了五個猜測檔名全部 `None`，改用 `list=search` 才找到
`File:Giotto - Scrovegni - -38- - Ascension.jpg`。**`exact()` 回 None 分不出
「檔案不存在」與「授權不合格」**，所以拿不準的先跑一次搜尋再指名。
Commons 查太快會 429，逐張之間 `time.sleep(3)`。
...
2026-09-17 大批重出後對齊：複製 247 檔，收斂到「待複製 0、相同 700」；
三門課的簡報兩邊逐檔 sha256 全同（38／26／10 個檔，含 pptx 與 pdf）。

### 🚨 跑 `--apply` 之前先看「會覆蓋」與「反方向」那兩組

`--apply` 是無差別雙向複製，會照做清單上的每一筆。真正該先看的不是總數，
而是這兩組（`簡報\圖片\` 那一堆缺檔不必逐條看）：

1. **鏡射→課堂「較新」**＝會蓋掉課堂夾的舊版。2026-09-17 這批裡夾著三份
   **教學大綱 docx**，不是當輪動過的東西；覆蓋前要確認使用者近期沒在課堂夾手改過。
2. **課堂→鏡射「缺檔」**＝把課堂夾獨有的東西推進鏡射夾。這裡最容易出事：
   - 該輪抓到一份 `…四項核心傳統 (1).pptx`——**帶「(1)」的重複下載檔**，
     內容是已經被取代的舊標題版。正本剛刪掉，這份一同步就把舊版又帶回去。
     **同步前先掃一次 `*(1)*`、`* - 複製*` 這類重複檔。**
   - **調課單 docx／pdf** 也在這一組。按本檔〈點名表〉那條規矩，調課單與點名表、
     修課須知同屬「課堂紙本」，本來只該留課堂夾；但這支腳本不分類，照樣推過去。
     使用者 2026-09-17 認可這樣（他要的是兩邊一致），只是要知道會這樣。

🚨 **使用者開著的 pptx 不影響同步**：同步是讀鏡射夾、寫課堂夾，而他開的是鏡射夾
那一份，只讀不寫，所以鎖檔擋不到。**但重出簡報會被擋**（見〈出 PDF 之前先把
Drive 上開著的檔關掉〉）——兩件事不一樣，別搞混。
...
**「文字掉出版面」不是一個問題，是六個**，每次都要先**量 PDF** 再改，
不要看到數字沒降就繼續調同一個參數：
...
| **放大時折行沒重算** | 條目少的頁被放到 45pt，每條折成兩行、最後一行掉出版面 | `fit()` 的 `height_at(k)`＋二分搜尋（2026-09-17，見上節） |
...
### 🚨 放大字級（GROW）會自己製造折行——估算式必須重算，不能乘上去

2026-09-17 稽核 PDF 抓到八份的「上次講到哪裡」那一頁溢出版面。
**根因不是條目太長，是 `fit()` 的算式在放大這一側是錯的。**

`fit()` 用**基準字級**算每行塞得下幾個字，再假設「高度與縮放係數成正比」。
縮小時這是對的（字變小 → 折行只會變少 → 估算偏保守）；但 2026-09-16 開放
放大之後，同一條條目在 45pt 下每行少塞兩成的字，本來一行的會變兩行——
**高度是跳著長的**。估算沒看到那些多出來的行，最後一行就落在 y=541.7，
而版面只有 540pt。

2026-09-16 加的 `GROW_MARGIN` 擋不住：它乘的是同一個算錯的數字。

**改法**（`fit()` 內的 `height_at(k)` ＋ 二分搜尋）：放大時**每試一個字級就用
那個字級真正的折行數重算一次高度**，取仍然裝得下的最大值。
只會讓放大後的字變小或不變，所以不可能生出新的溢出；`raw=True` 那條路沒動，
**拆頁判斷與張數完全不受影響**。

🚨 **這一類毛病不要用「刪短條目」去修。** 我第一輪照舊規矩把九條條目刪短，
結果第 6 週的症狀從「壓到頁尾」變成「溢出版面」——因為條目一短，GROW 就把字
放得更大，折行照樣發生。刪短只是把同一個 bug 推到另一個字級上，
而且白白損失內容（「所有宗教都要回答的那個問題」被我改成「生死那個問題」）。
**先判斷那一頁的 k 是大於 1 還是等於 1**：k>1 是放大路徑的估算問題，
k≈1 才是真的內容太多、該刪短。

### `course_slides_fitcheck.py`：出 PDF 之前先掃一遍

```
python scripts/course_slides_fitcheck.py            # 四門全掃，幾秒
python scripts/course_slides_fitcheck.py christianity
```

拿渲染器自己的估算式跑過每一頁（含 quote 與 imgbullets，文字欄寬走同一支
`img_layout`），報出「估算上就裝不下」的頁。

🚨 **它不取代 `course_slides_audit.py`。** 那支量的是 PDF——權威，但要先跑
...
## 2026-09-17 這一輪的三個操作陷阱

### 🚨 `course_slides_weekly.py <課程> N` 的 N 是「第幾個單元」，不是週次

`units()` 跳過演講與參訪那幾週，所以基督宗教概論傳 6 出到的是**第 7 週**那一份，
而且完全不報錯。要單出某一週之前先跑 `python scripts/course_schedule.py` 對一次，
或乾脆整門重出。

### 🚨 出 PDF 之前先把 Drive 上開著的檔關掉

`~$XXX.pptx` 是 PowerPoint 的鎖檔。掃 pptx 的腳本照樣會 glob 到它，
`python-pptx` 開它會丟 `PackageNotFoundError`，整支腳本中斷——
看起來像「簡報壞了」，其實只是使用者正開著那一份。
**掃 Drive 的簡報資料夾一律先 `if name.startswith('~$'): continue`。**
不要去砍那個 PowerPoint 行程（同 [[feedback_no_kill_other_tasks]]）。

`office_to_pdf.py` 走 Windows COM，一份約三十秒，四十八份要半小時以上；
**兩個資料夾平行跑不會比較快**（COM 只有一個 PowerPoint 實例，反而互相卡）。

### 🚨 Drive 的簡報夾裡還留著舊世代的檔

`第N次_第X-Y章.pptx`（2026-09-04／09-05 那批）是**週次化之前**的舊產物，
檔名本身就帶章號，而且內容與現行的 `{代號}_第N週_{標題}.pptx` 不同步。
另外 `BBE150_第1週_什麼是基督宗教：四項核心傳統.pptx`（09-09）也是舊標題版，
已被 `…：耶穌、定義與.pptx` 取代。**上課前抽錯檔就會投出帶章號的舊版。**
要不要刪由使用者決定，腳本不主動刪 Drive 上的東西。


## 🚨 稽核器的 n-gram 比對會誤報「正解沒講到」

2026-09-17 重排基督宗教概論第 2 週之後，拿「正解片語有沒有落在簡報裡」重跑一次，
十題中報了五題落空。逐條查下去**五題全部是誤報**——概念都在簡報上，
只是選項的措辭與投影片的措辭不同（選項寫「同時『已經開始』與『尚未完成』」，
投影片寫成兩條「已經開始——…／尚未完成——…」）。

所以這個稽核的正確用法是：**n-gram 那一關只拿來產生候選，最後一定要逐條用關鍵詞
確認一次**（`'難堪判準' in deck`、`'塔西陀' in deck`…）。
把 n-gram 的結果直接當結論，就會去改本來沒問題的題目。
這與 [[feedback_silent_zero_is_a_bug]] 是一體兩面：稽核回「有問題」跟回「零問題」
一樣都要先驗稽核本身。
