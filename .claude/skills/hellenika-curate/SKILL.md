...
希臘羅馬宗教文獻是一個**已經停止生產、且被學界徹底編目**的語料庫：史詩循環就那七篇、荷馬詩頌就 33 首、俄耳甫斯詩頌就 87 首、赫密士文集就 18 篇、埃庇道洛斯治癒銘文就那四石。
...
- ❌ **百科式開頭**：「《神譜》是古希臘詩人赫西奧德所作的一部長詩，成書於……」——書目欄位已經寫了作者年代，不要重複
...
工具：`scripts/hellenika_intro.py`（單卷 `--volume A`／全書 `--all`／看進度 `--status`），
驅動器 `scripts/hellenika_overnight.py` 可整夜輪跑並自動 commit。
產物寫進 `data/hellenika/intros.json` 覆蓋層（鍵＝`canon:volume:division:index`），
由 `data/hellenika/index.ts` 載入時掛回 work 物件——**批次不改寫 greek.ts／roman.ts 的物件
字面量**，diff 乾淨、可整批回退，且手寫在 .ts 裡的 intro 永遠優先不被蓋掉。

Prompt 必須帶：該卷 `summary`、該部 `label`、條目全部欄位、四要件與禁忌、以及**同卷已寫好的
兩三筆作為 few-shot**。逐卷跑而非全書一次跑——卷的功能位置是第二要件的依據，跨卷混在一起會寫散。

### 🚨 防編造閘（本管線最重要的一道，2026-08-19 實測後加）

**LLM 會編造。** 首輪試產把阿庫西勞《譜系》寫成「以風、天、地解釋原始諸神」「刪去閹割、吞子等
細節」「現存約四十則殘篇」——三件事欄位全都沒給，全是憑印象生成的，而且看起來非常像真的。
在一部工具書裡，這種錯誤比空白危險得多。

因此第 3 要件（具體細節）**改為只能取自欄位已給的東西**（規模、年代、地點、載體、轉引來源、
note 提到的事）。欄位不夠寫，就把篇幅移到第 2 與第 4 要件，寧可平實不可編造。

腳本另設機械閘 `fabrication()`：
- 文中出現的**每個阿拉伯數字**都必須在該條目欄位裡找得到，否則整筆退回
- 文中出現的**每段「」引語**都必須在欄位裡找得到，否則整筆退回
- 溫度壓到 0.25

退回率高是正常的，退回遠好過寫錯。批次產出仍屬**草稿**，需人工複審——staging 留在
`c:/tmp/hellenika/staging/{卷}.jsonl` 作為複審軌跡。
...
| 卷 | 該有的骨架 | 權威參考書 |
|---|---|---|
| Α 神譜 | 赫西奧德本文＋四系異版（俄耳甫斯三種神譜、費瑞居德、阿庫西勞、斯多噶寓意） | West, *The Orphic Poems*；Betegh, *The Derveni Papyrus* |
...
- `era`：成書／定型年代。層積文本（西比拉神諭、魔法紙草、赫密士文集、俄耳甫斯詩頌）按**現存文本定型年代**歸位，材料更早者在 `intro` 註明。
...
✅ **`intro` 250/250 全數完成**，三十卷無缺。

最後兩筆是手寫的，**批次寫不出來是對的不是失敗**，這兩個個案值得記住：

- **巴克基利得斯《祝祭歌與酒神頌》**：欄位只有作者、年代、語言與一行 `note`，
  任何額外細節都會被防編造閘擋下——閘正是該這樣。欄位太稀的條目本來就該人工寫。
- **敘馬庫斯《第三號陳情》**（Ψ 卷）：本來就是互見樁（正文在羅馬卷 VI），簡介該寫
  的是「為何這一卷也列一筆」，批次沒有這個脈絡。**互見樁一律人工寫。**

### 下一輪策展的入口

書目本身尚未做過完整性核對——目前 250 種是初建時列的，還沒拿 §2 的權威參考書
逐卷對表補漏。條目數偏少而參考書覆蓋充分的幾卷可以先做：

`Μ 分族記`（4 條）、`Ξ 聖所志`（4 條）、`Ι 遠征記`（4 條）、`Σ 虛空篇`（6 條）、
`Η 遷徙與建城`（6 條）。`Ο 詩頌集` 的希臘化與帝國期兩段偏薄，Furley–Bremer
兩卷本裡還有可收的祭典聖詩。

補完新條目後記得跑 `python scripts/hellenika_intro.py --all` 補它們的簡介，
並跑 `python scripts/hellenika_glossary.py --check` 確認譯名沒有脫離詞庫。
