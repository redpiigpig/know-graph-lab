...
**E. layout 問題（已知）**：
- 撒羅馬（亞拿之第三夫）在 catholic mode 不渲染 — 因 spine 只解析「主幹配偶」（約亞敬→亞拿），不會遞迴 亞拿's 其他配偶。需要在 [BiblicalSpineTree.vue](../../../components/genealogy/BiblicalSpineTree.vue) 的 `placeOne` 加 cross-spouse 處理。
- 蘇比（亞拿之姊）不渲染 — 因為「兄妹」關係在現有 layout 沒有專門 logic（蘇比只是亞拿的姊妹，沒有父母 anchor）。可能要加 sibling-via-shared-parent 處理 OR 直接把 蘇比 anchor 在某個 placeholder。
- 革羅罷（亞拿之第二夫）的 Y 軸放錯（顯示「第 73 代」label 但 Y 對齊到 J74）— 因為他是被當成「kid 馬利亞-革羅罷 的 kidWife」，kidWife 永遠放在 kid 同列。要修 [BiblicalSpineTree.vue](../../../components/genealogy/BiblicalSpineTree.vue) 第 1010 行附近 marY 算法，依 wife.generationNum 取真實 Y。
