...
## 🟢 當前狀態快照（2026-05-14，commit `d93894b`）
...
- **視角機制**：4 個 view，全域 query param `?view=protestant|early_consensus|orthodox|catholic`，預設 protestant
  ...
  - 規則：累進納入更多傳統人物 + JSONB 套用
    - protestant: biblical + rabbinic
    - early_consensus: + early_consensus 人物 + orthodox JSONB（Epiphanian view，Jerome 393 前主流）
    - orthodox: + orthodox 人物 + orthodox JSONB
    - catholic: + early_consensus + catholic 人物 + catholic JSONB
...
- **線條顏色**：只有「夫妻間橫向婚姻線」紅色；婚姻線→T-bar drop / T-bar / T-bar→子女 drop 全部灰色（=亞當夏娃示例語意）
...
### ⚠️ 已知未解 / 待驗收
1. **截圖腳本 timeout** — `node scripts/biblical-shot.mjs` 一直 `waitForSelector('.node-card') Timeout 15000ms`，但 user 瀏覽器看圖正常、API 直連正常。可能是 auth race 或 dev server reload race。下次 session 接手前先：
   - 確認 dev server 在哪個 port（`tail c:/tmp/devserver.log`）；可能改在 3002
   - 重啟 dev server 確認 clean
   - 試 `$env:APP_BASE='http://localhost:3002'; node scripts/biblical-shot.mjs ...`
