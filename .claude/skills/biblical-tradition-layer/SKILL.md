...
## 🧷 Toggle 展開／收起 — tri-state（不要再用單 set！）

`expandedClans` / `collapsedClans` 兩個 ref **互斥**維護 user 的 explicit 操作；auto-expand 規則 (subtree ≤20) 是 fallback。

```ts
// 渲染時判斷
const expanded = collapsedClans.has(kid)
  ? false
  : (expandedClans.has(kid) || autoExpand)
```

```ts
// toggle handler (button @click)
toggleExpand(id, currentlyExpanded) {
  if (currentlyExpanded) { exp.delete(id); col.add(id) }   // 收起
  else                   { col.delete(id); exp.add(id) }   // 展開
}
```

**過去的 bug**（commit a27746f 修）：原本 toggleExpand 只翻一個 set，當 user 按 auto-expanded 小 clan 的 ▲ 想收起，反而把它加進 expandedClans → 觸發 occlusion bbox（line 1316）→ 把鄰近 spine 後代誤遮（按 該隱 ▲ → 塞特 spine 那段消失）。

**Occlusion 規則**（line 1316）：`expandedClans.has(kid) && subtree.length > 20` — 只有手動展開大 clan 才 occlude。小 clan 即使手動 re-expand 也不 occlude，因為要跟鄰近 spine 共存（per 斯多蘭→{蘇比/以利沙白/施洗約翰} 與 拿鶴 設計）。
