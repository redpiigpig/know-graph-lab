...
- 三卷現況 **M1 8章（愛的萬物論，v2.1）/ M2 14章（虛構的烏托邦，時間軸版）/ M3 9章（人子）**，manifest 同步。**book id A/B/C→A1/A2/A3→最終 M1/M2/M3**（倫理=Morality，前綴與 E/O/V/B 一致；2026-06-22 定）。組裝一律 `python scripts/assemble_genesis_book.py ethics M1 8 M2 14 M3 9`（改 draft 後重跑即重組部署；draft 目錄已改名 `c:/tmp/genesis_ethics/draft/{M1,M2,M3}/`）。⚠️ 治理文件 intro_schedule §3 仍以 A/B/C 當「卷內章」簡寫（A=愛的萬物論、B=虛構的烏托邦、C=人子），對應 draft 目錄 M1/M2/M3。
