...
|---|---|---|---|---|
| 赫西俄德《神譜》 | Α | 1 | 53 | ✅ 53 |
| 赫西俄德《工作與時日》 | Β | 1 | 44 | ✅ 44 |
| 荷馬詩頌 第 1–33 首 | Ο | 33 | 138 | ✅ 138 |
| 《俄耳甫斯讚歌》序詩＋87 首 | Ο | 88 | 88 | ✅ 88 |
| **《伊利亞特》廿四卷** | **Γ** | **24** | **797** | ⏳ **0** |
| **《奧德賽》廿四卷** | **Δ** | **24** | **618** | ⏳ **0** |

合計 **171 檔／1,738 段／33,000 餘行**，其中 **323 段有繁中**。
（此表的數字可隨時用 `python scripts/hellenika_text_align.py --status` 核對。）
...
**唯一的待辦是史詩中譯 1,415 段。** 取源、對齊、reader、書目連結、定名都已完工，
不必重做；動手前只要跑一次 `--status` 確認數字仍是 323/1738。

```bash
# 1. 先看一卷，肉眼確認再放大（一卷約 25–45 段，BATCH=3 故約 8–15 次呼叫）
HELLENIKA_ENGINE=haiku python scripts/hellenika_text_align.py --file iliad-01 --limit 1
HELLENIKA_ENGINE=haiku python scripts/hellenika_text_align.py --file iliad-01

# 2. 順著卷次往下。一次一卷，不要 --all（會連 24+24 卷一起跑到天亮）
HELLENIKA_ENGINE=haiku python scripts/hellenika_text_align.py --file iliad-02
```

**建議次序**：《奧德賽》卷十一（`odyssey-11`，招魂／冥府，Δ 卷的宗教核心）與
《伊利亞特》卷一、卷六、卷廿四（祈禱、獻祭、赫克托爾贖屍）先譯——這幾卷是本藏經
真正要用的段落，先有它們，Γ／Δ 兩卷的 reader 就已可讀。其餘按卷次補完。

**每跑完一批要驗**（四項，本輪就是靠它們抓出四個靜默錯誤）：長度比落在
0.15–0.60、中間點是「‧」、中譯沒有生出來源沒有的東西、段數與 `--status` 對得上。

⚠️ 史詩的英譯是 **Murray 的散文**，不是詩行對詩行。翻譯 prompt（`PROMPT` 常數）
現行第 1 條寫「這是史詩，不是散文」，指的是**中譯的語體**，別誤讀成英譯是韻文。
...
2. **一首讚歌可能橫跨掃描本兩頁，那頁就有兩個以上 `<div class="poem">`。**
   ...
   > 🚨 **哨兵一開始用 `\x00`，而 lxml 會把 NUL 直接吃掉**——哨兵全數消失，
...
站上詞庫的 `deities` 原本只有六條合用（宙斯、赫拉、波塞頓、雅典娜、阿波羅、
阿瑞斯），其餘全查無。**2026-08-30 user 指示「62 條神名，你自己翻譯」即授權逕行
定名**（一般情況仍依 [[feedback_glossary_ancient_name_priority]] 只列清單不入庫），
62 條已由 `scripts/hellenika_seed_deities.py` 寫進 `deities` 表，逐條理由在該腳本，
依據存檔在 `data/hellenika/glossary-candidates-orphic.md`。語料與詞庫比對 0 條不符。

### 🚨 入庫前務必過一次 name_root

「同一個希臘詞頭在中文必須是同一個字」這條，抓出了本批自己的兩個不一致——
兩個都是我自己寫出來的，而且逐條看都不覺得有問題，**排在一起才看得出來**：

| 希臘詞頭 | 統一作 | 原本錯在哪 |
|---|---|---|
| Ὑ- | **許** | Ὑγεία 寫「許革亞」而 Ὕπνος 寫「修普諾斯」 |
| Πρω-／Προ- | **普羅** | Πρωτεύς 寫「普羅透斯」而 Πρωτόγονος 寫「普洛托格諾斯」 |

作法：把整批依希臘原文的詞頭排序看一遍，不要逐條核。

另有幾條**刻意不共用**的，理由記在候選檔裡，日後別「順手統一」掉：
Τύχη 提喀（機運）不可用「命運」（已配給摩伊賴）、Δίκη 狄刻 vs Δικαιοσύνη
狄開俄緒涅、Ἐρινύες vs Εὐμενίδες、Πλούτων vs Ἅιδης、Τηθύς 忒堤斯 vs Θέτις
忒提斯、Κρόνος 克洛諾斯 vs Χρόνος 時間。

「蓋亞／蓋婭」的長期不一致同日由 user 定調**用蓋亞**，語料 8 處已改齊。
