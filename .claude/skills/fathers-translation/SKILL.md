...
| 🔄 6 | ANF Vol 6 (Gregory Thaumaturgus + Dionysius the Great + Julius Africanus + Anatolius/Minor Writers + Archelaus + Methodius + Arnobius) | 翻譯中 (overnight 5/28→29) | `scripts/_overnight_master.py` Phase 4 在跑（Haiku, 846 chunks）。watchdog `_overnight_vol67_backfill_watchdog.py` 預備好等 marker，自動跑 [`_fix_vol6_volumes.py`](../../../scripts/_fix_vol6_volumes.py)（PREFIX_TO_VOL 72 規則）+ commit |
| 🔄 7 | ANF Vol 7 (Lactantius + Venantius + Asterius + Victorinus + Dionysius + Didache + Apostolic Constitutions + 2 Clement + Nicene Creed + Early Liturgies + Arnobius) | 排隊 (Vol 6 後) | Orchestrator Phase 5 + watchdog 自動接手 [`_fix_vol7_volumes.py`](../../../scripts/_fix_vol7_volumes.py)（含 8 卷使徒憲令、Lactantius 神學原理 7 卷 + 摘要） |
