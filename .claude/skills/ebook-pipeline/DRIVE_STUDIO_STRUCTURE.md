...
> 🚨 **2026-08-27 已重整頂層，這份文件下面的表是遷移當時的計畫，現況以 Drive 根的
> `_網站對應說明.md` 為準。** 現在的頂層是（順序對照首頁卡片分組）：
>
> `電子圖書館/`、`全集/`、`經典對照與註釋/`（原「教父著作」）、`研究資料/`、
> `寫作計畫/`（原「著作」）、`學術活動紀錄/`（原本各自獨立的 學位論文／學術著作／
> 會議論文集／演講 併入）、`語言/`、`照片/`、`讀書會/`、`影片/`、
> `_chunks/`、`_repo-一次性腳本/`。
>
> 資料夾一律中文命名。`電子圖書館`／`照片`／`研究資料`／`_chunks` 這四個名字被
> 幾千處程式碼寫死（照片 9127 處、電子圖書館 4035 處），**不要改名**。
...
## `知識圖工作室\學術活動紀錄\學術著作\`（2026-07-20 建立，對照網站 `/papers`）
