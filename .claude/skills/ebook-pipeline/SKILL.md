...
- **🚨 分批 OCR 的頁碼會整段錯位，而總頁數看起來是對的**（2026-09-12 修）。
  長書送 Gemini 會切成每批 60 頁；原本拿回來後照**位移**編號
  （`pg["page"] = lo + off + 1`），假設模型一定回傳剛好 60 頁。實測它會把
  60 頁的切片吐成 **16 頁或 359 頁** —— 於是整批頁碼寫歪，而**全書總頁數還對得上**
  （527 頁的書報 536 頁），`✓` 綠燈、`parsed_at` 照樣寫下去。
  《世界宗教理念史 卷一》的產物：63 個重複頁碼（p181 有兩份完全不同的內容）、
  198–240 **整段 43 頁消失**。
  現在走 `assign_batch_pages()`：優先信模型自己標的頁號、重覆的丟掉不重編、
  不足 85% 就重試一次，仍不足就**放棄整本**而不是寫出錯位頁碼。
  **驗收一定要查頁碼覆蓋**（相異頁數／重複頁號／連續缺口），不能只看總頁數。
- **🚨 分批路徑曾經沒有 JSON 搶救**：單次路徑有 `json_repair`，分批路徑是裸的
  `json.loads`，於是任何一批吐出非法跳脫字元（`\d` 這種）整本就陣亡。
  兩邊現在共用 `parse_pages_json()`。
...
| | 真實字元正確率 | 中位數 CER | 最差一頁 | 頁碼歸屬 | 12 頁耗時 |
|---|---|---|---|---|---|
| gemini-3.6-flash | 98.83% | 1.98% | 4.46% | ✅ | 123 秒 |
| MinerU 讀 `content_list.json` | 97.28% | 0.65% | 13.17% | 🚨 漂移 | 22 秒 |
| **MinerU 讀 `preproc_blocks`** | **99.64%** | **0.41%** | **0.92%** | ✅ | **22 秒** |

🚨 **只能讀 `middle.json` 的 `preproc_blocks`，不可以用 `content_list.json`。**
MinerU 會把**跨頁的段落合併、整段掛到段落起始的那一頁**。實測 p157 開頭那段 93 字的
古文引文（司馬光論孔氏出妻）被搬到 p156——`content_list.json` 與 `middle.json` 的
`para_blocks` 都中招，只有 `preproc_blocks`（合併前）頁碼正確。
本專案 `page_number` 要撐得起引用，區塊漂到隔壁頁＝註腳看起來正常卻指錯頁，
...
🚨 **Gemini 會在模型內部做簡繁轉換，而且會轉錯專業術語。** 實測《佛教的概念與方法》
12 頁裡，它把「熏習」(vāsanā) **62 次全部轉成「燻習」**（燻是燻肉），
而 `TRAD_FIXES` 沒有這一條，於是直接寫進館藏無從攔截。MinerU 輸出**原文原樣**
（簡體就是簡體），簡繁轉換留在管線裡由 `to_traditional()` 處理 —— `opencc s2tw`
實測「熏习→熏習」是對的，錯了也還有 TRAD_FIXES 可以修。**把轉換留在管線裡、
不要交給不透明的模型**，這一條比字元正確率更值錢。

真實掃描件那組（《佛教的概念與方法》）表面上 Gemini 97.68% > MinerU 96.72%，
...
`MINERU_MODEL_SOURCE` 蓋不掉，要改設定檔本身。ModelScope 源實測只有 30–40 kB/s，
HuggingFace 4.2 MB/s（差 110 倍），用 HF。首次跑要下載模型約 1GB。

### 引擎可用性現況（2026-09-16 實測）

- `gemini-flash-latest`（`DEFAULT_MODEL`）：**7 把 key 全部 503**。
- `gemini-2.5-flash`：key 1–3 可用但額度緊，**key 4–7 回 404「no longer available to
  new users」**，官方要求改用 `gemini-3.6-flash`。
- `gemini-3.6-flash`：可用（key#1 503、key#2 成功）。
