...
### MinerU（本機 GPU）對照結果 —— 準確度贏不了，但快 33 倍

同一份考卷（《孔子大歷史》p150–161，原生數位排版、答案零誤差），RTX 4050 6GB：

| | 真實字元正確率 | 中位數 CER | 最差一頁 | 頁碼歸屬 | 12 頁耗時 |
|---|---|---|---|---|---|
| **gemini-3.6-flash** | **98.83%** | 1.98% | 4.46% | ✅ 正確 | 123 秒 |
| **MinerU 3.4.5 pipeline** | 97.28% | **0.65%** | 13.17% | 🚨 見下 | **22 秒** |

- Gemini 的分數要**抹平「」引號樣式**才看得準：97.95% → 98.83%。它把 `"` 轉成
  `「」`，對繁體排版是正確處理，卻被逐字比對算成錯。真正認錯的字約 0.4%（679 字錯 3 個）。
- MinerU **中位數比 Gemini 好三倍**（0.65% vs 1.98%）、12 頁裡 3 頁 `0.00%`，
  但兩頁爆到 13%。
- 🚨 **MinerU 的爆掉不是認錯字，是版面區塊掛錯頁**：實測它把一段 93 字的古文引文
  （司馬光論孔氏出妻）從 p157 搬到 p156。對本專案這是**最不能接受的一種錯**——
  `page_number` 要撐得起引用，區塊跨頁漂移＝註腳看起來正常卻指錯頁
  （[[feedback_transcribe_page_numbers]] 的「假頁碼比沒有更糟」同一類）。
  而本館語料（教父原典／佛經／古文）滿滿都是引文區塊。

**所以本機引擎的定位是「產能與可用性」，不是「更準」**：0.55 秒/頁、零配額、可整晚跑
（≈6,500 頁/小時，Gemini 約 350 頁/小時且要抽 503／429 的籤）。要拿它上生產線，
**必須先解掉區塊跨頁漂移**，否則只能用在不靠頁碼引用的場合。
...
HuggingFace 4.2 MB/s（差 110 倍），用 HF。
