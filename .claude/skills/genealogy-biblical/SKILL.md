...
description: 聖經族譜的「教會傳統」視角圖層 — 早期教會（橘色，東西方共識）／天主教（紅）／東方教會（黃）／拉比傳統（藍）。包含資料表 schema、人物清單、UI 切換與配色設計（視角 widget／卡片配色／圖例均已上線）、layout 修正紀錄（Task 1-4 均完成；Task 2 CUV2010 校對 2026-07-12 收尾，僅 2 條歧義候選待使用者定奪）。
---
...
## 🧾 2026-07-11 傳統圖層補密（handoff B-3，+16 人）

四個非 biblical 傳統圖層人物補密。補後分佈：early_consensus 5／catholic 8／orthodox 5／rabbinic 98（apocrypha 79 不變），總人數 989。audit 六類檢查全歸零。**機制全部沿用本檔既有規則**：rabbinic／early_consensus 人物永遠顯示 → 直連 spouse/children 欄；catholic/orthodox 衝突用 tradition_hide_children JSONB（API mergeKey 只吃 catholic/orthodox 兩鍵，early_consensus/rabbinic 鍵不會被 merge，別用）。

**early_consensus（+1）**
- 西面（革羅罷之子）gen 64 — 優西比烏《教會史》3.11、3.32、4.22（引黑格西普斯）；耶路撒冷第二任主教。掛雙親直連：革羅罷（約瑟之弟）.children＋馬利亞（革羅罷之妻）.children（母親是 biblical 永遠可見，protestant view 也接得上）。
- ⚠️ 猶大之孫（佐革爾／雅各）**已存在**（「佐革爾（主血親）」「雅各（主血親後裔）」「猶大之子（名不詳）」，tradition=biblical）——不要重複建。

**catholic（+5）**
- 艾梅倫提亞（斯多蘭之妻）gen 72 — 中世紀晚期聖亞拿譜系傳統（Jan van Denemarken《聖亞拿史》約1490）；斯多蘭.spouse 已直連、children 對齊斯多蘭三女。
- 希斯梅麗亞（亞拿之妹）gen 73 — 《黃金傳說》〈聖母誕辰〉章（Hismeria）；children=以利沙白＋以律。**衝突處理**：蘇比（亞拿之姊）.tradition_hide_children[catholic]=以利沙白 → catholic view 施洗約翰外祖母＝希斯梅麗亞、東方 view＝蘇比，兩說並立不打架。
- 以律（希斯梅麗亞之子）gen 74 → 以米念（以律之子）gen 75 → 塞爾瓦提烏斯（以米念之子）gen 76 — 《黃金傳說》Eliud→Eminen→聖 Servatius（馬斯垂克主教，†384）鏈；「以律」沿馬太 1:14 既有音譯。

**orthodox（+1）**
- 撒羅米（約瑟之女）gen 64 — 東正教聖傳＋尼基弗魯斯‧卡利斯圖斯《教會史》2.3；攜香液女（可 15:40）＝約瑟前妻之女。掛 約瑟.children＋撒羅米（約瑟之前妻）.children（沿 亞西亞/呂底亞 同款直連 pattern）。西庇太不在 DB，夫妻關係只記 notes。

**rabbinic（+9）**
- 比提雅（法老之女）gen 26 — 代上 4:18 明文＋米吉拉 13a（米列＝迦勒）＋利未記拉巴 1:3；spouse=迦勒（耶孚尼之子）（互指已補）。
- 約伯（烏斯人）gen 23 — 巴瓦・巴特拉 15b＋創世記拉巴 57:4；spouse=底拿（互指已補）。
- 歌利亞（迦特人）／以實比諾（迦特人）／撒弗（迦特人）／拉哈米（歌利亞之弟）gen 31 — 撒下 21:15-22／代上 20:4-8＋梳妲 42b（俄珥巴＝哈拉法，四巨人之母）；掛 俄珥巴.children。**audit KID_SKIP 已加（俄珥巴, 基連）**——四子非基連所出，不可機械補進基連.children。
- 伊磯倫（摩押王）gen 29 — 拿細珥 23b／何拉約 10b（路得為其子之女）＋桑黑德林 105b；children=路得，中間一代無名、樹上壓縮直連（同 摩押/便亞米 式妥協，notes 有記）。
- 尼哲貝特（耶西之妻）gen 32＋亞達耶（尼哲貝特之父）gen 31 — 巴瓦・巴特拉 91a（大衛之母）；尼哲貝特.children 對齊耶西七子（wife.children 對齊規則）。

**存疑留使用者原記錄**（三項已於 2026-07-12 定奪，見下節）。

## 🧾 2026-07-12 三存疑項定奪（使用者裁決：全做）

1. **艾梅倫提亞 → 保留**（15 世紀晚期聖亞拿譜系傳統，最薄弱一筆；資料維持，正式收錄）。
2. **兩個拉比配偶認同說 → 做（view-gated hardcode）**：仿 rabbinicPC pattern 在 [biblical-graph.get.ts](../../../server/api/genealogy/biblical-graph.get.ts) 加 `rabbinicSpouse`，**只在 `view==='rabbinic'` 動態注入 spouse edge**（DB spouse 欄未動，明文視角零污染）：
   - 迦勒（希斯崙之子）↔ 米利暗（＝以法他/阿祖巴，戶珥之母；Sotah 11b-12a、代上 2:19）
   - 約書亞（嫩之子）↔ 喇合（眾先知之祖；Megillah 14b）
   - edge 帶 `relationKind='rabbinic'`；實測驗收：protestant view 無此二線、rabbinic view 有 ✅。
3. **西面（路加 2）＋三王 → 收**：
   - **西面**＝既有 `西滿（希勒爾之子）`（gen 63 rabbinic，Hillel-Nasi 鏈：哈突→希勒爾長者→西滿→加瑪列一世 已完整）——不建新 row，僅 notes 標注「傳統認同為路加 2:25 抱聖嬰、頌 Nunc dimittis 的西面」。
   - **東方三博士**＝加斯帕/梅爾基奧爾/巴爾塔薩三新 row（tradition=catholic，名出 6c《Excerpta Latina Barbari》）。**2026-07-12 使用者定奪錨定**：掛於基土拉之孫**約珊**（Jokshan，其子示巴/底但為阿拉伯黃金乳香商，對應三博士禮物，賽 60:6）名下，走 `約珊.tradition_children['catholic']`——**只在 catholic view 渲染**。世代設 gen 22（與示巴/底但同列，乾淨群集在約珊下）；notes 明載「非字面世代直系、表『東方諸子後裔』象徵」。實測 catholic view 展開約珊→三博士紫色卡群集顯示 ✅（三博士真實世系無權威文獻，此為象徵性傳統錨定）。
   - 992 人 audit 六類歸零、229 測試綠。
...
**猶大 + 她瑪（創 38）— 利未拉特婚虛線橋（2026-07-08 重設計）**
- DB：她瑪.spouse = `'珥（猶大之子）、俄南（猶大之子）、猶大'`（三任丈夫順序：嫁長子→嫁次子→嫁公公）；珥.spouse = `'她瑪'`、俄南.spouse = `'她瑪'`；她瑪 gen=23（與猶大同代，妻位）
- ⚠️ 舊設計「她瑪畫多張卡 + ♻」已在後續重構中被 `renderedAsSpouseOnSpine` 過濾器擋掉（spine 妻不再於旁支重複渲染）——利未拉特敘事一度隱形。
- **現行設計（BiblicalSpineTree「利未拉特婚虛線橋」段）**：通用偵測「spine 妻的其他丈夫＝spine 人物的兒子」，從妻卡底部畫**紅色虛線**（`dashed: true`，5-4 dash）下到前夫兒子卡頂側（stub 錯開 NW/4 避開他自己的 parent drop、elbow 在兒子列上方 8px 避開母系 T-bar）。不新增卡片、不寫死人名。Legend 有對應虛線圖例。
- 同代前夫（路得的瑪倫、拔示巴的烏利亞）**不走此路**——他們經 extraSpouses/前夫 reorder 直接排進婚姻列（路得—瑪倫—波阿斯）。
...
- ~~Task 2：CUV2010 剩餘章節名字校對~~ ✅ 2026-07-12 候選清單收尾（10 條改名、2 條拒改待定奪，見 Task 2 節）；未校對章節（1 Chr 1/2/7、Num 26、Gen 49、Gen 30、Luke 3）如要續掃需重寫 verify 腳本
...
### Task 2: CUV2010 名字校對 — ✅ 2026-07-12 候選清單收尾完成

已查證的章節：
- 1 Chr 3:5 — **朔法→朔罷** 已 rename ✅
- 1 Kgs 4:11, 4:15 — Solomon's daughters 她法、巴實抹 ✅ inserted
- **Gen 10**: 0 discrepancies（已校對 100% match）
- **Gen 36 / Gen 46 候選清單 — 2026-07-12 逐條驗證後套用**（驗證源＝本地 `G:/我的雲端硬碟/資料/聖經/_verses/gen.json.gz` 等 gz JSON 的 `cuv2010` 欄，逐節直接比對字形；非憑記憶）

**✅ 已改 10 條**（name_zh 本欄＋全表 spouse/children/tradition_* token-boundary 引用同步改）：

| 舊名 | 新名 | CUV2010 出處 | 引用欄同步 |
|---|---|---|---|
| 阿勒完 | 亞勒文 | 創 36:23 | 朔巴（西珥之子）.children |
| 瑪拿哈 | 瑪拿轄 | 創 36:23 | 朔巴（西珥之子）.children |
| 以巴 | 以巴錄 | 創 36:23 | 朔巴（西珥之子）.children |
| 示弗 | 示玻 | 創 36:23（Shepho，朔巴之子） | 朔巴（西珥之子）.children |
| 黑幔 | 希幔 | 創 36:22 | 羅坍.children |
| 亞雅 | 愛亞 | 創 36:24 | 祭便.children |
| 黑慕丹 | 欣但 | 創 36:26 | 底順（西珥之子）.children |
| 以實班 | 伊是班 | 創 36:26 | 底順（西珥之子）.children |
| 比利罕 | 辟罕 | 創 36:27 | 以察.children |
| 書含（但之子） | 戶伸（但之子） | 創 46:23（earliest；民 26:42 作書含；disambiguator 保留） | 但.children |

**⛔ 拒改 2 條（原候選表有誤，待使用者定奪）**：

1. **撒雲 →（原候選「示玻」）**：張冠李戴。DB 撒雲 name_en=**Zaavan、以察之子**（創 36:27），與朔巴之子示弗（Shepho→示玻）是兩個人。CUV2010 創 36:27 作「**撒番**」——若要改應是 撒雲→撒番，但不在核可映射內，留給使用者定奪。
2. **雅述 →（原候選「雅薛」）**：兩個不同的人。DB 雅述 name_en=**Jashub、以薩迦之子**（民 26:24 CUV2010 正作「雅述」；earliest 創 46:13 作「約伯」，會與約伯記主角混淆）；而「雅薛（拿弗他利之子）」（Jahzeel，創 46:24）**已另有 entry**，改名會撞名。維持雅述不動。

改名腳本 pattern（本輪重寫版，已跑過、留 scratchpad 不留 c:\tmp）：token-boundary 匹配（split by `、`）、先長後短、name_zh 精確全名比對（含 disambiguator 整體）、tradition_children/tradition_spouse/tradition_hide_children JSONB 也掃、改完全表覆核 leftover=0。驗收 gate：`python -X utf8 scripts/genealogy_data_audit.py` 六類檢查全歸零 ✅（2026-07-12，973 人）。

**未校對章節**（候選清單之外，如要續掃需重寫 verify 腳本）：1 Chr 1（156 entry，最大）、1 Chr 2、1 Chr 7、Num 26、Gen 49、Gen 30、Luke 3。驗證管道：本地 `_verses/*.json.gz`（schema 見 `server/utils/bible-verses.ts`）比 WebFetch rcuv.hkbs.org.hk 快且穩。

`safe_rename` 規則（沿用）：token-boundary 匹配（split by `、`），只改 `name_zh == old` 或 `name_zh.startswith(old+'（')`，**不動 disambiguator** `（X之子）` 系列（除非父名整體 propagate）。
