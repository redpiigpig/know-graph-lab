...
> 姊妹工具：[maps-world-religions](../maps-world-religions/SKILL.md)（文化圈著色版本）
...
**過濾策略**（多層）：
- **build 階段**：`historical-states.geojson`（給本工具）用 `NON_STATE_PATTERNS` blacklist 排除明顯非政體；`historical-sphere-fills.geojson`（給 maps-world-religions）用 `STATE_NAMES` whitelist
...
- 與 maps-world-religions 整合：分類確定後反向更新 world-religions 的 sphere-history
...
## 與 maps-world-religions 的差別

| 工具 | URL | 主視覺 | 用途 |
|---|---|---|---|
| maps-world-religions | /maps/world-religions | sphere 著色（8 realm × 30 sphere 配色） | 文化／宗教界域呈現 |
| **maps-historical-borders** | **/maps/historical-borders** | 每國唯一色（hash HSL）+ 國家列表 | 純政治體呈現、資料庫查閱 |
