...
> 🔢 **數字寫法（2026-09-25 使用者定調）**：
> - **西元的年、月、日一律用阿拉伯數字**：「1969年2月18日」「1960年代」，不寫「一九六九年」「二月」。
> - **四位數以上的非整數盡量用阿拉伯數字**：「1,382人」「4,500英里」。整數可以照中文習慣寫：「四千年」「兩萬人」。
> - 章名序號（第一章）、詩句、成語與慣用語中的數字（七年、三心二意）照中文。
> - 新譯要寫進翻譯 prompt；已上架的書校對時照此改。
> - **既有譯文批次修正的範圍（2026-09-25 使用者定，比新譯更窄）**：只改「一千兩百三十四」這種三位以上有效數字的四位數以上數字，以及「一千零三十六年」這類年數。**不改**：經文（聖經、佛經等經文本身）、「十九世紀」「六十年代」、兩位有效數字（四千五百、二十五萬）、其他本來就用中文簡潔表達的數字。
>
> 📖 **譯名依脈絡、先收詞再翻（2026-09-25 使用者定）**：
> - 譯名統一**要看脈絡**，不是英中一對一：不同宗教、教派、時代的書各有慣用譯名（天主教：伯多祿、梅瑟；新教：彼得、摩西），**同類脈絡內要統一**。
> - **翻一本書之前，先蒐集該領域的相關詞彙輸入翻譯詞庫**（/translation-glossary），例如修道院用語、日本文化用語、印度文化用語，再開始翻。
> - 人名中間點全站統一用「‧」。
> - 模型拒答或回話、`<think>` 外洩、亂碼字元（U+FFFD）、該譯而未譯的外文段落（含日文殘段），一律**清空後重譯**，不可留在譯文裡。
...
- **《迴旋梯：掙脫幽暗的攀爬之路》（使用者定名）**（Karen Armstrong, *The Spiral Staircase*, 2004）｜`ebook_id: 39e70498-9d27-437c-909a-793a927d52d0`｜2026-09-24 起翻｜**使用者私人參考、寫完《無境界者》書評後刪除**。查過無中譯本（博客來／天下文化書目／中英維基皆無）；原檔是使用者自備的 calibre 轉出 PDF（有文字層），走本次新增的 `pdf_to_chunks`（按書籤分章、校正錯位書籤、尾註上標轉 `[^N]` 並把註文掛回所屬章末、**不寫頁碼**——轉檔 PDF 的頁序不是印刷頁）。14 個 source chunk（讚譽＋序＋題詞＋八章＋三個附件）。lane `spiral-staircase`：`--engine auto --resume --max-piece 6000 --docx-out next-to-source`，全書譯齊＋R2 成功＋Word 讀回驗過才印 `TRANSLATE_BOOK_COMPLETE`。`collection` 刻意留空（cw-word 不會碰它），只在需登入的 `/ebook/[id]` 可讀。
  2026-09-24 完工：lane 只譯到第 3 章（Gemini 一直 429），使用者改派 6 個 agent 親手譯第 4–8 章＋書末；前三章用腳本校對後寫回。全書採**天主教／思高本用語**（天主、聖神、長上、某某姆姆、托辣、梅瑟…，作者當過修女），譯名走共用表統一（Praeterita 是修院院長名不是書名）。另依使用者要求在 Drive 出分章朗讀夾。書目頁的英文原名括注會被 docx 驗證判成 untranslated，只留中文書名。
  **寫完書評後要刪**：①Drive 原檔 `全集\宗教學\凱倫‧阿姆斯壯\Karen Armstrong，The Spiral Staircase My Climb Out of Darkness.pdf`（作家夾留不留到時問使用者）②同夾的 `凱倫·阿姆斯壯，迴旋梯：掙脫幽暗的攀爬之路.docx` ，以及 `G:\我的雲端硬碟\迴旋梯\`（整本 docx 副本＋分章朗讀夾 00–13 共 14 個 docx；使用者 09-25 定：分章檔只放 Drive 首頁這份，全集夾只留整本）③Drive `_chunks\39e70498-….jsonl` ④DB `ebooks` 那一列 ⑤R2 `ebook-chunks/39e70498-….jsonl.gz` ⑥`scripts/fleet_keeper.ps1` 的 `spiral-staircase` lane（連同 `$STALL_PER_LANE` 那行、`scripts/state/fleet_spiral-staircase.*`、`scripts/logs/fleet_spiral-staircase.*`）。
