...
| hongshi  | 🔄 持續上傳中 | 35,927 | 🔄 ~56% 跑中 | 使用者持續從 Drive 網頁`電腦 > 02. 歷年活動照片-69-119` 補上 |
...
**最新進度（2026-05-21 14:22）**：
- File pointer **21,000 / 35,927 ≈ 58%**
- 本 try（try 2）新生成 thumbs：**23,022**（其餘 18,770 從 cache skip + 1 skiplist + 206 持續 err = HEIC plugin 解不開）
- Disk cache 累計 **66,818 thumbs / 6.40 GB**
- 速度（08:38–14:22 ~6 小時觀察）：~32 files/min；Drive 越進到 113-114 年（較近期上傳）越冷越慢
- Try 1 在 9,400 檔時 Drive I/O 中斷 crash（exit -1073741818 = STATUS_IN_PAGE_ERROR），try 2 自動 relaunch 從 8:19 跑到現在
- 剩 14,927 檔，估再 **~8 小時**完成（22:00 左右收尾）
