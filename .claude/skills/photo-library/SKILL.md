...
|---|---|---:|---|---|
| chenwei  | ✅ 完成 (2026-05-19)＋ 159 stragglers 待跑 | 13,022 | ✅ 25,001 thumbs | 2026-05-17 第一輪；2026-05-19 補匯入 594 JPG + 36 MP4（Sony HX99）+ 修 440 iPhone 截圖；2026-05-21 刪 626 個 `.AAE` iPhone sidecar；2026-05-21 audit 發現 3 個事件夾共 159 檔未 rename（hongshi prewarm 完再處理，見「待辦」）|
...
| hongshi  | 🔄 持續上傳中 | 35,927 | 🔄 ~92% 跑中（早上 09:30 左右收尾） | 使用者持續從 Drive 網頁`電腦 > 02. 歷年活動照片-69-119` 補上 |

### 🔄 進行中：hongshi thumb prewarm（重要交接）

> **接手 session 第一件事**：跑下面「快速檢視進度」那塊，確認 (a) loop PID 18652 是否還活著、(b) 最新 try log 結尾是否已寫 `=== Done` — 若是，hongshi prewarm 完成，可進入「待辦 #1」等使用者 ping 上傳。若 loop 死了但沒寫 `=== Done`，看 loop log 是否到達 try 50 上限（exit budget 燒完）或 PowerShell wrapper 異常退出 — 視情況手動重啟 retry-loop wrapper（看 `## Photo thumbnails` 段最下方啟動指令）。
...
**最新進度（2026-05-22 07:57）— 給接手 session**：
- File pointer **33,100 / 35,927 ≈ 92%**
- 本 try（try 17）新生成 thumbs：**10,182**
- Disk cache 累計 **90,378 thumbs / 8.42 GB**
- 速度（早上 06:08–07:38 觀察）：~35 files/min（後段年份 95+ 比較順）
- 剩 **2,827 檔** @ ~35/min → 估再 **~80 分鐘**完成（早上 **~09:30** 收尾）
- Skiplist **316 entries**：原始 15（91-92年 + 93年破檔自動加）+ 300（2026-05-21 21:50 手動預先 preempt 27,200-27,499 段，避開 `93年/930703-930731` 那串爆 STATUS_IN_PAGE_ERROR 的事件夾）+ 1（try 17 自動加）。300 預跳過的檔讀者首次點開該月份要等 cold thumb 生成，可日後手動 re-prewarm 補
- Loop uptime **23.7 h**（08:13 啟動到現在）。歷次 crash & relaunch：
  - try 1（08:13–08:19）：Drive 醒身崩，9,400 檔
  - try 2（08:19–16:23 = 8h）：crash @ 25,500，生 32,014 thumbs ⭐
  - try 3–15（16:23–21:53 = 5.5h）：13 次連環短崩，全卡在 27,200-27,400 死區，skiplist 從 1 累積到 15
  - try 16（21:53–22:05 = 12 min）：終於 break through @ 27,700
  - **try 17（22:05–現在 = 9.5h）：穩定推進 ✅**

**判定完成**：最新 try log 結尾出現 `=== Done` + Loop log 寫 `All done after N tries`。Loop budget 還剩 33/50 次。

**手動 kill**（不該需要）：`Stop-Process -Id 18652 -Force; Get-Process node | Stop-Process -Force`
...
5. **chenwei 159 stragglers**（hongshi prewarm 完成後再跑，避免 Drive I/O 互卡）— audit 2026-05-21 14:00 發現三個事件夾沒被 rename，跑 `python scripts/rename_photos.py plan chenwei` → review → `execute chenwei` 即可一次清掉：
   - `2019相片/法國照片/手機相片/` — 64 檔（HTC `P_20190712_xxxx.jpg`，filename 帶日期）
   - `2019相片/法國照片/相機相片/` — 62 檔（Canon `IMG_xxxx.JPG`，靠 EXIF）
   - `2016相片/03.19美女的圖片東西區/新增資料夾/` — 33 檔（無 EXIF 無檔名日期的下載梗圖，mtime=2026-03-20）會被打成 2026 日期。建議跑前先確認：要嘛接受 2026 命名、要嘛跑前手動把 mtime 改成 2016-03-19 → rename → 改回 mtime；或考慮整個 `新增資料夾` 排除掉
