...
- **引擎與 GPU 鎖的真相**：電子圖書館的 `mineru_ocr.py queue` 是**整個佇列期間握著 GPU 鎖**
  ...
  10–20 頁的段攤下來只要 8–10 秒；🚨 排程班預設優先權 7＝低於正常，機器忙時掉到 27 秒／頁，已把任務 Priority 改成 5），一班 25 分鐘約 60–150 頁；GPU 約 2–3 秒／頁。不搶鎖、不殺別人、不改電源。第一班 09-25 10:50 實跑：GPU 回 4 → CPU，18 頁 512 秒，切出單篇 3 件，續跑點正確。
