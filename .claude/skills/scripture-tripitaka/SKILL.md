...
description: 佛教大藏經（/tripitaka）—— 把 CBETA 的《大正新脩大藏經》《卍新纂大日本續藏經》與《漢譯南傳大藏經》全文收進站內（3,784 部・1.85 億字），並掛上梵／巴利／藏原典對照。含 CBETA TEI P5 的解析規矩（缺字、異文、夾註、偈頌、悉曇）、佛典「段」該怎麼切與怎麼編號（學界對此沒有共識，本站定調為大正藏頁欄行）、SuttaCentral 平行經目的對接與其陷阱、以及三級對照來源的分辨。Use when 要補抓／重解析某部經、要新增一種原文對照（梵本、藏譯、漢譯南傳、白話）、要改 /tripitaka 的頁面或 schema、要處理某部經解析成 0 段或字數異常少、或使用者提到「大藏經」「CBETA」「阿含」「巴利對照」「卍續藏」「續藏」「禪宗語錄」。與 [[scripture-canon]] 的基督教區平行，同掛在 /scripture-canon 之下。
---
...
### DK 的書目後設資料：84000（`scripts/tripitaka_derge_titles.py`）

`tripitaka_derge.py` 從 TEI 正文抽藏文題名（1,075／1,124），梵文題名、英譯題名與
譯者另從 **84000 的 Linked Open Data** 補（GitHub `84000/data-rdf`，**CC0**，
1,254 個 RDF 涵蓋 Toh 2–4458，甘珠爾加丹珠爾）。用 tarball 一次抓，不要逐檔抓 1,254 次。

補完：藏文題名 **99.4%**、梵文題名 77.8%、英譯題名 98.5%、譯者 41.9%。
...
1. **`title_sa` 有 21% 是假的。** 84000 在「這部經沒有梵文題名」時，`WAITOH`
   那一格塞的是英譯或 `[no Sanskrit title]`。照收的話 238 筆的梵文題名會是英文，
   而頁面顯示「梵文題名：The Sūtra of Vasiṣṭha」**看起來完全正常**。
   判準：梵文轉寫不會出現英文虛詞（the／of／on／to…）。修掉後 1,119 → 874。
2. **梵文題名裡有軟連字符**（U+00AD）。`Saddharma­puṇḍarīka` 看起來正常，
   但站上搜「Saddharmapuṇḍarīka」永遠查不到。要清掉 U+00AD／U+200B／U+FEFF。
3. **RDF 的四個 `skos:prefLabel` 都沒有 `xml:lang`**，只能靠掛在哪個
   `rdf:Description` 分辨：`WAITOH`＝梵＋英、`WATTOH`＝藏、`WAETOH`＝英＋譯者。
4. **兩邊的藏文題名比對不能用字串相等。** 本站抽的是正文全名，84000 給的是
   通行略稱（`ཤེར་ཕྱིན` 是 `ཤེས་རབ་ཀྱི་ཕ་རོལ་ཏུ་ཕྱིན་པ` 的標準縮寫），
   本站保留敬語前綴而 84000 多半省略。直接比會得到「879 筆不一致」，
   讓人以為切錯了七成八的經。改用**最長共同片段**（與縮寫慣例無關）：
   92.9% 有 ≥8 字母的共同片段、6.3% 有 5–7，合計 **99.2% 確認是同一部**。
   剩下 9 筆逐筆看，挖出本站的真實缺陷——**4 筆題名只抽到敬語前綴「聖」本身**
   （Toh 548／796／894／1035），正題名沒抓到。那幾筆改採 84000 的。

### DK 的漢譯對照：東北目錄（`scripts/tripitaka_derge_zh.py`）

甘珠爾是藏譯，**沒有「漢文書名」這種東西**。能填的只有「漢譯對照本」——
同一部印度原典另外譯成漢文的那一部經。這個關係不能猜，只能抄目錄。

來源＝**東北大學 touda.tohoku.ac.jp 的西藏大藏經資料庫**，即 1934 年宇井伯壽等編
《西藏大藏經總目錄》（Toh 編號的出處）的官方數位化。每筆詳目的「内容構成」表
列出漢譯對照本並附 **SAT No.＝大正藏經號**。爬法：`canon/{id}`，
id ＝ `1501001` ＋ 7 位流水號，甘珠爾是 1–1109。

| | |
|---|---|
| 涵蓋 | **447／1,124（39.8%）** 有漢譯對照（單一 229、多部 218） |
| 無漢譯對照 | 661（多為密續，本來就沒漢譯） |
| 東北目錄無此條 | 14（Esukhia TEI 細分出的後綴本 `539a–f` 等，留空不套母條） |
| `title_zh` | 主對照本的**繁體**書名 | 
| `zh_parallels` | `[{id, t, title}]` 全部對照本；`checked:false` ＝ 待判讀 |

**做法的核心：只取 SAT 經號，書名回我們自己的 CBETA 目錄拿。**

#### 🚨 這一步踩到的六個坑

1. **絕對不要抄東北大那一欄的漢文書名。** 它是日本新字體，而且有系統性錯字。
   光 Toh 1 一部就四處（`膳意事`→隨意事、`設`→說、`毘奈那`→毘奈耶、`薬`→藥），
   另見 `大賓積經`（寶→賓）、`大般若波羅蜜多罷`（經→罷）、`增ー阿含親`、
   `觀世昔菩薩`（音→昔）、`無凝`（礙→凝）、`佛說大般泥恆經`（洹→恆）。
   照抄會得到一個長得完全像漢文書名、其實是日文加錯字的字串，**肉眼看不出來**。
   取經號回查 CBETA 目錄，這些全部自動校正。

2. **一格可以掛多個 SAT，`.search()` 只會拿到第一個。** 而且多個書名是
   **直接黏在一起、沒有分隔符**的（Toh 44：`大方廣佛華厳経［八十華厳］（…）普賢菩薩行願讃`
   後接兩個連結）。用 `.search()` 的話「多部對照」從 **218 掉到 1**，
   Toh 176 從三部縮成一部，而目錄照樣有值。要 `finditer`。

3. **主對照本不能用「經號最小」挑。** Toh 120（大般涅槃經）對到
   T0037／T0375／T0376，取最小號會挑到《緣本致經》——一部毫不相干的小經。
   正解：東北大一格裡只寫**一個代表性書名**，那就是它認的主對照本；
   用 `jp2t` 抹平字形後拿它去比，比中的那一部當主。

4. **CBETA 的分卷有兩種，不能一律取第一個。**
   (a) 真的是不同的經共用一個大正藏號（T1005A《大寶廣博樓閣善住祕密陀羅尼經》
   vs T1005B《寶樓閣經梵字真言》、T0974A–F）→ 拿書名比對挑。
   (b) 只是按檔案卷段切開、書名一樣（T0220a–p）→ **把卷段括號拿掉回傳本名**。
   不分的話，Toh 8／9／10／12… 十二部不同的般若經會全部顯示成
   「大般若波羅蜜多經(第1卷-第200卷)」，而 Toh 10 其實是第三會。
   另外密教部的分卷用**大寫** A／B，般若部用**小寫** a–p，只認小寫會漏 7 個經號。

5. **對帳閘只能用在「一格一個 SAT」。** 一格多個 SAT 時它只寫一個代表性書名，
   逐名比對必然失敗——實測 50% 門檻擋下 37 筆，其中 **36 筆是對的**。
   而且對不上也不代表錯：多半是「藏文對到漢譯某部經**當中的一品**」
   （Toh 289「頻鞞娑邏王迎佛經」＝《中阿含經》第 62 經，所以經號給母經、
   書名寫品名）。所以閘**只標記不丟棄**，確定錯的才進 `UPSTREAM_WRONG`。

6. **東北大自己會標錯經號。** Toh 119 書名寫「大般涅槃経（巻第一…巻第四十）」，
   SAT 卻掛 **0874**＝《金剛頂一切如來真實攝大乘現證大教王經》，一部密續
   （多半 0374 打成 0874）。照收的話涅槃經底下會冒出一部金剛頂經，
   頁面上完全看不出異常。正確經號無從由該資料推定，**留空不臆造**。

#### 試過而不能用的四條路（別再試一次）

* **84000 `data-rdf`** — 1,254 個 RDF 檔，**零個漢字**，只有藏／梵／英。
* **rKTs** — 藏文本位（強項是各版甘珠爾的版本異同），頁面裡沒有 Taishō／南條編號。
* **SuttaCentral parallels** — 德格↔大正藏只有 **14 組**，落在我們目錄內 13 組。
* **Lancaster《高麗大藏經解題目錄》**（acmuller.net，有 Tōhoku 索引）— 全藏只標了
  391 個 Toh，甘珠爾段 **313 個（27.8%）**，是隨手註的交叉參照不是系統性對照表。
  站台只走 http，https 會 ECONNREFUSED。

#### 兩個一般性的教訓

* **`--catalog-only` 會洗掉別的腳本併進來的欄位。** `tripitaka_derge.py`
  用固定欄位白名單重出目錄，`title_sa`／`title_en`／`translator_en`／
  `title_zh`／`zh_parallels` 都不在裡面，重跑一次就沒了，而部數照樣 1,124、
  結構照樣完整。已改成 `_merge_kept()`：非本次解析產出的欄位一律原樣帶回。
...
Toh 113 藏文題名直譯是「正法白蓮華」，漢譯正式書名是**《妙法蓮華經》**。
把前者填進去，讀者會以為那是真的漢譯書名。沒有對照表的那 661 部，**寧可空著。**
...
| 規模 | **3,784 部・184,871,342 字・2,097,345 段**，零空檔（T 2,471／X 1,230／N 83；X 幾乎讓全藏字數翻倍） |
...
scripts/tripitaka_derge_titles.py  甘珠爾補梵／英題名與譯者（84000 RDF）
scripts/tripitaka_derge_zh.py      甘珠爾補漢譯對照（東北大學東北目錄，SAT 經號）
...
25. **🚨 建好了不等於看得到 —— 本機 `out/` 與 Drive `_tripitaka/` 是兩份。**
    ...
    **🚨 根因是各腳本的產出目錄不一致，2026-08-31 又咬了一次：**

    | 腳本 | 寫到哪 |
    |---|---|
    | `tripitaka_cbeta.py`（正文 JSONL） | **Drive** `_tripitaka/`（`.env` 的 `TRIPITAKA_DIR`） |
    | `tripitaka_sanskrit.py`／`tripitaka_vernacular.py` | **本機** `C:/tmp/cbeta/out` |

    `--push-r2` 原本寫成「本機目錄存在就用本機」，於是推卍續藏那 2,480 個檔時
    它去掃本機（沒有 X）、發現本機的檔 R2 都已有，印出 **「✓ R2 0 檔」當作成功**
    —— X 一個都沒上去，而指令回傳 0、看起來完全正常。
    已改成一律以 Drive（正本）為來源，並在開頭印出來源路徑與檔數。
    **加新腳本時先確認它寫哪個目錄，別假設大家都寫同一處。**
...
python scripts/tripitaka_derge_titles.py --merge   # 併入 84000 的梵／英題名
python scripts/tripitaka_derge_zh.py --fetch       # 爬東北目錄詳目（可續跑，約 40 分）
python scripts/tripitaka_derge_zh.py --audit       # 涵蓋率與對帳閘，不寫檔
python scripts/tripitaka_derge_zh.py --build       # 併入 title_zh／zh_parallels
```
...
## 記憶庫併入：project_tripitaka

2026-08-28 建置。`/scripture-canon` → 佛教 → `/tripitaka`，與基督教區八工具、
[[project_hellenika_dazangjing]] 平行。skill：`scripture-tripitaka`。

**規模**：大正藏 T01–T55＋T85（2,471 部）＋**卍新纂續藏 X01–X88（1,230 部）**＋漢譯南傳 N01–N70（83 冊）
＝ **3,784 部・1.85 億字・210 萬段**（2026-08-31 收 X，字數幾乎翻倍）。原文對照（**去重後、且以 Drive 上實際有的
檔為準**；建置腳本 stdout 是去重前的，本機 out/ 又常與 Drive 不一致，
對外兩個都別抄 —— 見 [[feedback_build_not_equal_deployed]]）：
巴利 4,582 區塊／53.7 萬行／20 部、漢譯南傳 3,487 區塊／14.1 萬行／18 部、
梵文 137 區塊／12 部、藏文 29 區塊／3 部、**繁中白話 2,964 段／14 部（100%）**；
平行經目 23,321 筆（87% 對到段落）、CBETA 漢梵巴詞條 29,930 組、
大正藏原註巴利對應 1,597 條（99.8% 貼回段落）。
法華與心經已是漢梵藏三語。存放：Drive **7,798** 檔（2026-08-31 收 X 後）。

**段號定調（使用者定的，別再改）**：漢文佛典沒有章節制，段的引用式一律用
**大正藏頁欄行**（`T09n0262_p0008a13`），不自編號。跨語言主對齊層是**品**；
阿含類另可對到經、論頌類對到頌、律部對到條，其餘大乘經品以下無學界共識。

🚨 但**行號不是唯一鍵** —— 同一行可起頭好幾段（大智度論 p0416b07 上有 20 段
問答），全藏 6.5%／672 部如此。故每段兩欄：`seg`＝行號（引用式，可重複）、
`uid`＝同行第二段起加 `.2`（對照／詞條／DOM anchor 的鍵）。拿行號當鍵會讓
對照在同行所有段上一起冒出來，而頁面看不出異狀。

**對照來源分三級必須分色**：大正藏原註（綠，最權威）／CBETA 詞條（藍）／
SuttaCentral（靛）／本站對齊（琥珀）。混為一談就毀了這個工具的價值。

**存放**：目錄進 Supabase（**3,784 列**）；1.85 億字正文一律 file-backed，
Drive `_tripitaka/` 正本、R2 `tripitaka/` 後備。遵守
[[project_db_quota_rescue]] 的大表禁回 DB。

**梵藏對齊有一道閘**：品數不合一律拒絕逐品對齊，要對就得手寫 `chapter_map`。
🚨 但閘只擋得住**數量不符**，擋不住**數量相同而次序不同** —— 法華我第一版
手寫的表就錯了四個品：以為梵漢只差「提婆達多併入見寶塔」一處，寫成「梵 12 起
加一」；實際羅什另把囑累品移到第 22、陀羅尼排到第 26（梵藏本陀羅尼 21、
囑累最末 27）。**手寫的對照表本身也要逐條驗**。

**藏文正文來源**：84000/data-tei 是英譯不能用（藏文只在術語表）；
要用 84000/data-translation-memory 的 TMX（按 Toh 號命名的藏英逐句對齊，383 部）。
多數 Toh 的 TMX 沒有章標記，漢本有品時會被閘擋下。

**尚未完成**（2026-08-31 更新）：梵文 12 部、藏文 3 部（GRETIL 有 194 部、
84000 TMX 383 部）；梵文另有 4 部被對齊閘擋著要手寫 `chapter_map`
（大乘莊嚴經論／菩提行經／道行般若／佛所行讚）；
悉曇字 SD-xxxx 缺字表沒有（X 真言宗那區 18.7% 的 <g> 查不到，退到 [SD-xxxx] 佔位符）；
漢譯南傳已接八本（長部 34／中部 152／相應部 56 相應等硬閘全過），
**增支部刻意不接**（集號時有時無、一集可跨冊，兩種判準得 9 與 15 集、
定數是 11，要先逐冊人工核定集界）；律部條級對齊、T56–84 日本撰述部
（CBETA 未提供 XML）仍無。繁中白話已 100%（14 部）。

🚨 這批程式的錯幾乎全是「看起來成功的失敗」—— 動手前先讀 SKILL.md 的
「踩過的坑」三十三條（`cb:` 命名空間靜默回 0 筆、`sa-2.180` 被誤對成雜阿含第 2 經
還算成命中、南傳經號是冊內序號、律部對照塌成整部重複三萬七千次…）。
呼應 [[feedback_reader_silent_failures]]。

**卍續藏 X（2026-08-31 收）**：CBETA 是**選錄**（1,230 部／經號到 1671）。部類抓站方原書目錄 `cbdata.dila.edu.tw/stable/catalog_entry?q=orig-X`（`api.cbetaonline.cn` 連不通、`/v1.2/` 全 404），7 部類／60 子類。🚨 X 特有的五個靜默錯：6 部書拆兩檔同經號會互蓋、每行標兩套行號（`ed="X"` 與影印本 `ed="R0NN"`）、禪宗語錄 `<cb:dialog><sp>` 與辭書體 `<entry><form>` 沒走訪會丟三百萬字、`division_of` 的 else 會把新藏套到南傳表。全部見 SKILL 踩坑 27–33。

**索引壓縮時移入（2026-09-11）：**
- 巴利4582區塊/53.7萬行(20部)、漢譯南傳1974區塊(17部)、梵文94區塊(7部)、藏文29區塊(3部,用84000 TMX非data-tei)
