...
**這一支目前完全沒動過**——條目與簡介都寫好了，但 89 條 `fragment` 的 `via` 只有
散文式的轉引來源，沒有一條填了標準殘篇集編號。這是《希臘羅馬大藏經》現在**最大的
一塊空白**，也是三支取源 skill 裡唯一還沒開工的（[[hellenika-epigraphy]] 已完成
Κ 祭儀法卷四篇與 Χ 卷治癒銘文四石）。

建議順序：

1. **Ε 循環補遺**（11 條）——編號體系最成熟（Bernabé **PEG**），且全卷都是殘篇，
   做完一卷就能驗證整套流程
2. **Ζ 列祖傳**（12 條）——Merkelbach–West **M–W**，紙草復原成果豐富
3. **Α 神譜**的俄耳甫斯異版——PEG 與 Kern **OF** 雙標
4. **Φ 論神書**的前蘇格拉底與斯多噶——**DK**／**SVF**
5. **Τ 神諭書**的德爾菲諸則——附 **Fontenrose 分級**（Q/H/L），那本身就是可信度判準
6. **Λ 金葉片**——Graf–Johnston 編號，與 [[hellenika-epigraphy]] 的取原文工作合併做

其餘：

- ⏳ 9 條 `hostile` 的 `intro` 已寫，但尚未逐段做 §B2 三級標註（直引／轉述／敵意框架）。
  那需要先取得原文（ANF 的奧利金《駁塞爾蘇斯》站上已有，見 [[scripture-fathers]]），屬下一階段。
- ⏳ 考慮在 `HellenWork` 加 `siglum?: string` 欄專放編號——目前 [[hellenika-epigraphy]]
  的銘文編號也只寫在 skill 裡沒進資料檔，兩邊同一個問題，宜一次解決。
