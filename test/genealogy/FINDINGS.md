...
## ⏳ 待你定調：教座分裂的「平等分叉再回歸」呈現
- 現況：分裂支（`is_split`，39 條）畫成主軸旁一條虛線「繞出再回來」的側枝，非對等分叉。
- 你要的「平等分成兩支/三支再回歸軸線」需重排 layout（分配對等的平行欄、畫分歧+匯流）。
- **資料現實**（稽核所得，影響設計）：分裂分布極不均 —— 安提阿 **12** 條、羅馬 7、塞琉西亞 5、
  亞歷山卓 4、君士坦丁堡 3，其餘多為 1 對 1；且多數是**永久**分裂（天主教/東正教/聖公會不回歸），
  只有少數**暫時**分裂會「回歸軸線」（如 1378-1417 亞威農/比薩對立教宗，最後併回羅馬）。
- 所以「2-3 對等分叉 + 回歸」只乾淨適用一部分；12 路的安提阿無法對等三分。需要你選定呈現策略
  （見對話中的提案）再動手 —— 屬主觀視覺、先對齊再改。
