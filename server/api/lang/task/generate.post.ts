...
export default defineEventHandler(async (event) => {
  ...
  const { language, mode, exam, skill, taskType, topic, direction, usePaid } = (await readBody(event)) as {
    language: string;
    mode?: string;
    exam?: string;
    skill: string;
    taskType?: string;
    topic?: string;
    direction?: string; // 翻譯方向：zh2target / target2zh
    usePaid?: boolean;
  };
  ...
  // 翻譯遊戲（中翻外 / 外翻中）
  const dir = direction === "target2zh" ? "target2zh" : "zh2target";
  let system: string;
...
{
  "topic": "主題",
  "prompt": "翻譯指示（繁中說明要把下面這段翻成什麼語言）",
  "source_text": "${dir === "zh2target" ? "要翻譯的繁體中文原文（2–4 句，含該程度該練的句型/詞彙）" : `要翻譯的 ${coach.langLabel} 原文（2–4 句）`}",
  "tip": "翻譯重點提示（繁中，如：注意時態/語氣/慣用語）"
}
...
${exam ? "" : "題材以宗教／宗教學／人文為主，偶爾理工醫/生活/旅遊。"}繁體中文不可簡體。`;
  }
  ...
  if (skill === "translation") {
    materials.source_text = g.source_text ?? "";
    materials.direction = dir;
  }
  ...
  const { data: task } = await supabase
    ...
    .insert({
      ...
      skill,
      task_type: skill === "translation" ? `translation_${dir}` : taskType ?? null,
      level: lv,
