...
7. Build 100 memory units after vocabulary is fixed:
   ...
   - **Then move the books to Drive.** `output/` is scratch, not the products'
     ...
     `output/print-masters/` and **deletes** the superseded render（2026-09-25 起不再留
     `_superseded/`，舊版留在 git 的資料層就夠；Drive 分夾：讀本／印刷母版\讀本，
     單字卡／印刷母版\單字卡，撲克牌只在 印刷母版\撲克牌）. Reader PDFs and DOCX never go to R2 — see
...
9. Build the word-by-word layer over every running-text unit before layout（🚨 2026-09-25 起紙本不印這一層，只印一段原文一段中譯；逐詞層仍是線上讀本與資料閘的一部分）:
