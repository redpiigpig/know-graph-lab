...
## 2026-09-25 擁有者裁示：行距 1.5–1.8、作答線要能寫字、一課放寬到十頁

看了 09-24 那版之後的裁示，四本一起改，全部在 `build_hebrew_full_reader` 的常數與
共用函式裡（希臘那本自己複製的一份逐詞行距也同步改了）：

| 項目 | 舊 | 新 |
|---|---|---|
| 中文正文 `BODY_LINE_MULTIPLE` | 1.25 | **1.6** |
| 逐詞對譯：原文列／詞義列行距 | 1.18／1.0 | **1.3／1.35** |
| 逐詞對譯：單元內折行的列間距 `INTERLINEAR_LINE_GAP_PT` | 3.5 | **7** |
| 整句中譯行距／段後 | 1.3／9pt | **1.6／12pt** |
| 練習作答線 | 一條 15pt（5mm，寫不下一句中文） | **兩條各 34pt（12mm）**，`add_answer_lines()` 四本共用 |
| 一課頁數上限 `PAGE_LIMIT`／`MAX_LESSON_PAGES` | 8 | **10** |

實測（希伯來第 4／15／25／35／45 課）：7→9、8→9、7→9、4→5、5→6 頁；十題練習
剛好 5＋5 佔兩頁。行距量法：PDF 同一段落相鄰兩行的基線距 ÷ 字級。

重排後七冊變成 **3,175 頁／十一冊**（原 2,632 頁／七冊），超過 500 頁的都照擁有者
「內容不動、再分冊」切開，切點把整冊厚度（含只印在後一本的附錄）算平：

| 書 | 冊 | 課次 | 頁數 |
|---|---|---|---:|
| 聖經希伯來文 | 上冊／下冊 | 1–25（聖經）／26–50（禱文＋Haggadah＋附錄） | 219／291 |
| 通用希臘文 | 上冊／下冊（一）／下冊（二） | 1–50／1–35／36–50＋附錄 | 468／301／316 |
| 教會拉丁文 | 上冊（一）／上冊（二）／下冊（一）／下冊（二） | 1–30／31–50＋附錄／1–28／29–50＋附錄 | 253／261／253／251 |
| 日文宗教學 | 第一冊／第二冊 | 1–50／1–50 | 402／411 |

🚨 希伯來本來沒有 PARTS，這一輪才加（`build_hebrew_full_reader.PARTS`／`part_label`／
`part_output`，`HBO_LESSON_SUBSET` 的版面樣張仍出單檔）；`qa_hebrew_full_reader` 的
預設 DOCX／PDF 路徑還指著單冊的 stem，要跑就 `--docx`／`--pdf` 指定某一冊。
🚨 拉丁有 48 課印到 11–13 頁：生詞表的主要部分（median 22 字母）在 12pt 下折行，
二十個詞佔兩頁半——這是 09-18 就記過的物理極限，不是這輪的版面浪費；
`audit_reader_pages` 對拉丁四冊因此是紅的，別為了它降字級。
🚨 「目錄開始要換頁」的病根：`drop_spacer_before_break` 只看 `pageBreakBefore` 元素
在不在，而 python-docx 把 `page_break_before = False` 寫成 `val="0"`——目錄前面真正的
分頁段落（`document.add_page_break()`，空文字）於是被當成 spacer 刪掉，四本書的目錄
都接在體例頁後面，而每一道關卡都放行。現在 val 為 0／false 不算換頁。

同一天一起定的：

- **封面不印規格行**（「50 課．1000 詞．讀本 N 詞（課次編號與線上讀本一致；分冊
  只是印刷單位…）」、希伯來的「五十課・一千詞・…」副標、日文的「N 課．N 詞…」、
  希臘體例頁的兩行合計）。**凡例不印發音段、不印著作權段**（拉丁的「全書採羅馬式
  教會發音…」「所引各版本之著作權…」、希伯來「原文與音標原則」改「原文原則」並拿掉
  音標那一條、日文的重音欄說明與附錄一的權利註記）。生詞表裡的 BBH2 音標欄與日文
  重音欄**保留**——擁有者說「只拿掉說明段」。
- **拉丁字母與阿拉伯數字一律 Times New Roman。** 實作在 `set_rfonts`：`w:ascii`
  釘死 `FONT_LATIN`，`w:hAnsi` 也是，除非該字型登記在 `HANSI_KEEP`（希臘字母走
  hAnsi 槽，`build_greek_full_reader` 把 Palatino 登記進去）。`FONT_TRANSLIT`／`FONT_LA`
  也改成 Times。驗法：`page.get_page_fonts()` 看到 TimesNewRomanPSMT，且生詞表的
  課次數字 span 的 font 是它。
- **目錄另起一頁**（四本本來就有 page_break；重出後要在 PDF 上確認「目錄」標題在頁首）。
- **Drive 分夾**：`印刷母版\讀本\`、`印刷母版\單字卡\`、`印刷母版\撲克牌\`；
  「讀本」夾不再放撲克牌。`sync_reader_artifacts.targets_for` 已改。
