...
`sync_reader_artifacts.retire()` 把作廢的版次搬進 `_superseded`，但它只掃
`output/original-readers` 與 Drive 的「讀本」——**Drive 的「印刷母版」沒掃**。
那一夾正是送印時會被打開的那一夾。2026-09-17 作廢的希臘第五、六冊在裡面躺到
2026-09-18 才被發現，旁邊還多了並冊後作廢的另外五冊；每一份都打得開、頁面完全
正常，只是那本書已經不存在了。腳本第一節（對齊副本）一直有掃那一夾，所以「過期
0 份」讀起來像是全部乾淨。

**推廣**：一支負責「清掉舊東西」的腳本，要逐夾檢查它**實際**走過哪些路徑，不要
看它宣稱管幾處。同一支腳本的兩個階段掃不同的範圍，是最容易漏的形狀。

## 待辦事項寫在引文出處欄裡，就會印上紙本

希臘下冊第 30 課讀以弗所會議的八條教規，而那一欄的 `source` 掛著一句
「Cyril 致 Nestorius 第二封信、12 anathemas 之希臘原文仍待補（候選來源 Schwartz
ACO Vol 1）」。`source` 是**會印出來的引文出處**，所以那句研究待辦跟著上了紙本；
`audit_reader_pages.py` 報的「還沒補完的欄位」就是它。

而且那個「缺口」其實不是這一課的缺口：本課題目就是「教規」、`extent` 也寫著
「教規本文」，信件與十二章從來不在這一課的範圍內。待辦被寫進出處欄之後，看起來
卻像這一課少了東西。

**推廣**：出處欄要說這一份材料**是什麼**，不是說它還缺什麼。研究待辦寫進程式碼
或資料檔的註解，那裡不會被印出來。

## 版面關卡全綠，內容錯了一千七百條（2026-09-23）

七冊 2,632 頁把 3d、3e 兩道逐頁光柵關卡、四支資料層驗證器、八頁／12pt 兩條硬約束
全部跑綠，Drive 與本機 SHA1 一致——然後逐頁**讀**了一遍，記下 1,771 條錯（高 509）。
沒有任何一道既有關卡能看見其中一條，因為它們問的都是「排得對不對、數得齊不齊」，
沒有一道問「這個詞在這句裡是不是這個意思」。這一輪的四種病灶，每一種都值得單獨記：

1. **按詞不按出現的對譯，錯一次就錯到底。** 日文逐詞層原本一個詞形只問模型一次、
   全書共用（`子|名詞→私生子`、`行|名詞→行政區域`、`なん→難處`），再加上 janome 不認
   歷史假名遣（なんぢ→なん＋ぢ、たまふ→たま＋ふ、あつた→あつ＋た），第二冊文語課
   幾乎每一行都錯。改法不是逐條修詞義，是把設計換掉：逐句在上下文裡斷詞＋對譯，
   並用一道硬閘——**所有 surface 依序接起來必須等於原文**——擋住任何切錯的斷詞
   （`build_japanese_interlinear.py` 第二版）。
2. **爬蟲殘留印進正文，還被逐詞對譯。** `# Trent Session 4 — Latin # Source: …`
   （Source→來源）、每頁頁尾的 `www.internetsv.info` 與孤立頁碼「1」、`&lt;oratio&gt;`、
   `## DE FIDE`、`[^[2]]`、ΤΟ ΑΚΟΥΤΕ（來源網站的「聽音檔」連結字）、Schaff 的異文
   校勘與術語索引整段混進信經正文。清洗要放在**唯一**一個入口
   （`translate_latin_readings_zh.clean_paragraph`），翻譯前切段、builder 備援讀檔、
   快取修補都走它；修快取的時候，每一層按段號索引的資料（interlinear 鍵、gap 鍵、
   練習 `answerKeyRef`、語料 `L26#1`）都要跟著重編號。
3. **抓錯了整篇也一樣過關。** 拉丁第 9 課印的是 Migne／Baluze 的編者導言與註腳
   （島嶼經緯度、「Baluzius」），不是味增爵《備忘錄》——因為 `divisions()` 把註腳的
   「[1] [2] …」認成了節號。標題、段數、字數、覆蓋率全部正常。
4. **給維護者看的字印在課本上。** 「拉丁 25 節，中文 26 節，需逐節核對」（scripture
   的 alignmentNote）、`content_complete_layout_pending`、「TTS 不算數」、
   「pdftotext UTF-8 抽取」、「角色由排版推定…不需重跑整份」、「人工填寫（2026-09-08）」。
   每一個都來自一個「順手把資料欄印出來」的 add_body。規則：**印出來的欄位，只能是
   為讀者寫的欄位**；維護註記寫在程式註解或另立的 note 欄裡。

**推廣**：機械關卡過了只代表「印得出來、排得整齊」。付印前一定要有一道**內容**關卡：
逐頁讀（人或模型），原文、逐詞、整句、練習、附錄五層各挑得出錯才算跑過。

## 修了快取、沒重組主檔，垃圾自己長回來（2026-09-23）

拉丁的 `build_latin_interlinear.py` 斷詞的來源是**主檔** `latin-reader-two-volumes.json`，
不是 `readings-zh.json`。清乾淨 readings-zh 與 interlinear 之後直接重跑逐詞層，
它拿舊主檔的髒文字重新斷詞，把 `(DCO)`、`&lt;oratio&gt`、`[^[2]]` 一個不少地長回來，
builder 印的又正是 interlinear 的 token，所以成書和修之前一模一樣——而驗證器全綠。
順序永遠是：資料 → `build_latin_reader_data.py --write` → 逐詞層 → builder。

**推廣**：改資料之前先問「這一層下游的每一支腳本各從哪個檔讀」，有主檔的管線，
主檔就是必經的一站，跳過它等於沒改。
