...
- **附錄那幾張表原本印的是英文。** 〈數字、羅馬數字與度量衡〉〈親屬稱謂〉〈羅馬
  ...
  這 548 條**尚未人工覆核**，與課內釋義同一個狀態。抽讀時看到的問題都是「忠實翻譯
  了一句古怪的英文詞條」而不是憑空捏造：`avia` 的 Whitaker 釋義是「grandmother;
  rooted prejudice, old wives tale」，中文就出現「祖母、祖先、老派故事」。覆核時對
  照 `glossEn` 讀，會比只讀中文快得多。

## 2026-08-28

- **逐詞對譯層建好了**（`scripts/build_latin_interlinear.py`，2,438／2,440 單元、
  84,058 個可對譯詞）。單元取自組好的主檔，所以對譯列與書上印的那一行必然對齊。
  網頁課頁已顯示；**紙本還沒**——加上去下冊會從 488 頁膨脹到近千頁，跟希臘下冊
  1,184 頁是同一個裝訂問題，等使用者定奪。
- **單字卡配圖 1,996／2,000。** 手挑 631 張；留白的四張是族屬形容詞，照體例歸附錄。
- **三個會被當事實背起來的錯已修：**
  1. 同形異義的中文詞義撞號（詞義層鍵在 headword）：labor 名詞印著動詞的意思、
     mundus 形容詞印著名詞的意思、liber 形容詞印著名詞的意思，共六筆。已改鍵到
     整條詞典行（forms），圖也一樣。
  2. OCR 把 Collins 的「satis (+ partitive gen.)」切斷，尾巴「titive gen.」成了
     一個詞條——兩千個課內詞裡有一個根本不是詞。
  3. 詞類欄十張空白：短標籤表沒有「片語」這一格，in aeternum、grātiās agere
     這類就靜靜掉出去。現在 2,000／2,000 全有詞類。
