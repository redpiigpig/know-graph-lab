...
## 英文托福 B2–C2 六千字（2026-09-25，Quizlet 專用，沒有印刷卡）

來源不是讀本詞表，是語言教練的共用字庫 `lang_vocab_bank`（en，三萬字，依詞頻分帶
A1/A2/B1/B2/C1/C2，每字有繁中釋義、例句、18 類語意主題）。`scripts/export_quizlet_english_b2c2.py`
取詞頻 4001 起、**濾掉專名**（釋義寫人名地名姓氏縮寫的、例句裡只以大寫形出現的——
頻率表混了 harris、wendy、sydney 這種，第一版第 1 課開頭就是 graham、pierre、jen）後
取前 6,000 字（實際落到詞頻 11,115）。使用者定：**25 字一課、相關的分在一起**——按語意
主題（具體→抽象→宗教）排，同主題內按詞頻，每 25 字一課，共 240 課。產物：三個 2,000
字大檔（Quizlet 單集上限）＋每課一個小檔＋課次表，在 `output/flashcards/quizlet/` 與
Drive 單字卡 底下 Quizlet匯入 子夾。🚨 字庫的「B2／C1」是詞頻帶不是 CEFR 官方分級。
