<template>
  <div class="bl-page">
    <div class="bl-topbar">
      <NuxtLink to="/pong-archive/sermons/by/type" class="bl-back">← 按類型瀏覽</NuxtLink>
    </div>

    <header class="bl-header">
      <p class="bl-eyebrow">By Type</p>
      <h1 class="bl-title">{{ typeName }}</h1>
      <p class="bl-meta">{{ sermons.length }} 篇</p>
    </header>

    <section class="bl-body">
      <div v-if="!sermons.length" class="bl-empty">此類型目前沒有講道記錄。</div>
...
const typeName = computed(() => decodeURIComponent(route.params.type))
...
const { data: sermons } = await useAsyncData(
  () => `pong-sermons-by-type-${typeName.value}`,
  async () => {
    const { data, error } = await supabase
      .from('pong_sermons')
      .select('id, sermon_date, title, occasion, location, sermon_type')
      .eq('is_published', true)
      .eq('sermon_type', typeName.value)
...
.bl-item { display: grid; grid-template-columns: 100px 1fr; gap: 16px; padding: 14px 0; border-bottom: 1px solid #ECE7DD; align-items: baseline; }
.bl-item:last-child { border-bottom: none; }

.bl-date { font-family: 'Noto Serif TC', serif; font-size: 0.78rem; color: #9A9080; letter-spacing: 0.04em; white-space: nowrap; }
.bl-content { min-width: 0; }
.bl-link { font-family: 'Noto Serif TC', serif; font-size: 0.95rem; font-weight: 500; color: #3A3025; letter-spacing: 0.03em; text-decoration: none; transition: color 0.2s; }
