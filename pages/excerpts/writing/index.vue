<template>
  <div class="min-h-screen bg-gray-50">
    <AppHeader title="著作書摘" :back="{ to: '/excerpts', label: '書摘庫' }" container-class="max-w-5xl">
      <template #actions>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div v-else-if="projects.length" class="grid sm:grid-cols-2 gap-5">
        <NuxtLink
          ...
          :to="`/excerpts/writing/${proj.id}`"
          class="group bg-white rounded-2xl border border-gray-200 hover:border-purple-300 hover:shadow-lg transition-all duration-200 p-6 flex flex-col"
        >
          ...
          <div class="mt-auto flex items-center gap-4 text-sm text-gray-500">
            <span class="text-purple-600 font-semibold">{{ proj.excerpt_count }} 筆素材</span>
...
async function fetchProjects() {
  ...
  projects.value = await $fetch<Project[]>("/api/projects?type=待寫著作", {
