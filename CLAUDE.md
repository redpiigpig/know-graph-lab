...
## 🚨 使用者改過的 Word 檔只能就地改，絕不重建覆蓋

交給使用者的 .docx 一產出就以 docx 為正本（使用者會直接在 Word 裡改）。寫入前先讀現檔、比對自己上次的輸出；
只就地改指定處，**禁止從 md／原稿／修訂稿重跑產生器存回同名檔**；資料夾裡有 `~$` 鎖檔＝Word 開著，一律不寫；
要大改就另存新檔。救援走 Drive「管理版本」。2026-09-24、09-26 已犯兩次。
...
筆電闔蓋睡眠、待機時管線停下來是**正常的**，醒來由 fleet keeper 接著跑就好。
**不准改 powercfg（闔蓋動作、睡眠逾時、待機），也不准建議使用者改**——
闔蓋還持續滿載會把電腦燒壞。進度慢沒關係，別為了趕進度耍小聰明。
